...
import { isDefined, normalizeUrl } from 'diex-shared/utils';
import {
...
const IMAGE = 'diexcrm/diex-app-dev:latest';
...
export class CreateAppCommand {
  ...
  async execute(options: CreateAppOptions = {}): Promise<void> {
    ...
    try {
      ...
      if (!skipLocalInstance) {
        this.logNextStep('Starting Diex server');
      ...
      if (serverReady && authSucceeded) {
        ...
        if (!syncSucceeded) {
          this.logDetail('Sync failed. Run `yarn diex dev --once` manually.');
          return;
        }
      } else {
  ...
  private async ensureDockerServer(
    ...
    this.logDetail('Ensuring latest Diex server image...');
  ...
  private async openMainPage(
    ...
    try {
      ...
      const token = config.diexCLIAccessToken ?? config.apiKey;
  ...
  private async syncApplication(appDirectory: string): Promise<boolean> {
    this.logDetail('Running `yarn diex dev --once`...');
    return new Promise((resolve) => {
      const child = spawn('yarn', ['diex', 'dev', '--once'], {
        cwd: appDirectory,
  ...
  private async tryExistingAuth(serverUrl: string): Promise<boolean> {
    try {
      ...
      for (const remoteName of remoteNames) {
        ...
        const token = remoteConfig.diexCLIAccessToken ?? remoteConfig.apiKey;
  ...
  private async authenticateWithDevKey(serverUrl: string): Promise<boolean> {
    try {
      ...
      console.log(
        chalk.yellow(
          '  Authentication failed. Run `yarn diex remote:add` manually.',
        ),
      );
  ...
  private async authenticateWithOAuth(serverUrl: string): Promise<boolean> {
    try {
      ...
      console.log(
        chalk.yellow(
          `  OAuth failed: ${result.error.message}\n` +
            `  Run \`yarn diex remote:add --url ${serverUrl}\` manually.`,
        ),
      );
      ...
      console.log(
        chalk.yellow(
          `  Authentication failed. Run \`yarn diex remote:add --url ${serverUrl}\` manually.`,
        ),
      );
  ...
  private logSuccess(
    ...
    if (!authSucceeded) {
      console.log(chalk.white(`  ${stepNumber}. Connect to a Diex instance`));
      console.log(
        chalk.cyan('     yarn diex remote:add --url <your-instance-url>\n'),
      );
      stepNumber++;
    }

    console.log(chalk.white(`  ${stepNumber}. Start developing`));
    console.log(chalk.cyan('     yarn diex dev\n'));
    stepNumber++;

    console.log(chalk.white(`  ${stepNumber}. Open your diex instance`));
    console.log(chalk.cyan(`     ${serverUrl}\n`));

    console.log(
      chalk.gray(
        '  Documentation: https://docs.diex.com/developers/extend/apps/getting-started/quick-start',
      ),
    );
  }
}
