...
import createDiexAppPackageJson from 'package.json';
...
describe('copyBaseApplicationProject', () => {
  ...
  beforeEach(async () => {
    testAppDirectory = join(
      tmpdir(),
      `test-diex-app-${Date.now()}-${Math.random().toString(36).slice(2)}`,
    );
  ...
  it('should update package.json with app name and SDK versions', async () => {
    ...
    expect(packageJson.devDependencies['diex-sdk']).toBe(
      createDiexAppPackageJson.version,
    );
    expect(packageJson.devDependencies['diex-client-sdk']).toBe(
      createDiexAppPackageJson.version,
    );
  });
