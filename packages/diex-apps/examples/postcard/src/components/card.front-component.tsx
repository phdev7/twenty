...
import { RestApiClient } from 'diex-client-sdk/rest';
import { defineFrontComponent } from 'diex-sdk/define';
import { useRecordId } from 'diex-sdk/front-component';
    ...
    return (
      ...
      >
        ...
        <div style={{ marginTop: '8px', fontSize: '11px', color: '#ccc' }}>
          ...
          {process.env.DIEX_API_URL ? 'set' : 'missing'} | token:{' '}
          {(process.env.DIEX_APP_ACCESS_TOKEN ?? process.env.DIEX_API_KEY)
            ? 'set'
            : 'missing'}
        </div>
      </div>
