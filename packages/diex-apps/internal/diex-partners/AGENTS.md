# Architecture — diex-partners

Vertical-slice modules, one folder per domain. Adapted from `diexhq/diex-eng`
(the largest Diex SDK app), which migrated flat → modular as it grew. Repo-wide
...
`<feature>/` level — `modules/<domain>/<feature>/<primitive>/` (the diex-eng standard,
