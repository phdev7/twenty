...
import {
  ...
  DIEX_EXPERIENCE_NOTES_MIN_LENGTH,
...
const longNotes = (suffix = ''): string => {
  const base =
    'Built a Diex workspace for Acme with custom objects, roles, and a sales pipeline. ';
  let notes = '';
  while (notes.length < DIEX_EXPERIENCE_NOTES_MIN_LENGTH) {
    notes += base;
  }
  return `${notes}${suffix}`;
};
...
const validRow = (
  ...
  "What you've built in Diex": 'Custom apps, Workflows',
...
describe('mapTallyExperienceCsvRow', () => {
  it('maps a valid Tally row to an update intent by partnerId', () => {
    ...
    expect(result).toEqual({
      ...
      intent: {
        partnerId: VALID_PARTNER_ID,
        diexExperience: ['CUSTOM_APPS', 'WORKFLOWS'],
        diexExperienceNotes: notes.trim(),
        diexExperienceProofLink: 'https://www.loom.com/share/abc123',
      },
    });
  });
  ...
  it('rejects unknown milestone labels on the row', () => {
    const result = mapTallyExperienceCsvRow(
      validRow({
        "What you've built in Diex": 'Custom apps, Self-hosting',
      }),
    );
  ...
  it('resolves Tally-style headers with helper copy via partial match', () => {
    const result = mapTallyExperienceCsvRow({
      partnerId: VALID_PARTNER_ID,
      "What you've built in Diex (select every area)": 'Data models',
    ...
    if (result.ok) {
      expect(result.intent.diexExperience).toEqual(['DATA_MODELS']);
      expect(result.intent.diexExperienceProofLink).toBe(
