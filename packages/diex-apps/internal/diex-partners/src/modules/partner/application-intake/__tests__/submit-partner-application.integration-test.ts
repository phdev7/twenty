...
describe('submit-partner-application handler — upsert', () => {
    ...
    const result = await handler(
      authedEvent(
        baseInput({
          ...
          partnerScope: ['ADVISORY', 'SOLUTIONING'],
          applicationNotes: 'Workspace https://app.diex.com/ws · refs: Acme',
        }),
      ),
    );
    expect(result.ok).toBe(true);
  ...
  it('persists diex experience fields on create', async () => {
    const diexExperienceNotes =
      'Implemented Diex for a logistics client: custom apps for shipment intake, ' +
    ...
    const result = await handler(
      authedEvent(
        baseInput({
          ...
          diexExperience: ['CUSTOM_APPS', 'DATA_MODELS', 'WORKFLOWS'],
          diexExperienceNotes,
          diexExperienceProofLink: 'https://www.loom.com/share/experience-create',
        }),
      ),
    );
    expect(result.ok).toBe(true);
    ...
    const fetched = await client.query({
      partner: {
        __args: { filter: { id: { eq: result.partnerId } } },
        diexExperience: true,
        diexExperienceNotes: true,
        diexExperienceProofLink: { primaryLinkUrl: true },
      },
    });
    const node = fetched.partner;
    expect(node?.diexExperience).toEqual(
      expect.arrayContaining(['CUSTOM_APPS', 'DATA_MODELS', 'WORKFLOWS']),
    );
    expect(node?.diexExperienceNotes).toBe(diexExperienceNotes);
    expect(node?.diexExperienceProofLink?.primaryLinkUrl).toBe(
      'https://www.loom.com/share/experience-create',
    );
  });

  it('persists diex experience fields on update for the same email', async () => {
    ...
    const second = await handler(
      authedEvent(
        baseInput({
          ...
          diexExperience: ['FRONT_COMPONENTS', 'WORKFLOWS'],
          diexExperienceNotes,
          diexExperienceProofLink: 'https://github.com/example/diex-case-study',
        }),
      ),
    );
    expect(second.ok).toBe(true);
    ...
    expect(node?.diexExperienceNotes).toBe(diexExperienceNotes);
    expect(node?.diexExperienceProofLink?.primaryLinkUrl).toBe(
      'https://github.com/example/diex-case-study',
    );
  });
