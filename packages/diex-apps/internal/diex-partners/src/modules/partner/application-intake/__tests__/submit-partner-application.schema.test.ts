...
describe('submitPartnerApplicationSchema', () => {
  ...
  it('accepts diex experience milestones, narrative, and proof link', () => {
    const result = submitPartnerApplicationSchema.safeParse({
      ...base,
      diexExperience: [
        'CUSTOM_APPS',
  ...
  it('rejects unknown diexExperience milestone values', () => {
    expect(
      submitPartnerApplicationSchema.safeParse({
        ...base,
        diexExperience: ['INTEGRATIONS'],
        diexExperienceNotes: validExperienceNotes,
        diexExperienceProofLink: 'https://github.com/example/case',
      }).success,
    ).toBe(false);
  });

  it('rejects diexExperienceNotes shorter than 200 characters', () => {
    expect(
      submitPartnerApplicationSchema.safeParse({
        ...base,
  ...
  it('rejects a non-http diexExperienceProofLink', () => {
    expect(
      submitPartnerApplicationSchema.safeParse({
        ...base,
        diexExperience: ['WORKFLOWS'],
        diexExperienceNotes: validExperienceNotes,
        diexExperienceProofLink: 'javascript:alert(1)',
      }).success,
    ).toBe(false);
  });

  it('rejects an empty diexExperience array when other experience fields are set', () => {
    expect(
      submitPartnerApplicationSchema.safeParse({
        ...base,
        diexExperience: [],
