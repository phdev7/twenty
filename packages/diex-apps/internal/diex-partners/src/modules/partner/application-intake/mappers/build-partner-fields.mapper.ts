import { type CoreSchema } from 'diex-client-sdk/core';
...
export type PartnerFieldsForUpsert = {
  ...
  diexExperience?: CoreSchema.PartnerDiexExperienceEnum[];
  diexExperienceNotes?: string;
  diexExperienceProofLink?: { primaryLinkUrl: string };
};
...
export function buildPartnerFields(input: SubmitPartnerApplicationInput): PartnerFieldsForUpsert {
  ...
  if (input.diexExperience !== undefined && input.diexExperience.length > 0) {
    fields.diexExperience =
      input.diexExperience as CoreSchema.PartnerDiexExperienceEnum[];
  }
  if (isNonEmptyString(input.diexExperienceNotes)) {
    fields.diexExperienceNotes = input.diexExperienceNotes.trim();
  }
  if (isNonEmptyString(input.diexExperienceProofLink)) {
    fields.diexExperienceProofLink = {
      primaryLinkUrl: input.diexExperienceProofLink.trim(),
    };
  }
  return fields;
}
