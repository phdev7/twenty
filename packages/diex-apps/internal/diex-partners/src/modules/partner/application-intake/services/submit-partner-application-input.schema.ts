...
export const submitPartnerApplicationSchema = z
  .object({
    ...
    diexExperience: z.array(z.enum(DIEX_EXPERIENCE_VALUES)).optional(),
  ...
  .superRefine((data, context) => {
    const hasMilestones = (data.diexExperience?.length ?? 0) > 0;
    const hasNotes = data.diexExperienceNotes !== undefined;
    const hasProof = data.diexExperienceProofLink !== undefined;
    ...
    if (!hasMilestones) {
      context.addIssue({
        code: 'custom',
        message: 'At least one Diex experience milestone is required.',
        path: ['diexExperience'],
      });
    }
    ...
    if (!hasNotes) {
      context.addIssue({
        code: 'custom',
        message: `Diex experience notes must be at least ${DIEX_EXPERIENCE_NOTES_MIN_LENGTH} characters.`,
        path: ['diexExperienceNotes'],
      });
    }
    ...
    if (!hasProof) {
      context.addIssue({
        code: 'custom',
        message: 'Diex experience proof link is required.',
        path: ['diexExperienceProofLink'],
      });
    }
  });
