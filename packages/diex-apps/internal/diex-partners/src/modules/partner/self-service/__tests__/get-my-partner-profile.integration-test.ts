...
import { type RoutePayload } from 'diex-sdk/define';
    ...
    createPartnerContent: {
      __args: {
        data: {
          ...
          body: { markdown: 'Moved 12 teams to Diex.' },
...
describe('get-my-partner-profile', () => {
  ...
  const originalToken = process.env.DIEX_APP_ACCESS_TOKEN;
  ...
  afterEach(async () => {
    if (originalToken === undefined) delete process.env.DIEX_APP_ACCESS_TOKEN;
    else process.env.DIEX_APP_ACCESS_TOKEN = originalToken;
  ...
  it('returns NO_PARTNER when the token userId matches no workspace member', async () => {
    ...
    process.env.DIEX_APP_ACCESS_TOKEN = makeToken({
    ...
    expect(result.profile.caseStudies).toEqual([
      {
        ...
        clientName: 'Acme Corp',
        headline: 'CRM migration',
        bodyMarkdown: 'Moved 12 teams to Diex.',
        coverImageUrl: null,
