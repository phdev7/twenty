...
// harness DIEX_APP_ACCESS_TOKEN is a crafted identity-only token (decoded for userId but
...
export const buildAppClient = (): CoreApiClient => {
  const apiKey = process.env.DIEX_API_KEY;
...
export const resolvePartnerFromRequest = async (event: {
  ...
  const claims = decodeJwtClaims(process.env.DIEX_APP_ACCESS_TOKEN ?? '');
