...
describe('resolvePartnerFromRequest guards (no network)', () => {
  const original = process.env.DIEX_APP_ACCESS_TOKEN;
  afterEach(() => {
    if (original === undefined) delete process.env.DIEX_APP_ACCESS_TOKEN;
    else process.env.DIEX_APP_ACCESS_TOKEN = original;
  });
