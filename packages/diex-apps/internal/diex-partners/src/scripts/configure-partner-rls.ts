...
async function main() {
  const baseUrl = requireEnv('DIEX_PARTNERS_API_URL').replace(/\/$/, '');
  const apiKey = requireEnv('DIEX_PARTNERS_API_KEY');
  ...
  if (missingLocks.length > 0) {
    console.error(
      `\n[rls:configure] DRIFT DETECTED: ${missingLocks.length} Opportunity field(s) ` +
        ...
        `  yarn diex dev --once -r <remote>\n\n` +
        `(e.g. \`yarn diex dev --once\` for local, ` +
        `\`yarn diex dev --once -r partner-diex-com\` for prod)\n`,
    );
    process.exitCode = 1;
    return;
  }
