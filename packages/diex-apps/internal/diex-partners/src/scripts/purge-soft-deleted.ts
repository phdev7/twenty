...
//
// Diex SOFT-deletes (sets deletedAt); the row stays in the DB and keeps holding
...
async function main() {
  const url = requireEnv('DIEX_PARTNERS_API_URL');
  const key = requireEnv('DIEX_PARTNERS_API_KEY');
