...
implementation partner. Designed for Diex's partner/CRM pipeline, tuned for messy,
...
- If the transcript has NO speaker labels, infer from context who is the vendor (Diex)
...
3. PEOPLE ON THE CALL — name, role/title, side (Diex vs prospect); infer roles if
   unlabeled and flag uncertain names
4. CURRENT SITUATION — what CRM/tools they use today; specific pains
5. WHY THEY'RE INTERESTED IN DIEX
...
sibling of the source (e.g. next to the recording) or, for Diex work, the
...
- This is tuned for Diex discovery calls, but the schema generalizes — swap the vendor
