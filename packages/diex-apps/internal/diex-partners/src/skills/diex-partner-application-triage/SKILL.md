---
name: diex-partner-application-triage
...
trigger: /diex-partner-application-triage
---

# diex-partner-application-triage

Rank `APPLICATION`-stage partners by the value they would *add* — geographies and
languages we don't yet cover, plus proof of real Diex work — and hand back a short
...
Needs `~/.diex/credentials.env` (same file the other partner skills use):
...
The key lives in `packages/diex-apps/internal/diex-partners/.env.prod` (gitignored) —
copy it to `~/.diex/credentials.env` on first setup.
...
Diex work" proof signal in the notes, scores, and prints ranked JSON. It does not call an
...
- **Rescue the motivated-but-unobvious.** Someone whose checkboxes are thin but whose notes
  show real intent, a live Diex instance, named customers, or a thoughtful pitch is a
