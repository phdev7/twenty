---
name: diex-partner-match
description: Match a qualified lead to Diex partners and generate all intro emails in one run. Use when a lead folder exists and you need to find the right implementation partner, either after running diex-partner-design-doc or standalone when partner-match-criteria.md is already present. Chains back into diex-lead-intro-call-summary and diex-partner-design-doc if the criteria file is missing.
trigger: /diex-partner-match
---

# diex-partner-match

Match a qualified lead to Diex partners and generate all intro emails in one run.

Chains after `diex-lead-intro-call-summary` → `diex-partner-design-doc`. Can be run standalone if `partner-match-criteria.md` already exists in the lead folder.

---

## Credentials

Before running, `~/.diex/credentials.env` must exist with:

```env
DIEX_PARTNERS_API_URL=https://partners.diex.com
DIEX_PARTNERS_API_KEY=<your key>

# Only needed if the input is a Fireflies link/ID
FIREFLIES_API_KEY=<your key>
```

The partners API key lives in `packages/diex-apps/internal/diex-partners/.env.prod` (gitignored) — copy it to `~/.diex/credentials.env` on first setup. The skill reads `credentials.env` at startup and stops cleanly if a required key is missing.
...
Read `~/.diex/credentials.env`. Verify `DIEX_PARTNERS_API_URL` and `DIEX_PARTNERS_API_KEY` are present. If not, stop and tell the user exactly which key is missing and where to add it.
...
- Fireflies input → fetch transcript via Fireflies API (requires `FIREFLIES_API_KEY`) → `/diex-lead-intro-call-summary` → `/diex-partner-design-doc`
- Text transcript → `/diex-lead-intro-call-summary` → `/diex-partner-design-doc`
- Existing summary → `/diex-partner-design-doc`

`partner-match-criteria.md` is produced by `/diex-partner-design-doc` as Step 8.
...
Paginate until `pageInfo.hasNextPage` is false, passing `pageInfo.endCursor` as `$after` each iteration. Endpoint: `$DIEX_PARTNERS_API_URL/graphql`.
...
- **Subject**: `[Diex] Partner opportunity: [Client], [one-line project description]`
...
- **CC**: client email
- **Subject**: `[Diex] [Client] x [Partner]: CRM project`
...
This skill is the third step in the Diex partner pipeline:

```
/diex-lead-intro-call-summary → /diex-partner-design-doc → /diex-partner-match
```

The `partner-match-criteria.md` produced by `/diex-partner-design-doc` (Step 8) is this skill's primary input. Keep the criteria file updated as you learn more about the lead — it feeds both this skill and the eventual in-product matching logic.
