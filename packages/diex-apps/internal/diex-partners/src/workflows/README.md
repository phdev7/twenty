...
The Diex Partners app relies on two **manual-trigger workflows** built in the Diex
...
staging, prod). After `yarn diex install`, prod is empty until an admin follows this
runbook.

## Prerequisites

- Diex Partners app installed and synced (`yarn diex dev --once` or `install`).
