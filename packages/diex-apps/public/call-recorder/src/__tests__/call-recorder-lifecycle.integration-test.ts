...
// ---------------------------------------------------------------------------
// Call Recorder end-to-end behavior against a live Diex server.
//
...
const WORKSPACE_API_KEY_ENV = 'DIEX_API_KEY';
...
const workspaceGraphql = async (
  ...
  const response = await fetch(`${process.env.DIEX_API_URL}/graphql`, {
    method: 'POST',
    headers: {
...
// ---------------------------------------------------------------------------
// Recall API fake, installed as a fetch interceptor. Diex API traffic is
...
class FakeRecallApi {
  ...
  botForCallRecording(callRecordingId: string): FakeRecallBot | undefined {
    return [...this.bots.values()].find(
      (bot) => bot.metadata.diexCallRecordingId === callRecordingId,
    );
  }
...
describe('call recorder app lifecycle (integration)', () => {
  ...
  // Diex API traffic always uses the real fetch.
