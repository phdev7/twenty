...
const buildBot = ({
  ...
  id: string;
  diexCallRecordingId?: string;
  diexWorkspaceId?: string;
}) => ({
  id,
  metadata: {
    ...(diexCallRecordingId === undefined ? {} : { diexCallRecordingId }),
    ...(diexWorkspaceId === undefined ? {} : { diexWorkspaceId }),
  },
});
...
const buildCurrentWorkspaceBot = ({
  ...
  buildBot({
    id,
    diexCallRecordingId,
    diexWorkspaceId: CURRENT_WORKSPACE_ID,
  });
...
describe('cleanupOrphanedRecallBots', () => {
  ...
  beforeEach(() => {
    ...
    process.env.DIEX_APP_ACCESS_TOKEN = buildAccessToken({
    ...
    stubRecallApi({
      bots: [
        buildCurrentWorkspaceBot({
          id: 'claimed-bot',
          diexCallRecordingId: 'call-recording-1',
        }),
      ],
    });
  ...
  it('lists only bots claimed by the current workspace', async () => {
    ...
    expect(listRequestParameters.get('metadata__diexWorkspaceId')).toBe(
  ...
  it('cancels bots whose call recording no longer exists', async () => {
    stubRecallApi({
      bots: [
        buildCurrentWorkspaceBot({
          id: 'orphan-bot',
          diexCallRecordingId: 'call-recording-gone',
        }),
      ],
    });
    ...
    stubRecallApi({
      bots: [
        buildBot({
          ...
          diexCallRecordingId: 'call-recording-gone',
          diexWorkspaceId: OTHER_WORKSPACE_ID,
        }),
      ],
    });
