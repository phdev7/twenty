...
describe('retryFailedRecallCancellations', () => {
  ...
  it('recovers and cancels a provider bot when endsAt is missing after the meeting starts', async () => {
    ...
    expect(listRequestParameters.has('metadata__diexCallRecordingId')).toBe(
      false,
    );
    expect(fetchMock).toHaveBeenCalledWith(
              ...
              results: [
                {
                  ...
                  metadata: {
                    diexWorkspaceId: WORKSPACE_ID,
                    diexCallRecordingId: 'call-recording-recent',
                  },
                },
                {
                  id: 'recall-bot-aged',
                  metadata: {
                    diexWorkspaceId: WORKSPACE_ID,
                    diexCallRecordingId: 'call-recording-aged',
                  },
                },
              ],
            }),
