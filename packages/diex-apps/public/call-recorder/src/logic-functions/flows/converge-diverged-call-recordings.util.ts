...
import { DIEX_PAGE_SIZE } from 'src/logic-functions/constants/diex-page-size';
...
const listRecallBotsByIdForConvergence = async (
  ...
  const listResult = await listScheduledRecallBots({
    ...
    metadata: { diexWorkspaceId: currentWorkspaceId },
  });
...
const isCurrentWorkspaceManagedBot = ({
  ...
  const claimedWorkspaceId = bot.metadata.diexWorkspaceId;
