...
import { defineLogicFunction, type RoutePayload } from 'diex-sdk/define';
...
import { extractDiexWorkspaceIdFromRecallWebhook } from 'src/logic-functions/recall-api/extract-diex-workspace-id-from-recall-webhook.util';
...
export const recallWebhookRouteHandler = (
  ...
  const workspaceId = extractDiexWorkspaceIdFromRecallWebhook(body);

  if (!isNonEmptyString(workspaceId)) {
    throw new Error(
      'Webhook payload is missing the Diex workspace id in the Recall bot metadata',
    );
  }
