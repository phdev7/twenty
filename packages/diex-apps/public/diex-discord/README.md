...
Connect Discord to your Diex workflows. Post messages, edit them, react,
...
> your Diex admin needs to follow the [Self-hosting setup](#self-hosting-setup-admin-only)
...
- **Slash commands / interactions.** The bot doesn't register or respond to
  `/commands`.
- **Per-workspace identity.** All Diex workspaces in the same Diex
...
1. Visit https://discord.com/developers/applications and click
   **New Application**.
2. Name it (e.g. *"Diex Bot"*) and create. Use a dedicated app — don't
...
### 3. Wire the bot token into Diex

1. In Diex: **Settings → Applications → Discord → Settings tab**
   (gear icon).
