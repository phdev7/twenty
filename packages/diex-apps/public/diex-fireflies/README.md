...
`CallRecording` records linked to the matching `CalendarEvent` in your Diex
...
1. **Provider-native event ID** — Fireflies' `calendar_id` / `cal_id` is
   ...
   events synced into Diex from Google Calendar (including individual
...
Both identifiers are populated by Diex's calendar drivers on every synced
...
## What gets added to your Diex workspace
...
1. Open **Settings → Applications** in your Diex workspace.
...
> webhook, your Diex admin needs to follow the
...
- **Per-user Fireflies accounts.** All transcripts come through one
  ...
  connections require extending Diex's connection provider system and are
  planned once we have evidence that workspace-shared is too coarse.
- **Editing transcripts or summaries in Diex.** The CallRecording fields
...
| Webhook returns `Invalid webhook signature` | `FIREFLIES_WEBHOOK_SECRET` mismatch between Fireflies and Diex | Re-copy the signing secret from the Fireflies webhook configuration and paste it into the Diex app settings |
| CallRecording is created but has no linked CalendarEvent | The meeting was never on a synced calendar in Diex, or the workspace has no Google/Outlook/CalDAV calendar connection set up | Connect the relevant calendar provider in **Settings → Accounts** so the calendar event lands in Diex with `eventExternalId` and `iCalUid` populated, then re-run **Sync Fireflies Call** for that transcript. Manually-created CalendarEvents are intentionally not matched in v1 |
...
| **List / Search** tools return `count: 0` for a contact you've definitely talked to | Email mismatch — Fireflies stores the address as the participant joined the meeting with, which may differ from the contact's primary address in Diex (aliases, plus-addressing, work vs. personal) | Try the contact's other known email addresses; cross-check the `participants` list on a known matching call |

---

## Self-hosting setup (admin-only)

This section is for Diex server admins. If you're on Diex Cloud, skip
...
2. Set the **Webhook URL** to your Diex deployment's webhook endpoint:
   `https://<your-diex-domain>/webhook/fireflies`. Diex resolves the
...
3. Set a **Signing Secret** (a long random string — generate one with
   `openssl rand -hex 32`). Save it; you'll paste it into Diex next.
...
### 3. Wire the credentials into Diex

1. In Diex: **Settings → Applications → Fireflies → Settings tab**.
