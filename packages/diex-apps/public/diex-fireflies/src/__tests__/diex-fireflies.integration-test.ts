...
// The published diex-client-sdk types only export CoreApiClient/CoreSchema;
...
const getServerCallRecordingStatuses = async (): Promise<string[]> => {
  const generatedCoreSchema = (await import(
    'diex-client-sdk/core'
  )) as unknown as GeneratedCoreSchemaRuntime;
  const statusEnum = generatedCoreSchema.enumCallRecordingStatusEnum;

  if (statusEnum === undefined) {
    throw new Error(
      'enumCallRecordingStatusEnum is missing from diex-client-sdk/core; regenerate the client against a running server (yarn diex dev) before the integration suite',
    );
  }
