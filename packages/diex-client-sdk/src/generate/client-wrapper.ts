...
export const buildClientWrapperSource = (
  ...
  source = source.replace("'__DIEX_DEFAULT_URL__'", options.defaultUrl);

  source = source.replace(/DiexGeneratedClient/g, options.apiClientName);
  ...
  return `\n// ${options.apiClientName} (auto-injected by diex-client-sdk)\n${source}`;
};
