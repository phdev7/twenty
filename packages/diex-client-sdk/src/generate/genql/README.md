...
`listr`, etc.) that Diex never executed.
...
- **Dropped the live-endpoint introspection path** (`schema/fetchSchema.ts`),
  which was the only consumer of `undici` / `native-fetch` / `qs`. Diex always
...
- **Replaced `@graphql-tools/load`** with graphql's own `buildSchema` — Diex
...
- **`Config` was narrowed** to the schema-string inputs Diex actually passes
  ...
  generated client still defaults its url/fetch to `undefined` (Diex's wrapper
