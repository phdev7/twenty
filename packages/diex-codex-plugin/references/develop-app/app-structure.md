...
Use this reference when creating or modifying files inside an existing Diex app.
...
Before changing app entities, confirm the current directory is a Diex app:
...
Use official Diex docs or local SDK source when exact imports, entity fields, or configuration shapes matter.
...
- `src/<service>-client/` — wrappers around external SDKs or HTTP clients. One folder per service, matching Diex's `*-client` convention (`redis-client/`, `sdk-client/`, ...).
- Tests as `*.spec.ts` in sibling `__tests__/` folders next to the code they cover.

Filenames are kebab-case; conventional suffixes are `.util.ts`, `.dto.ts`, `.service.ts`, `.spec.ts` — matching the Diex backend.
...
`yarn diex dev:typecheck` checks generated app types and `yarn lint` checks local lint rules. `yarn diex dev --once` then builds the app and pushes entity definitions to the active remote; if any definition is invalid, the sync reports the error. Run all three a single time once every edit is done, not repeatedly after each step.

When the user explicitly asks to run tests, follow `tests.md`: unit tests may use the package's unit-test script, and the full suite must run with `DIEX_API_URL=http://localhost:2021` against the isolated test instance.
