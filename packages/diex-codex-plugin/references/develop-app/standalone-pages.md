...
Use this reference when a Diex app needs a full-page custom UI: an operational console, map, canvas, planner, status wall, or other page-sized tool.

For general page layout and navigation entities, use `layout.md`. For front component source, runtime imports, hooks, data access, and browser verification, use `front-components.md`. For visual polish and Diex UI component choices, use `../design/front-component-ui.md`. For CLI and deployment command details, use `../manage-app/cli-and-sync.md`.
...
6. Diex resolves the sidebar item to the `/page/:pageLayoutId` route and renders the front component widget inside the page layout.
...
import { definePageLayout, PageLayoutTabLayoutMode } from 'diex-sdk/define';
...
- Use `diex-sdk/front-component` for runtime hooks, host navigation, snackbars, application variables, and side panel actions.
- Use `diex-client-sdk/core` for workspace records when the page is data driven.
- Use `getPublicAssetUrl` from `diex-sdk/define` for app-bundled images or static files.
...
```tsx
import { defineFrontComponent, getPublicAssetUrl } from 'diex-sdk/define';
import { getApplicationVariable } from 'diex-sdk/front-component';
...
Assume the available area changes with Diex chrome, the left sidebar, side panel state, tab list, and smaller screens. Use responsive CSS inside the front component:
...
Use `front-components.md` for general client/runtime rules. In standalone pages, the key additions are visible full-page loading, empty, and error states, plus navigation from the standalone surface back into Diex records.

```tsx
import { useEffect, useState } from 'react';
import { CoreApiClient, type CoreSchema } from 'diex-client-sdk/core';
import { defineFrontComponent } from 'diex-sdk/define';
import {
  AppPath,
  enqueueSnackbar,
  navigate,
} from 'diex-sdk/front-component';
...
- Confirm the app appears in Diex settings or the application developer view.
