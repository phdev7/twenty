...
Use this reference for the publish-facing documentation and visuals of a Diex app. It complements `create-app`; read this when the app is ready for packaging, marketplace listing, npm publishing, private sharing, or user-facing review.

## When To Use

Use this when the user asks to:

- Write or improve a Diex app `README.md`.
- Prepare npm package content or Diex marketplace copy.
...
Diex marketplace metadata comes from `defineApplication()`:
...
One-sentence summary of what the app adds to Diex.
...
- Diex server/version requirement, if any.
...
4. Open the relevant Diex view or app settings page.
...
- Name the Diex objects and views users will actually interact with.
...
- Do not use the Diex logo as the app logo unless the app is officially owned by Diex and the user intends that.

Screenshot guidance:

- Capture real app surfaces after `yarn diex dev --once` or watch-mode sync.
...
If the app requires a specific Diex server version, set `engines.diex` in `package.json`. If publishing to npm, add the `diex-app` keyword.
...
- `package.json` has `keywords: ["diex-app"]` when the app is intended for npm marketplace publishing.
