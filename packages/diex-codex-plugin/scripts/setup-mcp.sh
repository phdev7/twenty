...
Examples:
  setup-mcp.sh acme.diex.com
  setup-mcp.sh --force-login https://crm.example.com
  setup-mcp.sh --name diex-local acme.localhost:3001
  setup-mcp.sh --name diex-prod https://crm.example.com/mcp
...
Environment:
  DIEX_MCP_URL   MCP URL to use when no URL argument is provided.
EOF
}
...
if [[ -z "$url" ]]; then
  echo "A Diex workspace URL is required, for example: https://crm.example.com" >&2
  echo
  usage
  exit 1
fi
