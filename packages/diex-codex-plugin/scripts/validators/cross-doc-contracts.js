...
const assertDiexMcpFormattingContract = (fail) => {
  const skillPath = path.join(PLUGIN_ROOT, 'skills/use-diex-mcp/SKILL.md');
  const resultFormattingPath = path.join(PLUGIN_ROOT, 'references/use-diex-mcp/result-formatting.md');
  ...
  const requiredSkillFragments = [
    ...
    'MUST link each display name back to Diex',
  ...
  for (const fragment of requiredSkillFragments) {
    if (!skill.includes(fragment)) {
      fail(`use-diex-mcp/SKILL.md is missing formatting contract fragment: ${fragment}`);
    }
  }
...
const assertFrontComponentGuidance = (fail) => {
  ...
  const requiredDevelopSkillFragments = [
    ...
    'Diex UI imports',
    'Use `layout.md` for placement, `standalone-pages.md` for full-page custom UI, and `front-component-ui.md` for visual design and Diex UI component selection',
  ];
  ...
  const requiredFrontComponentFragments = [
    ...
    'Use `diex-sdk/front-component`',
    'Use `diex-client-sdk/core` or `diex-client-sdk/metadata`',
    'diex-ui@1.0.0-alpha.1',
    'import Diex UI components, icons, and theme tokens from its subpaths',
    'ThemeProvider',
    'showcase/diex-ui-example.front-component.tsx',
    'themeCssVariables',
    'mocks `diex-ui` during manifest extraction',
...
const assertCliGuidanceSplit = (fail) => {
  ...
  const forbiddenAppStructureFragments = [
    ...
    'Do not run `yarn diex dev:typecheck`',
  ...
  const requiredCliFragments = [
    ...
    'Do not use bare `yarn diex dev` (watch mode)',
    'yarn diex dev --once --verbose',
    'yarn diex remote:list',
    'yarn diex dev:build',
...
module.exports = {
  assertDiexMcpFormattingContract,
