...
const assertJsonMetadata = (fail) => {
  ...
  const packageJson = readJson('packages/diex-codex-plugin/package.json');
  const pluginJson = readJson('packages/diex-codex-plugin/.codex-plugin/plugin.json');
  const mcpJson = readJson('packages/diex-codex-plugin/.mcp.json');
  ...
  const marketplaceEntry = marketplaceJson?.plugins?.find((entry) => entry.name === 'diex');

  if (marketplaceJson && !marketplaceEntry) {
    fail('.agents/plugins/marketplace.json includes plugins but not the diex plugin entry');
  } else if (marketplaceEntry && marketplaceEntry.source?.path !== './packages/diex-codex-plugin') {
    fail('marketplace diex source path must be ./packages/diex-codex-plugin');
  }

  if (fs.existsSync(path.join(REPO_ROOT, 'plugins', 'diex'))) {
    fail('legacy plugins/diex path must not exist; use packages/diex-codex-plugin directly');
  }
};
...
const assertNoBundledMcpConfig = (fail) => {
  ...
  if (fs.existsSync(gitignorePath) && readText(gitignorePath).split(/\r?\n/).includes('.mcp.json')) {
    fail('packages/diex-codex-plugin/.gitignore must not ignore the public .mcp.json');
  }
...
const assertMarketplaceTemplate = (fail) => {
  const { readJson, readOptionalJson } = createJsonReaders(fail);
  const templatePath = 'packages/diex-codex-plugin/templates/marketplace.example.json';
  ...
  const diexEntry = entries.find((entry) => entry?.name === 'diex');

  if (!diexEntry) {
    fail(`${templatePath} must include a plugin entry named "diex"`);
    return;
  }

  if (diexEntry.version !== pluginJson?.version) {
    fail(`${templatePath} diex.version must match plugin.json version`);
  }

  if (diexEntry.source?.path !== './packages/diex-codex-plugin') {
    fail(`${templatePath} diex.source.path must be ./packages/diex-codex-plugin`);
  }

  if (!diexEntry.policy?.installation) {
    fail(`${templatePath} diex.policy.installation is required`);
  }

  if (!diexEntry.policy?.authentication) {
    fail(`${templatePath} diex.policy.authentication is required`);
  }

  if (diexEntry.category !== pluginJson?.interface?.category) {
    fail(`${templatePath} diex.category must match plugin.json interface.category`);
  }
};
