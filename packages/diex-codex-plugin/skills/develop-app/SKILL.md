---
name: develop-app
description: Use when the user wants to add or modify Diex app entities, including objects, layouts, logic functions, and front components inside an existing Diex app.
---

# When To Use

Pick this skill when the user wants to change what an existing Diex app *does* — its data model, UI, logic, or workflows. Representative triggers:

- "add a new object to my Diex app"
...
Do not use this skill to scaffold a brand-new app (use `create-app`), to sync/deploy/troubleshoot (use `manage-app`), to prepare marketplace assets (use `publish-app`), or to query workspace records (use `use-diex-mcp`).
...
First, confirm the setup is sane before changing entities. The current directory should be the root of an existing Diex app:
...
- `src/<service>-client/<name>.ts` — wrappers around external SDKs or shared HTTP clients. One folder per service, matching Diex's `*-client` convention.
...
`yarn diex dev:typecheck` checks generated app types, `yarn lint` checks local lint rules, and `yarn diex apply` syncs entity definitions to the active remote. Run all three a single time once every edit is done, not repeatedly after each step. When the user explicitly asks to run tests, follow `../../references/develop-app/tests.md`.

Use the official Diex docs or local SDK source when exact entity fields, imports, or configuration shapes matter.
...
- Front component source, Diex UI imports, data hooks, runtime imports, and browser verification: `../../references/develop-app/front-components.md`
...
For front components, read `front-components.md` before implementation. Use `layout.md` for placement, `standalone-pages.md` for full-page custom UI, and `front-component-ui.md` for visual design and Diex UI component selection.
