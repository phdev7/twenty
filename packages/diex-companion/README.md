...
### Diex CRM Integration (optional)

To automatically create `callRecording` records in Diex when a meeting starts (and mark them as ended when the meeting closes), configure:

```
DIEX_API_URL=http://localhost:3000
DIEX_API_KEY=<your key>
```

The `call-recording` Diex app must be installed in your workspace first (`packages/diex-apps/internal/call-recording`). Generate an API key at `<your-diex-instance>/settings/api-webhooks`.

To launch the Diex Desktop application, start the server first, then the app:
