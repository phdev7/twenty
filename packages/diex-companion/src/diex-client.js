...
const LOG_PREFIX = '[Diex]';

let configLogged = false;

function getApiUrl() {
  return process.env.DIEX_API_URL;
}

function getWorkspaceSubdomain() {
  return process.env.DIEX_WORKSPACE_SUBDOMAIN;
}

function getApiKey() {
  return process.env.DIEX_API_KEY;
}
...
function isConfigured() {
  ...
  if (!configLogged) {
    ...
    if (!configured) {
      console.log(
        `${LOG_PREFIX} Integration disabled — DIEX_API_URL or DIEX_API_KEY not set`,
      );
    } else {
      console.log(
