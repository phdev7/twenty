...
const diexClient = require('./diex-client');
require('dotenv').config();

const diexIconDataUrl = require('./assets/diex-logo-256.png');

function getAppIcon() {
  try {
    return nativeImage.createFromDataURL(diexIconDataUrl);
  } catch (error) {
...
// Check Diex CRM integration status at startup
diexClient.isConfigured();
...
app.whenReady().then(() => {
  if (process.platform === 'darwin' && app.dock) {
    ...
    if (dockIcon && !dockIcon.isEmpty()) {
      ...
      console.log('Diex dock icon set successfully');
    } else {
...
// Poll Recall for the audio URL then call the Diex end-recording logic function.
...
async function endCallRecordingWithRetry(windowId, maxAttempts = 10, delayMs = 5000) {
  try {
    ...
    if (!meeting?.diexRecordId || !meeting?.recallRecordingId) {
      console.log('[Diex] No Diex record or Recall recording ID found, skipping end-recording');
      return;
    }
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        ...
        if (audioUrl) {
          await diexClient.endCallRecording({
            callRecordingId: meeting.diexRecordId,
            audioUrl,
            transcriptUrl,
        ...
        console.error(`[Diex] Attempt ${attempt}/${maxAttempts} — error fetching recording:`, error.message);
      }

      if (attempt < maxAttempts) {
        console.log(`[Diex] Audio URL not ready, retrying in ${delayMs / 1000}s (${attempt}/${maxAttempts})`);
        await new Promise((resolve) => setTimeout(resolve, delayMs));
      }
    }

    console.error('[Diex] Audio URL never became available, giving up');
  } catch (error) {
    console.error('Failed to end callRecording in Diex:', error.message);
  }
}
...
async function createMeetingNoteAndRecord(platformName) {
  ...
  try {
    ...
    // Create a call recording record in Diex CRM
    if (diexClient.isConfigured()) {
      try {
        const diexRecord = await diexClient.createCallRecording(meetingTitle);

        if (diexRecord?.id) {
          newMeeting.diexRecordId = diexRecord.id;

          if (global.activeMeetingIds && global.activeMeetingIds[detectedMeeting.window.id]) {
            global.activeMeetingIds[detectedMeeting.window.id].diexRecordId = diexRecord.id;
          }

          console.log('Created callRecording in Diex:', diexRecord.id);
        }
      } catch (error) {
        console.error('Failed to create callRecording in Diex:', error.message);
      }
    }
