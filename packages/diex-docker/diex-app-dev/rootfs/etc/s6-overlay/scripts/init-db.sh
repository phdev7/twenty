...
su-exec postgres psql -h localhost -tc \
  "SELECT 1 FROM pg_roles WHERE rolname='diex'" | grep -q 1 \
  || su-exec postgres psql -h localhost -c "CREATE ROLE diex WITH LOGIN PASSWORD 'diex' SUPERUSER"

# Create database if it doesn't exist
su-exec postgres psql -h localhost -tc \
  "SELECT 1 FROM pg_database WHERE datname='default'" | grep -q 1 \
  || su-exec postgres createdb -h localhost -O diex default

# Run Diex database setup and migrations
cd /app/packages/diex-server

has_schema=$(PGPASSWORD=diex psql -h localhost -U diex -d default -tAc \
...
has_workspace=$(PGPASSWORD=diex psql -h localhost -U diex -d default -tAc \
