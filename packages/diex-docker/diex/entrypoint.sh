...
setup_and_migrate_db() {
    ...
    has_migrations_table=$(psql "${PG_DATABASE_URL}" -tAc "SELECT to_regclass('core._typeorm_migrations') IS NOT NULL")
    if [ "$has_migrations_table" = "f" ]; then
        echo "Database is empty or partially initialized, running migrations."
...
case "${DIEX_PROCESS_TYPE:-server}" in
    server)
        ;;
    worker)
        ...
        ;;
    *)
        echo "Invalid DIEX_PROCESS_TYPE: ${DIEX_PROCESS_TYPE}. Expected server or worker." >&2
        exit 1
        ;;
esac
