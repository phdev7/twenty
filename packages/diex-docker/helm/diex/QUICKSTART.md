...
- Create the PostgreSQL database "diex" and schema "core" automatically
...
```bash
kubectl get secret tokens -n diexcrm -o jsonpath='{.data.accessToken}' | base64 --decode && echo
```
...
```bash
helm uninstall my-diex -n diexcrm
kubectl delete namespace diexcrm
```
