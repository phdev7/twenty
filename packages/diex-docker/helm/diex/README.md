...
Deploy Diex CRM on Kubernetes with server, worker, PostgreSQL, and Redis components.
...
helm install my-diex ./packages/diex-docker/helm/diex \
  --namespace diexcrm --create-namespace
```
...
- Database `diex` and schema `core` are created automatically by server init container
...
```bash
helm lint ./packages/diex-docker/helm/diex
helm template my-diex ./packages/diex-docker/helm/diex
helm plugin install https://github.com/quintush/helm-unittest
helm unittest ./packages/diex-docker/helm/diex
```
...
helm install my-diex ./packages/diex-docker/helm/diex -f values-secrets.yaml
```
