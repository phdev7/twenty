# Kubernetes for Diex CRM (Helm)
DISCLAIMER: The k8s and podman deployments are not maintained by the core team.
These files are provided and maintained by the community. Diex core team
maintains support for docker deployment.

## Overview

This repository contains a world-class Helm chart and (legacy) raw Kubernetes manifests plus Terraform files to help you deploy and manage the Diex CRM application. The chart is located at `packages/diex-docker/helm/diex` and supersedes the manifests in `packages/diex-docker/k8s/manifests`.
...
helm install my-diex packages/diex-docker/helm/diex \
  --namespace diexcrm --create-namespace --wait \
...
See [QUICKSTART](../helm/diex/QUICKSTART.md) and [chart README](../helm/diex/README.md) for details.
...
cd diex/packages/diex-docker/k8s
```
...
2. Create Server Secret
    ...
    ``` bash
    kubectl create secret generic -n diexcrm tokens --from-literal accessToken=changeme --from-literal loginToken="changeme" --from-literal refreshToken="changeme" --from-literal fileToken="changeme"
    ```
