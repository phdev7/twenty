...
2. Start diex by running `podman-compose up -d`.

If you need to stop diex, you can do so by running `podman-compose down`.


### Install systemd service (optional)

If you want to install a systemd service to run diex, you can use the provided systemd service. 

Edit `diexcrm.service` and change these two variables:


	WorkingDirectory=/opt/apps/diex
	EnvironmentFile=/opt/apps/diex/.env
...
Note: this script will enable the service and also start it. So it will assume that diex is not currently running.
