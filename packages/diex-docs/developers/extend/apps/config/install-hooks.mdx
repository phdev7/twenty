...
- The usual logic-function environment (`APPLICATION_ID`, `APP_ACCESS_TOKEN`, `API_URL`) is injected, so you can call the Diex API with your app's token.
...
- **Not executed in dev mode**: `yarn diex dev` skips the install flow and syncs files directly, so hooks never run there. Trigger them manually instead:
