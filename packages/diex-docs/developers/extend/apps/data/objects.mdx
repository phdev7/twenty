...
- You can scaffold new objects with `yarn diex dev:add object`, which guides you through naming, fields, and relationships. See [Architecture → Scaffolding entities](/developers/extend/apps/getting-started/scaffolding).

<Note>
**Base fields are added automatically.** When you define a custom object, Diex creates standard fields like `id`, `name`, `createdAt`, `updatedAt`, `createdBy`, `updatedBy`, and `deletedAt` for you. You don't need to declare them in your `fields` array — only your custom fields. You can override a default field by declaring one with the same name, but this is rarely a good idea.
</Note>

## Field types

The full set of `FieldType` values, exported from `diex-sdk/define`:
