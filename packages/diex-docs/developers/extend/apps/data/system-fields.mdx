...
Every object in Diex ships with a set of **system fields** that you never declare yourself. They are created automatically by the server when the object is provisioned:
...
Since Diex 2.19, a system field's universal identifier is **derived deterministically** by the server from three inputs: the application universal identifier, the object universal identifier, and the field name. Inventing an id and hardcoding it won't work: it matches nothing on the server, and the sync rejects the dangling reference:
...
<Note>
  `getFieldUniversalIdentifier` is available from `diex-sdk` 2.21 onward.
</Note>
...
<Note>
  `getSystemRelationFieldUniversalIdentifier` is available from `diex-sdk`
  2.23 onward and requires a Diex server on 2.23 or later.
</Note>
...
## Standard Diex objects

For a **standard** Diex object (Person, Company, Opportunity, …), you don't need to derive anything: the system field identifiers are pre-computed constants you can import directly.
