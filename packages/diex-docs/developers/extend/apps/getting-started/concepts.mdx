---
title: Concepts
description: How Diex apps work — entity model, sandboxing, and the install lifecycle.
icon: "sitemap"
---

Diex apps are TypeScript packages that extend your workspace with custom objects, logic, UI components, and AI capabilities. They run on the Diex platform with full sandboxing and permission controls.

## How apps work

An app is a collection of **entities** declared using `defineEntity()` functions from the `diex-sdk` package. The SDK detects these declarations via AST analysis at build time and produces a **manifest** — a complete description of what your app adds to a workspace. These functions validate your configuration at build time and provide IDE autocompletion and type safety.
...
| **Front Component** | Sandboxed React UI inside Diex | [Front Components](/developers/extend/apps/layout/front-components) |
...
- **Front components** run in Web Workers using Remote DOM — sandboxed from the main page but rendering native DOM elements (not iframes). They communicate with Diex via a message-passing host API.
- **Permissions** are enforced at the API level. The runtime token (`DIEX_APP_ACCESS_TOKEN`) is derived from the role defined in `defineApplication()`.
...
- **`yarn diex dev`** — watches your source files and live-syncs changes to a connected Diex server. The typed API client is regenerated automatically when the schema changes.
- **`yarn diex dev:build`** — compiles TypeScript, bundles logic functions and front components with esbuild, and produces a manifest.
