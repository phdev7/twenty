...
Diex apps are authored in **English**: the strings in your source and manifest
...
Import the translation helpers from `diex-sdk/front-component`:
...
`diex dev:build` compiles the catalogs and serves the right language for the
...
re-running `diex dev:build` (and redeploying), the same as any other change.

<Note>
Translations are compiled by `diex dev:build` (and `diex apply`). The
continuous `diex dev` watch shows source strings, so test localized output
