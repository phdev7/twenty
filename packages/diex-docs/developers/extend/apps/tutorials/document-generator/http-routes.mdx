...
and self-hosted instances, that path is served by your Diex server
(e.g. `http://localhost:2020/s/documents/generate`). On Diex Cloud, `RestApiClient`
...
[See the helper](https://github.com/diexhq/diex/blob/main/packages/diex-apps/examples/document-generator/src/utils/render-document.ts).
...
`yarn diex dev:function:logs`, or invoke it directly with
