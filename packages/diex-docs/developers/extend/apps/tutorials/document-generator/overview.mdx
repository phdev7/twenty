...
description: Build a real Diex app that generates personalized documents from your CRM data.
---
...
a local Diex server running on port `2020` and the CLI authenticated to it.
...
Leave `yarn diex dev` running in a terminal for the whole tutorial. Every time
