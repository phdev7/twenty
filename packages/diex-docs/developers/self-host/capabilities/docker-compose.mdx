...
This guide provides step-by-step instructions to install and configure the Diex application using Docker Compose. The aim is to make the process straightforward and prevent common pitfalls that could break your setup.
...
Install the latest stable version of Diex with a single command:
```bash
...
If you host diexCRM on your own computer, open your browser and navigate to [http://localhost:3000](http://localhost:3000).
...
### Expose Diex to External Access

By default, Diex runs on `localhost` at port `3000`. To access it via an external domain or IP address, you need to configure the `SERVER_URL` in your `.env` file.
...
SSL (HTTPS) is required for certain browser features to work properly. While these features might work during local development (as browsers treat localhost differently), a proper SSL setup is needed when hosting Diex on a regular domain.
...
We strongly recommend setting up Diex behind a reverse proxy with SSL termination for optimal security and functionality.
...
<Tip>
  Prefer not to run and maintain Diex yourself? [Find a certified Diex hosting partner](https://diex.com/partners/list?categories=HOSTING&ref=docs-docker-compose) to deploy, host, and manage your instance end-to-end. *(Or reach our team: [contact@diex.com](mailto:contact@diex.com).)*
</Tip>
