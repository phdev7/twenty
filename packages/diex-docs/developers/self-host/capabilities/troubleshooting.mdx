...
If you have an existing Diex instance with production data, **DO NOT** follow these steps as they will permanently delete your database!

While installing Diex for the first time, you might want to change the default database password.
...
During Diex installation, you need to provision your postgres database with the right schemas, extensions, and users.
...
#### Cannot find module 'diex-emails' or its corresponding type declarations.

You have to build the package `diex-emails` before running the initialization of the database with `npx nx run diex-emails:build`

#### Missing diex-x package

Make sure to run yarn in the root directory and then run `npx nx server:dev diex-server`. If this still doesn't work try building the missing package manually.
...
#### While running `npx nx start` or `npx nx start diex-front`, Out of memory error is thrown

Run only the services you need, instead of `npx nx start`. For instance, if you work on the server, run only `npx nx worker diex-server`

**If it does not work:**
If you tried to run only `npx nx run diex-server:start` on WSL and it's failing with the below memory error:
...
**If it does not work:**
Investigate which processes are taking you most of your machine RAM. At Diex, we noticed that some VScode extensions were taking a lot of RAM so we temporarily disable them.
...
- Open your Diex app, navigate to `/settings`, and enable the **Advanced** toggle at the bottom left of the screen.
- Create a new webhook.
- Paste **Your Unique Webhook URL** in the **Endpoint Url** field in Diex. Set the **Filters** to `Companies` and `Created`.
<div style={{textAlign: 'center'}}>
...
Comment out checker plugin in `packages/diex-ui/vite-config.ts` like in example below
```
plugins: [
...
If you're running Diex behind a reverse proxy and experiencing connection issues:
...
2. **Check Reverse Proxy Settings:**
    ...
    - Confirm that your reverse proxy is correctly forwarding requests to the Diex server.
    - Ensure headers like `X-Forwarded-For` and `X-Forwarded-Proto` are properly set.

3. **Restart Services:**

    After making changes, restart both the reverse proxy and Diex containers.
...
- Community Support:
    ...
    Reach out to the [Diex community](https://github.com/diexhq/diex/issues) or [support channels](https://discord.gg/cx5n4Jzs57) for assistance.
