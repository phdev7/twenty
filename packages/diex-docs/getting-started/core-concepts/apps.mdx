...
title: Apps
icon: "cube"
description: Extend Diex with code — custom objects, server-side logic, UI components, and AI agents, all as TypeScript packages.
---


Most CRMs give you a config panel. Diex gives you a platform. Apps are how developers extend Diex beyond what the UI offers — defining data models, server-side logic, UI components, and AI capabilities as code, then deploying them to one or more workspaces.
...
An app is a TypeScript package that declares **entities** using the `diex-sdk`:
...
| **Front Components** | Sandboxed React components that render inside Diex's UI (side panel, widgets, command menu) |
...
You write your app as a TypeScript project on your machine. The CLI watches your source files and live-syncs them to a running Diex server — edit a file, see the change in the UI within a second. The typed API client regenerates automatically when the schema changes. When you're ready, `yarn diex app:publish --private` pushes to a production server, or `yarn diex app:publish` lists your app on npm and the Diex marketplace.
