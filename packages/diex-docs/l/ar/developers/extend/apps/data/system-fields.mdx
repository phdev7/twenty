...
كل كائن في Diex يأتي مع مجموعة من **الحقول النظامية** التي لا تقوم بتعريفها بنفسك أبدًا. يتم إنشاؤها تلقائيًا بواسطة الخادم عندما يتم توفير الكائن:
...
منذ Diex 2.19، يتم اشتقاق المعرّف الشامل للحقل النظامي **بشكل حتمي** بواسطة الخادم من ثلاثة مُدخلات: المعرّف الشامل للتطبيق، والمعرّف الشامل للكائن، واسم الحقل. ابتكار معرّف (id) وتضمينه بشكل ثابت لن ينجح: لن يطابق أي شيء على الخادم، وسترفض المزامنة المرجع المعلّق:
...
<Note>
  يتوفر `getFieldUniversalIdentifier` في `diex-sdk` بدءًا من الإصدار 2.21 فصاعدًا.
</Note>
...
<Note>
  `getSystemRelationFieldUniversalIdentifier` متوفر في `diex-sdk`
  بدءًا من الإصدار 2.23، ويتطلب خادم Diex بالإصدار 2.23 أو أحدث.
</Note>
...
## الكائنات القياسية في Diex

بالنسبة إلى كائن Diex **قياسي** (Person، Company، Opportunity، …)، لا تحتاج إلى اشتقاق أي شيء: فمعرّفات الحقول النظامية هي ثوابت مُسبقة الحساب يمكنك استيرادها مباشرة.
