...
تُكتب تطبيقات Diex باللغة **الإنجليزية**: السلاسل في شفرتك وملف البيان هي نص المصدر بالإنجليزية، و`en` هي الإعداد المحلي الذي تترجم منه، وأي إعداد محلي يُترك دون ترجمة يعود إليه افتراضيًا.
...
قم باستيراد أدوات الترجمة من `diex-sdk/front-component`:
...
يقوم `diex dev:build` بتجميع الكتالوجات وتقديم اللغة الصحيحة للمستخدم
...
إعادة تشغيل `diex dev:build` (وإعادة النشر)، تمامًا مثل أي تغيير آخر.

<Note>
يتم تجميع الترجمات بواسطة `diex dev:build` (و`diex apply`). تشغيل المراقبة المستمرة `diex dev`
