...
يقوم Diex بإرسال طلب HTTP POST إلى URL الخاص بك كلما تم إنشاء سجل أو تحديثه أو حذفه. جميع أنواع الكائنات مشمولة، بما في ذلك الكائنات المخصصة.
...
يرسل Diex خطافات الويب لأنواع الأحداث التالية:
...
يقوم Diex بتوقيع كل طلب خطاف ويب لأغراض الأمان. تحقّق من التواقيع للتأكد من أن الطلبات أصيلة.

### الرؤوس

| الترويسة                     | الوصف               |
| ---------------------------- | ------------------- |
| `X-Diex-Webhook-Signature` | توقيع HMAC SHA256   |
| `X-Diex-Webhook-Timestamp` | الطابع الزمني للطلب |

### خطوات التحقق

1. احصل على الطابع الزمني من `X-Diex-Webhook-Timestamp`
2. أنشئ السلسلة: `{timestamp}:{JSON payload}`
3. احسب HMAC SHA256 باستخدام سر خطاف الويب الخاص بك
4. قارِن مع `X-Diex-Webhook-Signature`
...
| **مشغّل خطاف ويب لسير العمل** | IN      | استقبال البيانات في Diex من الأنظمة الخارجية              |
