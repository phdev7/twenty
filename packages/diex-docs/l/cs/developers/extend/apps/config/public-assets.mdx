...
* **Zahrnuté do buildů** — `yarn diex dev:build` zabalí všechny veřejné prostředky do distribučního výstupu.

## Přístup k veřejným prostředkům pomocí `getPublicAssetUrl`

K získání plné URL souboru ve vaší složce `public/` použijte pomocnou funkci `getPublicAssetUrl` z `diex-sdk`. Funguje jak v logických funkcích, tak ve frontendových komponentách.
