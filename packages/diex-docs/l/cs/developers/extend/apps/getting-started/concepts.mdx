---
title: Pojmy
description: Jak aplikace Diex fungují — model entit, sandboxing a životní cyklus instalace.
icon: sitemap
---

Aplikace Diex jsou balíčky TypeScriptu, které rozšiřují váš pracovní prostor o vlastní objekty, logiku, komponenty UI a funkce AI. Běží na platformě Diex s plnou izolací (sandboxingem) a řízením oprávnění.

## Jak aplikace fungují

Aplikace je kolekce **entit** deklarovaných pomocí funkcí `defineEntity()` z balíčku `diex-sdk`. SDK tyto deklarace detekuje pomocí analýzy AST při sestavení a vytváří **manifest** — úplný popis toho, co vaše aplikace přidává do pracovního prostoru. Tyto funkce validují vaši konfiguraci v době sestavení a poskytují automatické doplňování v IDE a typovou bezpečnost.
...
| **Frontendová komponenta**    | Izolované React UI uvnitř Diex                                | [Frontendové komponenty](/l/cs/developers/extend/apps/layout/front-components)     |
...
* **Frontendové komponenty** běží ve Web Workerech s využitím Remote DOM — jsou oddělené od hlavní stránky, ale vykreslují nativní prvky DOM (nikoli iframy). Komunikují s Diex prostřednictvím hostitelského API pro předávání zpráv.
* **Oprávnění** jsou vynucována na úrovni API. Běhový token (`DIEX_APP_ACCESS_TOKEN`) je odvozen z role definované v `defineApplication()`.
...
* **`yarn diex dev`** — sleduje vaše zdrojové soubory a průběžně synchronizuje změny s připojeným serverem Diex. Typovaný klient API se při změně schématu automaticky znovu vygeneruje.
* **`yarn diex dev:build`** — zkompiluje TypeScript, zabalí logické funkce a frontendové komponenty pomocí esbuild a vytvoří manifest.
