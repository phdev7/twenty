...
* Použijte `serverVariables` (nikoli `applicationVariables`) — pověření OAuth jsou celoserverová a na jeden server Diex je jedna aplikace OAuth.
...
4. Otevřete nainstalovanou aplikaci v Diex jako správce serveru → nastavte hodnoty na odpovídajících `serverVariables`.
