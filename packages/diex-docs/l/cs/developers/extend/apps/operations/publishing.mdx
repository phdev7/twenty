...
title: Publikování
icon: upload
description: Distribuujte svou aplikaci Diex do Marketplace nebo ji nasaďte interně.
---
...
* **Nasaďte tarball** — nahrajte svou aplikaci přímo na konkrétní server Diex pro interní nebo soukromé použití.
* **Publish to npm** — uveďte svou aplikaci v Marketplace Diex, aby ji mohl kterýkoli pracovní prostor objevit a nainstalovat.
...
Tím se zkompilují zdrojové soubory TypeScriptu, transpilují logické funkce a frontendové komponenty a vše se zapíše do `.diex/output/`. Přidejte `--tarball`, abyste také vytvořili balíček `.tgz` pro ruční distribuci nebo příkaz publish.

## Nasazení na server (tarball)

U aplikací, které nechcete zpřístupnit veřejně — proprietární nástroje, integrace pouze pro enterprise nebo experimentální buildy — můžete nasadit tarball přímo na server Diex.

### Předpoklady

Před nasazením potřebujete nakonfigurovaný vzdálený cíl směřující na cílový server. Vzdálené cíle ukládají adresu URL serveru a přihlašovací údaje lokálně v `~/.diex/config.json`.
...
2. Spusťte `yarn diex app:publish --private` (nebo `yarn diex app:publish --private --remote production`)
...
Pokud vaše aplikace používá funkci zavedenou v konkrétní verzi serveru Diex (například poskytovatelé OAuth přidaní ve verzi 2.3.0), měli byste deklarovat minimální verzi serveru, kterou vaše aplikace vyžaduje, pomocí pole `engines.diex` v `package.json`:
...
* Pokud je `engines.diex` nastaveno a verze cílového serveru nevyhovuje rozsahu, nasazení (nahrání tarballu) nebo instalace je odmítnuto chybou `SERVER_VERSION_INCOMPATIBLE` a zprávou, která uvádí jak požadovaný rozsah, tak skutečnou verzi serveru.
* Pokud `engines.diex` **není nastaveno**, aplikace je přijata na jakékoli verzi serveru (zpětně kompatibilní se stávajícími aplikacemi).
* Pokud server nemá nakonfigurované `APP_VERSION`, kontrola se přeskočí.

<Note>
Server je rozhodující autoritou — ověřuje `engines.diex` jak při nahrání tarballu, tak při instalaci do pracovního prostoru. Pokud nasazujete tarball mimo standardní proces nebo instalujete z marketplace, server přesto vynucuje kompatibilitu.
</Note>

## Automatizované CI/CD (předpřipravené workflowy)

Aplikace vygenerované pomocí `create-diex-app` jsou hned připravené se třemi workflowy GitHub Actions ve složce `.github/workflows/`. CI běží bez jakéhokoli nastavování, CD vyžaduje jediný secret a publikování na npm vyžaduje jednorázové nastavení npm trusted-publisher.
...
2. Spustí izolovanou testovací instanci Diex pomocí složené akce `diexhq/diex/.github/actions/spawn-diex-app-dev-test@main` (ekvivalent v CI k `yarn diex docker:start --test`).
3. Povolí Corepack, nastaví Node.js podle vašeho `.nvmrc` a nainstaluje závislosti pomocí `yarn install --immutable`.
4. Spustí `yarn test` a předá `DIEX_API_URL` a `DIEX_API_KEY` ze spuštěné instance, aby vaše testy mohly komunikovat se skutečným serverem.

**Konfigurační volby:**

* `DIEX_VERSION` (env, výchozí hodnota `latest`) — uzamkněte v CI používanou verzi serveru Diex úpravou této hodnoty v `ci.yml`.
...
Nasazuje vaši aplikaci na nakonfigurovaný server Diex při každém pushi do `main` a volitelně také z pull requestu, pokud je přidán štítek `deploy`.

**K čemu slouží:**

1. Provede checkout headu PR (u označených PR) nebo pushnutého commitu.
2. Spustí `diexhq/diex/.github/actions/deploy-diex-app@main` — ekvivalent v CI k `yarn diex app:publish --private`.
3. Spustí `diexhq/diex/.github/actions/install-diex-app@main`, aby se nově nasazená verze nainstalovala do cílového pracovního prostoru.

**Požadovaná konfigurace:**

| Nastavení               | Kde                                                             | Účel                                                                                                 |
| ----------------------- | --------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------- |
| `DIEX_DEPLOY_URL`     | `env` v `cd.yml` (výchozí `http://localhost:3000`)              | Server Diex, na který se nasazuje. Před prvním použitím to změňte na skutečnou URL vašeho serveru. |
| `DIEX_DEPLOY_API_KEY` | GitHub repozitář **Settings → Secrets and variables → Actions** | API klíč s oprávněním k nasazení na cílovém serveru.                                                 |

<Note>
Výchozí `DIEX_DEPLOY_URL` `http://localhost:3000` je pouze zástupná hodnota — z runneru hostovaného GitHubem tato adresa nebude dosažitelná. Před povolením CD ji aktualizujte na veřejnou URL vašeho serveru (nebo použijte self-hosted runner s přístupem do sítě).
</Note>
...
Workflowy `ci.yml` a `cd.yml` odkazují na znovupoužitelné akce na `@main`, takže aktualizace akcí v repozitáři `diexhq/diex` se přeberou automaticky. Pokud chcete deterministická sestavení, nahraďte `@main` v každém řádku `uses:` za commit SHA nebo tag vydání.

## Publikování na npm

Publikování na npm zajistí, že bude vaše aplikace dohledatelná v Marketplace Diex. Jakýkoli pracovní prostor Diex může procházet, instalovat a aktualizovat aplikace z Marketplace přímo z UI.

### Požadavky

* Účet na [npm](https://www.npmjs.com)
* Klíčové slovo `diex-app` ve vašem poli `keywords` v souboru `package.json` (přidejte ho ručně — ve výchozím nastavení není zahrnuto v šabloně `create-diex-app`)
...
Server Diex synchronizuje svůj katalog tržiště z registru npm **každou hodinu**.
...
<Note>
Pokud vaše aplikace nedefinuje `aboutDescription` v `defineApplication()`, tržiště automaticky použije soubor `README.md` vašeho balíčku z npm jako obsah stránky O aplikaci. To znamená, že můžete spravovat jediný soubor README jak pro npm, tak pro tržiště Diex. Pokud chcete v tržišti jiný popis, explicitně nastavte `aboutDescription`.
</Note>

### Publikování pomocí CI

Vygenerované workflow `publish.yml` popsané výše publikuje na npm automaticky při verzovacích tagách, s provenance. Protože `yarn diex app:publish` při běhu v CI přidá `--provenance` a `--access public` za vás, workflow nepotřebuje žádné přepínače npm — pouze jednorázové nastavení důvěryhodného vydavatele.

Pro jiné CI systémy (GitLab CI, CircleCI atd.) spusťte `yarn install` a poté `yarn diex app:publish`. Provenance se vydává, pokud prostředí umí vytvořit token OIDC, a v opačném případě se automaticky vynechá.

<Note>
**npm provenance** přidává k vašemu záznamu na npm odznak důvěryhodnosti a umožňuje uživatelům ověřit, že balíček byl sestaven z konkrétního commitu ve veřejné CI pipeline. Také vám umožňuje uplatnit vlastnictví vaší aplikace na tržišti Diex. Podrobnosti najdete v [dokumentaci k npm provenance](https://docs.npmjs.com/generating-provenance-statements).
</Note>
...
Přejděte na stránku **Nastavení > Aplikace** v Diex, kde lze procházet a instalovat jak aplikace z tržiště, tak aplikace nasazené jako tarball.
...
K instalaci novější verze ji nejprve nasaďte nebo publikujte, poté znovu spusťte `yarn diex app:install`.
</Note>
