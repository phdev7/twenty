...
Aplikace Diex jsou psané v **angličtině**: řetězce ve vašem zdrojovém kódu a manifestu tvoří anglický zdrojový text, `en` je zdrojová locale, ze které překládáte, a jakákoli locale ponechaná bez překladu se na ni automaticky vrátí.
...
Importujte pomocné překladové funkce z `diex-sdk/front-component`:
...
`diex dev:build` zkompiluje katalogy a doručí správný jazyk pro
...
znovu spustit `diex dev:build` (a znovu nasadit) stejně jako jakoukoli jinou změnu.

<Note>
Překlady jsou kompilovány příkazy `diex dev:build` (a `diex apply`). Nepřetržité sledování `diex dev` zobrazuje zdrojové řetězce, takže lokalizovaný výstup
