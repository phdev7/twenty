...
a self-hostovaných instancích tuto cestu obsluhuje váš server Diex
(např. `http://localhost:2020/s/documents/generate`). Ve Diex Cloudu `RestApiClient`
...
[Viz pomocník] (https://github.com/diexhq/diex/blob/main/packages/diex-apps/examples/document-generator/src/utils/render-document.ts).
...
<Tip>
Během testování s
`yarn diex dev:function:logs`, nebo vyvolat přímo s
`yarn diex dev:exec`.
</Tip>
