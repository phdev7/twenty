...
Pokud máte existující instanci Diex s produkčními daty, **NEPROVÁDĚJTE** tyto kroky, protože trvale smažou vaši databázi!

Při první instalaci Diex můžete chtít změnit výchozí heslo pro databázi.
...
Během instalace Diex musíte nastavit svou postgres databázi s správnými schématy, rozšířeními a uživateli.
...
#### Nelze najít modul 'diex-emails' ani jeho odpovídající deklarace typů.

Před spuštěním inicializace databáze musíte sestavit balíček `diex-emails` s `npx nx run diex-emails:build`

#### Chybí balíček diex-x

Ujistěte se, že v kořenovém adresáři spouštíte `yarn` a poté spusťte `npx nx server:dev diex-server`. Pokud stále nefunguje, zkuste chybějící balíček sestavit ručně.
...
#### Při spuštění `npx nx start` nebo `npx nx start diex-front` došlo k chybě nedostatku paměti

Spusťte pouze služby, které potřebujete, místo `npx nx start`. Například pokud pracujete na serveru, spusťte pouze `npx nx worker diex-server`

**Pokud to nefunguje:**
Pokud jste se pokusili spustit pouze `npx nx run diex-server:start` na WSL a selhává to s následující chybou paměti:
...
**Pokud to nefunguje:**
Analyzujte, které procesy vám zabírají většinu paměti na stroji. V Diex jsme si všimli, že některá rozšíření VScode zabírala hodně paměti RAM, takže jsme je dočasně deaktivovali.
...
* Otevřete svou Diex aplikaci, přejděte na `/settings` a na levé dolní části obrazovky aktivujte přepínač **Pokročilé**.
* Vytvořte nový webhook.
* Vložte **Svou unikátní webhook URL** do pole **Endpoint Url** ve Diex. Nastavte **Filtry** na `Companies` a `Created`.
...
Zakomentujte plugin checker v `packages/diex-ui/vite-config.ts` jako v příkladu níže
...
Pokud provozujete Diex za reverzní proxy a máte problémy s připojením:
...
2. **Zkontrolujte nastavení reverzní proxy:**
   ...
   * Potvrďte, že vaše reverzní proxy správně směruje požadavky na server Diex.
   * Ujistěte se, že záhlaví jako `X-Forwarded-For` a `X-Forwarded-Proto` jsou správně nastaveny.

3. **Restartujte služby:**

   Po provedení změn restartujte jak reverzní proxy, tak kontejnery Diex.
...
* Podpora komunity:
  ...
  Obraťte se na [komunitu Diex](https://github.com/diexhq/diex/issues) nebo [podpůrné kanály](https://discord.gg/cx5n4Jzs57) pro pomoc.
