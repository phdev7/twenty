...
Nadřazený („jedna“ strana) musí v Diex existovat, než na něj budete moci odkazovat.
...
1. Exportujte nadřazené záznamy z Diex
...
Použijte `https://domain.com` (ne pouze `domain.com`). To odpovídá způsobu, jak Diex ukládá domény společností, a předchází chybám při párování.
</Warning>
...
Diex automaticky propojí každý podřízený záznam s odpovídajícím nadřazeným.
...
| **Neexistující záznamy**     | Nadřazený záznam v Diex neexistuje            | Nejprve importujte nadřazené záznamy nebo zkontrolujte překlepy |
| **Rozdílná velikost písmen** | `Acme.com` v souboru, ale v Diex `acme.com`   | Zajistěte přesnou shodu velikosti písmen                        |
...
<AccordionGroup>
  <Accordion title="Co když můj nadřazený záznam nemá doménu ani e-mail?">
    Máte dvě možnosti:
    1. Použijte Diex `id` (exportujte nadřazené záznamy, abyste získali jejich ID)
