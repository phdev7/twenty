...
* **In Builds enthalten** — `yarn diex dev:build` bündelt alle öffentlichen Assets in der Distributionsausgabe.

## Zugriff auf öffentliche Assets mit `getPublicAssetUrl`

Verwenden Sie den Helper `getPublicAssetUrl` aus `diex-sdk`, um die vollständige URL einer Datei in Ihrem `public/`-Verzeichnis zu erhalten. Dies funktioniert sowohl in Logikfunktionen als auch in Frontend-Komponenten.
