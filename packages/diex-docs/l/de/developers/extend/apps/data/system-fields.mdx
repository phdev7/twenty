...
Jedes Objekt in Diex wird mit einem Satz von **Systemfeldern** ausgeliefert, die Sie nie selbst deklarieren. Sie werden automatisch vom Server erstellt, wenn das Objekt bereitgestellt wird:
...
<Note>
  `getFieldUniversalIdentifier` ist ab `diex-sdk` 2.21 verfügbar.
</Note>
...
<Note>
  `getSystemRelationFieldUniversalIdentifier` ist ab Version 2.23 von `diex-sdk` verfügbar und erfordert einen Diex-Server in Version 2.23 oder höher.
</Note>
...
## Standardobjekte von Diex

Für ein **Standard**-Diex-Objekt (Person, Company, Opportunity, …) müssen Sie nichts ableiten: Die Systemfeld-Kennungen sind vorab berechnete Konstanten, die Sie direkt importieren können.
