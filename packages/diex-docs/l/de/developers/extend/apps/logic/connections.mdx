...
* Verwenden Sie `serverVariables` (nicht `applicationVariables`) — OAuth-Anmeldedaten gelten serverweit und es gibt eine OAuth-App pro Diex-Server.
...
4. Öffnen Sie die installierte App in Diex als Serveradministrator → setzen Sie die Werte in den entsprechenden `serverVariables`.
