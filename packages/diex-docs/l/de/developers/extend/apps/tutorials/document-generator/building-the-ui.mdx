...
Komponente mit Sandkasten in Diex. Wir liest den ausgewählten Datensatz ein, lädt die
...
`diex-ui` importiert. Das SDK verspottet das Paket während des Builds, so dass Modul-Level-Importe von
Theme-Konstanten `undefiniert` wären. Siehe
[vollständige Komponente](https://github.com/diexhq/diex/blob/main/packages/diex-apps/examples/document-generator/src/front-components/generate-document-form.front-component.tsx).
</Warning>
...
Styles über einen kleinen [`Markdown`](https://github.com/diexhq/diex/blob/main/packages/diex-apps/examples/document-generator/src/utils/markdown-to-react.tsx)
Helfer machen.
...
`RICH_TEXT`-Feld ist, stellt Diex bereits einen vollständigen Rich-Text-Editor dafür bereit – denselben, den auch die Standardobjekte Note und Task verwenden. Wir Oberflächen es nur auf der
...
[`template-record. age-layout.ts`](https://github.com/diexhq/diex/blob/main/packages/diex-apps/examples/document-generator/src/page-layouts/template-record.page-layout.ts).
