...
und selbstgehosteten Instanzen wird dieser Pfad von Ihrem Diex-Server
bereitgestellt (z. B. `http://localhost:2020/s/documents/generate`). In Diex Cloud löst `RestApiClient` den Pfad über die Funktionen-Domain des Workspaces auf.
...
[Siehe den Helfer](https://github.com/diexhq/diex/blob/main/packages/diex-apps/examples/document-generator/src/utils/render-document.ts).
