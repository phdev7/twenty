...
Wenn Sie bereits eine bestehende Diex-Instanz mit Produktionsdaten haben, **NICHT** diesen Schritten folgen, da diese permanent Ihre Datenbank löschen!

Bei der ersten Installation von Diex möchten Sie möglicherweise das Standard-Datenbankpasswort ändern.
...
Während der Diex-Installation müssen Sie Ihre Postgres-Datenbank mit den richtigen Schemata, Erweiterungen und Benutzern bereitstellen.
...
#### Modul 'diex-emails' oder seine entsprechenden Typdeklarationen nicht gefunden.

Sie müssen das Paket `diex-emails` erstellen, bevor Sie die Initialisierung der Datenbank mit `npx nx run diex-emails:build` ausführen.

#### Fehlendes Diex-x Paket

Stellen Sie sicher, dass Sie `yarn` im Root-Verzeichnis ausführen und dann `npx nx server:dev diex-server` ausführen. Wenn das immer noch nicht funktioniert, versuchen Sie, das fehlende Paket manuell zu erstellen.
...
#### Beim Ausführen von `npx nx start` oder `npx nx start diex-front` wird ein Speicherfehler angezeigt

Führen Sie nur die Services aus, die Sie benötigen, anstatt `npx nx start`. Wenn Sie zum Beispiel am Server arbeiten, führen Sie nur `npx nx worker diex-server` aus.

**Wenn es nicht funktioniert:**
Wenn Sie versucht haben, nur `npx nx run diex-server:start` auf WSL zu starten und es mit dem untenstehenden Speicherfehler fehlschlägt:
...
**Wenn es nicht funktioniert:**
Untersuchen Sie, welche Prozesse den größten Teil Ihres RAMs beanspruchen. Bei Diex haben wir festgestellt, dass einige vscode-Erweiterungen viel RAM verwenden, daher deaktivieren wir sie vorübergehend.
...
* Öffnen Sie Ihre Diex-App, navigieren Sie zu `/settings`, und aktivieren Sie den **Erweitert**-Schalter unten links auf dem Bildschirm.
* Erstellen Sie einen neuen Webhook.
* Fügen Sie **Ihre einzigartige Webhook-URL** in das Feld **Endpoint-Url** in Diex ein. Stellen Sie die **Filter** auf `Companies` und `Created` ein.
...
Kommentieren Sie das Checker-Plugin in `packages/diex-ui/vite-config.ts` wie im folgenden Beispiel aus
...
Wenn Sie Diex hinter einem Reverse-Proxy ausführen und Verbindungsprobleme haben:
...
2. **Reverse-Proxy-Einstellungen prüfen:**
   ...
   * Stellen Sie sicher, dass Ihr Reverse-Proxy Anfragen korrekt an den Diex-Server weiterleitet.
   * Stellen Sie sicher, dass Header wie `X-Forwarded-For` und `X-Forwarded-Proto` korrekt gesetzt sind.

3. **Dienste neu starten:**

   Starten Sie nach Änderungen sowohl den Reverse-Proxy als auch die Diex-Container neu.
...
* Unterstützung aus der Community:
  ...
  Wenden Sie sich an die [Diex Community](https://github.com/diexhq/diex/issues) oder [Support-Kanäle](https://discord.gg/cx5n4Jzs57) für Unterstützung.
