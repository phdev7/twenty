...
Esta guía proporciona instrucciones paso a paso para instalar y configurar la aplicación Diex utilizando Docker Compose. El objetivo es simplificar el proceso y prevenir errores comunes que podrían arruinar tu configuración.
...
Instala la última versión estable de Diex con un solo comando:
...
Si alojas diexCRM en tu propia computadora, abre tu navegador y navega a [http://localhost:3000](http://localhost:3000).
...
### Exponer Diex para acceso externo

Por defecto, Diex se ejecuta en `localhost` en el puerto `3000`. Para acceder a él mediante un dominio externo o dirección IP, necesitas configurar `SERVER_URL` en tu archivo `.env`.
...
Recomendamos encarecidamente configurar Diex detrás de un proxy inverso con terminación SSL para una seguridad y funcionalidad óptimas.
...
<Tip>
  ¿Prefieres no ejecutar y mantener Diex por tu cuenta? [Encuentra un partner de alojamiento certificado de Diex](https://diex.com/partners/list?categories=HOSTING\&ref=docs-docker-compose) para implementar, alojar y gestionar tu instancia de principio a fin. *(O contacta con nuestro equipo: [contact@diex.com](mailto:contact@diex.com).)*
</Tip>
