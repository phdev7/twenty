---
title: Servidor MCP
description: Conecta asistentes de IA a tu espacio de trabajo de Diex mediante el Model Context Protocol.
---

Diex expone un servidor [MCP](https://modelcontextprotocol.io/) para que los asistentes de IA — Claude Desktop, Claude Code, Cursor, ChatGPT y otros — puedan leer y escribir tus datos de CRM mediante lenguaje natural.

Usa tu **URL del espacio de trabajo** (la URL que utilizas para acceder a Diex) como el punto de acceso MCP. En Diex Cloud, la URL de tu espacio de trabajo puede ser `https://{mycompany}.diex.com` o un dominio personalizado. El servidor está disponible en:

| Entorno             | Punto de acceso MCP                                                             |
| ------------------- | ------------------------------------------------------------------------------- |
| **Nube**            | `https://{your-workspace-url}/mcp` (p. ej., `https://mycompany.diex.com/mcp`) |
...
Agrega esto a la configuración de tu cliente MCP, reemplazando `{your-workspace-url}` por el host de tu espacio de trabajo (p. ej., `mycompany.diex.com`):
...
1. Descubrir los metadatos de OAuth de Diex a través de `/.well-known/oauth-protected-resource` y `/.well-known/oauth-authorization-server`
...
Ve a **Settings → MCP & APIs → MCP** en Diex. Elige tu método de autenticación (OAuth o API Key), copia el fragmento JSON (ya usará la URL de tu espacio de trabajo) y pégalo en el archivo de configuración de tu cliente MCP.
...
Reinicia tu cliente MCP (o recarga la configuración). Si usas OAuth, se te redirigirá a Diex para autorizar el acceso. Si usas una clave de API, la conexión es inmediata.
...
Una vez conectado, el servidor MCP expone herramientas que reflejan la API de Diex. El flujo de trabajo recomendado es:
...
Para instancias autohospedadas, reemplaza `{your-workspace-url}` por la URL de tu servidor. Asegúrate de que `SERVER_URL` en tu entorno coincida con la URL pública de tu instancia de Diex — se utiliza para generar los metadatos de descubrimiento de OAuth.
