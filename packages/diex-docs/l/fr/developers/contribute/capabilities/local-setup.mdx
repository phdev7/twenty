...
icon: laptop-code
description: Le guide pour les contributeurs (ou les développeurs curieux) qui souhaitent exécuter Diex localement.
---
...
Avant de pouvoir installer et utiliser Diex, assurez-vous d'installer ce qui suit sur votre ordinateur :
...
```bash
git clone git@github.com:diexhq/diex.git
```
</Tab>
<Tab title="HTTPS">
...
<Tabs>
  <Tab title="Linux">
    ...
    **Option 2 :** Si vous avez installé Docker :
    ```bash
      make -C packages/diex-docker postgres-on-docker
    ```
  </Tab>
  <Tab title="Mac OS">
...
Diex nécessite un cache Redis pour offrir les meilleures performances.
...
<Tabs>
  <Tab title="Linux">
    ...
    **Option 2 :** Si vous avez installé Docker :
    ```bash
      make -C packages/diex-docker redis-on-docker
    ```
  </Tab>
  <Tab title="Mac OS">
...
```bash
cp ./packages/diex-front/.env.example ./packages/diex-front/.env
cp ./packages/diex-server/.env.example ./packages/diex-server/.env
```

<Info>
**Mode multi-espaces de travail:** Par défaut, Diex s'exécute en mode mono-espace de travail, où un seul espace de travail peut être créé. Pour activer la prise en charge multi-espaces de travail (utile pour tester des fonctionnalités basées sur des sous-domaines), définissez `IS_MULTIWORKSPACE_ENABLED=true` dans le fichier `.env` de votre serveur. Voir [Mode multi-espaces de travail](/l/fr/developers/self-host/capabilities/setup#multi-workspace-mode) pour plus de détails.
</Info>

## Étape 6 : Installation des dépendances

Pour construire le serveur Diex et injecter des données dans votre base de données, exécutez la commande suivante :
...
```bash
npx nx database:reset diex-server
```
...
## Étape 8 : Utiliser Diex

**Frontend**

Le frontend de Diex sera en cours d'exécution à [http://localhost:3001](http://localhost:3001).
Vous pouvez vous connecter en utilisant le compte démo par défaut : `tim@apple.dev` (mot de passe : `tim@apple.dev`)

**Backend**

* Le serveur de Diex sera en fonctionnement à [http://localhost:3000](http://localhost:3000)
