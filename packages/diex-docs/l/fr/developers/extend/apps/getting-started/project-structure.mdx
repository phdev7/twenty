...
description: Ce qui se trouve à l’intérieur d’une application Diex générée par un modèle — fichiers, dossiers et le rôle de chacun.
icon: folder-tree
---

Une nouvelle application générée par `npx create-diex-app` ressemble à ceci :
...
Les deux packages du SDK Diex doivent être placés dans `devDependencies`, et non dans `dependencies` :
...
Le générateur de projet fige `diex-sdk` et `diex-client-sdk` sur sa propre version — gardez les deux synchronisés lors de la mise à niveau.

* **`diex-sdk`** fournit le CLI `diex` ainsi que les outils de build et de scaffolding. Il ne s’exécute qu’au moment du développement et du build et n’est jamais importé par le runtime de l’application que vous publiez.
* **`diex-client-sdk`** *est* importé par le code de votre application (`CoreApiClient`, `MetadataApiClient`, `RestApiClient`), mais Diex le fournit au moment de l’exécution : les fonctions de logique l’obtiennent à partir d’une couche SDK générée, et les composants front le résolvent à partir de modules servis par le serveur. La copie que vous avez installée est uniquement utilisée pour la vérification de type et le build au moment du déploiement, elle n’a donc jamais besoin d’être incluse dans le bundle déployé.

Conserver l’un ou l’autre package dans `dependencies` l’intègre dans le bundle runtime de l’application installée, où il ne fait que l’alourdir inutilement. `diex dev:build` émet un avertissement lorsque l’un ou l’autre est encore répertorié dans `dependencies`.
