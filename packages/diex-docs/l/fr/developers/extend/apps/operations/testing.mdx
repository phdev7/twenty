...
Les deux environnements disposent de `diex-client-sdk/core` et `diex-client-sdk/metadata` en tant que modules pré-fournis — ils ne sont pas intégrés au bundle mais résolus à l'exécution par le serveur.
...
Créez un fichier de configuration global qui vérifie que le serveur est joignable, écrit une configuration de test pour le SDK (`~/.diex/config.test.json`) et synchronise l’application avant l’exécution des tests :
...
Le sous-chemin `diex-sdk/cli` exporte des fonctions que vous pouvez appeler directement depuis le code de test :
...
| `appDevOnce`   | Construire et synchroniser l’application une fois (identique à `yarn diex apply`) |
...
Assurez-vous que votre serveur Diex local est en cours d'exécution, puis :
...
Le générateur crée un workflow prêt à l’emploi dans `.github/workflows/ci.yml`. À chaque push sur `main` et à chaque pull request, il lance un serveur Diex éphémère dans le runner (via l’action `diexhq/diex/.github/actions/spawn-diex-app-dev-test`), puis exécute `yarn lint`, `yarn typecheck`, `yarn test:unit` et `yarn test` avec `DIEX_API_URL` / `DIEX_API_KEY` pointant vers ce serveur. Aucun secret n’est requis, et vous pouvez fixer la version du serveur via la variable d’environnement `DIEX_VERSION` en haut du workflow.
