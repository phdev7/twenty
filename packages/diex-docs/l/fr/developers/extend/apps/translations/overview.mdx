...
Les applications Diex sont rédigées en **anglais** : les chaînes de vos sources et de votre manifeste
...
Importez les helpers de traduction depuis `diex-sdk/front-component` :
...
`diex dev:build` compile les catalogues et sert la bonne langue pour
...
de relancer `diex dev:build` (et de redéployer), comme pour toute autre modification.

<Note>
Les traductions sont compilées par `diex dev:build` (et `diex apply`). La
surveillance continue `diex dev` affiche les chaînes source, donc testez la sortie localisée
