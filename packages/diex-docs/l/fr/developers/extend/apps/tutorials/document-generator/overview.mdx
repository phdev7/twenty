...
description: Créez une véritable application Diex qui génère des documents personnalisés à partir des données de votre CRM.
---
...
un serveur Diex local en cours d’exécution sur le port `2020` et le CLI authentifié dessus.
...
Laissez `yarn diex dev` s’exécuter dans un terminal pendant tout le tutoriel. Chaque fois que
