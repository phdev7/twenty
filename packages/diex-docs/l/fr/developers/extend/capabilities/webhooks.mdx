...
Les webhooks envoient des données à vos systèmes en temps réel lorsque des événements se produisent dans Diex — aucune interrogation n'est nécessaire. Utilisez-les pour maintenir les systèmes externes synchronisés, déclencher des automatisations ou envoyer des alertes.
...
Diex envoie des webhooks pour ces types d'événements :
...
Diex signe chaque requête de webhook pour des raisons de sécurité. Validez les signatures pour garantir l'authenticité des requêtes.

### En-têtes

| En-tête                      | Description              |
| ---------------------------- | ------------------------ |
| `X-Diex-Webhook-Signature` | Signature HMAC SHA256    |
| `X-Diex-Webhook-Timestamp` | Horodatage de la requête |

### Étapes de validation

1. Récupérez l'horodatage depuis `X-Diex-Webhook-Timestamp`
2. Créez la chaîne : `{timestamp}:{JSON payload}`
3. Calculez le HMAC SHA256 en utilisant votre secret de webhook
4. Comparez avec `X-Diex-Webhook-Signature`
...
const timestamp = req.headers["x-diex-webhook-timestamp"];
...
const isValid = expectedSignature === req.headers["x-diex-webhook-signature"];
```
...
| **Déclencheur de webhook de workflow** | ENTRÉE    | Recevoir des données dans Diex depuis des systèmes externes                         |
