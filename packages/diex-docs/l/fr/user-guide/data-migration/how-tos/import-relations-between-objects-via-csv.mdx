...
Le parent (côté "un") doit exister dans Diex avant que vous ne puissiez y faire référence.
...
1. Exportez les enregistrements parents depuis Diex
...
Utilisez `https://domain.com` (et non simplement `domain.com`). Cela correspond à la manière dont Diex stocke les domaines des Entreprises et évite les erreurs d'association.
</Warning>
...
Diex liera automatiquement chaque enregistrement enfant au parent correspondant.
...
| **Enregistrements inexistants** | L'enregistrement parent n'existe pas dans Diex            | Importez d'abord les enregistrements parents, ou vérifiez les fautes de frappe |
| **Incohérence de casse**        | `Acme.com` dans le fichier mais `acme.com` dans Diex      | Assurez une correspondance exacte de la casse                                  |
...
<AccordionGroup>
  <Accordion title="Que faire si mon enregistrement parent n'a pas de domaine ni d'e-mail ?">
    ...
    1. Utilisez l'`id` Diex (exportez les enregistrements parents pour obtenir leurs identifiants)
