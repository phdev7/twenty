...
| **Intégrations personnalisées** | Votre application → automatisation dans Diex                 |
...
3. Vous recevrez une URL de webhook unique, par exemple :
   ```
   https://api.diex.com/webhooks/workflow/abc123...
   ```
...
```bash
curl -X POST https://api.diex.com/webhooks/workflow/abc123... \
