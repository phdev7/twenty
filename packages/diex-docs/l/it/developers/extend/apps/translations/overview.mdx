...
Le app di Diex sono scritte in **inglese**: le stringhe nel tuo sorgente e nel manifest
...
Importa gli helper di traduzione da `diex-sdk/front-component`:
...
`diex dev:build` compila i cataloghi e serve la lingua corretta per l’utente
...
eseguire nuovamente `diex dev:build` (e ridistribuire), come per qualsiasi altra modifica.

<Note>
Le traduzioni vengono compilate da `diex dev:build` (e `diex apply`). La modalità watch continua di `diex dev` mostra le stringhe sorgente, quindi testa l’output localizzato
