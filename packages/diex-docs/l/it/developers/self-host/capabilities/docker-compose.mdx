...
Questa guida fornisce istruzioni passo passo per installare e configurare l'applicazione Diex usando Docker Compose. L'obiettivo è rendere il processo semplice ed evitare gli errori comuni che potrebbero compromettere il tuo setup.
...
Installa l'ultima versione stabile di Diex con un unico comando:
...
Se ospiti diexCRM sul tuo computer, apri il browser e naviga a [http://localhost:3000](http://localhost:3000).
...
### Esponi Diex per Accesso Esterno

Per impostazione predefinita, Diex gira su `localhost` alla porta `3000`. Per accedervi tramite un dominio esterno o indirizzo IP devi configurare il `SERVER_URL` nel tuo file `.env`.
...
Raccomandiamo fortemente di configurare Diex dietro un proxy inverso con terminazione SSL per una sicurezza e funzionalità ottimali.
...
<Tip>
  Preferisci non gestire e mantenere Diex in autonomia? [Trova un partner Diex certificato](https://diex.com/partners/list?categories=HOSTING\&ref=docs-docker-compose) per distribuire, ospitare e gestire la tua istanza end-to-end. *(Oppure contatta il nostro team: [contact@diex.com](mailto:contact@diex.com).)*
</Tip>
