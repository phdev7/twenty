...
Se hai un'istanza di Diex esistente con dati di produzione, **NON** seguire questi passaggi poiché elimineranno permanentemente il tuo database!

Durante l'installazione di Diex per la prima volta, potresti voler cambiare la password del database predefinita.
...
Durante l'installazione di Diex, devi fornire il tuo database postgres con gli schemi, le estensioni e gli utenti corretti.
...
#### Impossibile trovare il modulo 'diex-emails' o le relative dichiarazioni di tipo corrispondente.

Devi costruire il pacchetto `diex-emails` prima di avviare l'inizializzazione del database con `npx nx run diex-emails:build`

#### Pacchetto diex-x mancante

Assicurati di eseguire yarn nella directory principale e poi esegui `npx nx server:dev diex-server`. Se ancora non funziona prova a costruire manualmente il pacchetto mancante.
...
#### Durante l'esecuzione di `npx nx start` o `npx nx start diex-front`, viene generato un errore di memoria insufficiente

Esegui solo i servizi di cui hai bisogno, invece di `npx nx start`. Ad esempio, se lavori sul server, esegui solo `npx nx worker diex-server`

**Se non funziona:**
Se hai provato ad eseguire solo `npx nx run diex-server:start` su WSL e sta fallendo con l'errore di memoria sottostante:
...
**Se non funziona:**
indaga quali processi ti stanno portando la maggior parte della tua RAM. In Diex, abbiamo notato che alcune estensioni di VScode stavano occupando molta RAM quindi le disabilitiamo temporaneamente.
...
* Apri la tua app Diex, naviga su `/settings` e abilita il toggle **Avanzate** in basso a sinistra dello schermo.
* Crea un nuovo webhook.
* Incolla **Your Unique Webhook URL** nel campo **Endpoint Url** in Diex. Imposta i **Filtri** su `Companies` e `Created`.
...
Commenta il plugin checker in `packages/diex-ui/vite-config.ts` come mostrato nell'esempio sotto
...
Se stai eseguendo Diex dietro un reverse proxy e stai riscontrando problemi di connessione:
...
2. **Controlla le impostazioni del reverse proxy:**
   ...
   * Conferma che il tuo reverse proxy sta inoltrando correttamente le richieste al server Diex.
   * Assicurati che le intestazioni come `X-Forwarded-For` e `X-Forwarded-Proto` siano impostate correttamente.

3. **Riavvia i servizi:**

   Dopo aver apportato modifiche, riavvia sia il reverse proxy che i container Diex.
...
* Supporto comunitario:
  ...
  Contatta la [comunità Diex](https://github.com/diexhq/diex/issues) o [canali di supporto](https://discord.gg/cx5n4Jzs57) per assistenza.
