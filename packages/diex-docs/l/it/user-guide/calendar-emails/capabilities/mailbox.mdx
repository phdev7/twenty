...
description: Comprendere le funzionalità di integrazione delle email in Diex.
---
...
Diex collega automaticamente le email dalle tue caselle di posta connesse ai relativi record CRM, mantenendo la cronologia di comunicazione in un unico posto.
...
Diex importa email a circa **400 messaggi al minuto**. Questa velocità è attualmente limitata dalle limitazioni di frequenza dell'API di Gmail (e vale in modo analogo per altri provider).
...
Scegli se sincronizzare le email interne in Diex:
...
* **Email mancanti**: Verifica se:
  ...
  * La creazione automatica dei contatti è disabilitata (le email necessitano di record Diex esistenti)
