...
どちらも `httpRouteTriggerSettings` を使用します。 アプリのルートは `/s/` パスで指定されます。ローカル環境やセルフホスト環境では、そのパスは Diex サーバーによって提供されます（例: `http://localhost:2020/s/documents/generate`）。 Diex Cloud では、`RestApiClient` はワークスペースの functions ドメインを通じてパスを解決します。
...
[ヘルパーを参照](https://github.com/diexhq/diex/blob/main/packages/diex-apps/examples/document-generator/src/utils/render-document.ts)。
