...
description: 実際の Diex アプリを構築し、CRM データからパーソナライズされたドキュメントを生成しましょう。
---
...
ローカルの Diex サーバーがポート `2020` で動作しており、CLI がそのサーバーに対して認証済みになっている状態です。
...
[`packages/diex-apps/examples/document-generator`](https://github.com/diexhq/diex/tree/main/packages/diex-apps/examples/document-generator) にあります。
...
チュートリアル全体を通して、ターミナルで `yarn diex dev` を実行したままにしておいてください。 `src/` 配下のファイルを追加または編集するたびに、数秒以内にサーバーへ再同期されるため、ビルドしながら各機能が UI に現れていく様子を確認できます。
