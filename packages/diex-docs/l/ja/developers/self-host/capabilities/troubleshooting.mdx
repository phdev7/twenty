...
すでに稼働中のDiexインスタンスに本番データがある場合、これらの手順に従うとデータベースが完全に削除されますので、絶対に従わないでください。

Diexを初めてインストールする際には、デフォルトのデータベースパスワードを変更する必要があるかもしれません。
...
Diexのインストール中に、postgresデータベースを適切なスキーマ、拡張、およびユーザーで準備する必要があります。
この準備がうまくいけば、あなたのデータベースに`default`と`metadata`スキーマがあるはずです。
そうでない場合は、同じコンピュータ上で複数のpostgresインスタンスを実行していないことを確認してください。
この準備がうまくいけば、あなたのデータベースに`default`と`metadata`スキーマがあるはずです。
そうでない場合は、同じコンピュータ上で複数のpostgresインスタンスを実行していないことを確認してください。

#### モジュール'diex-emails' またはその対応する型宣言が見つかりません。

データベースの初期化を実行する前に`npx nx run diex-emails:build`でdiex-emailsパッケージをビルドする必要があります。

#### diex-xパッケージが見つかりません

ルートディレクトリでyarnを実行し、その後 `npx nx server:dev diex-server`を実行してください。 それでも動作しない場合は、足りないパッケージを手動でビルドしてみてください。 それでも動作しない場合は、足りないパッケージを手動でビルドしてみてください。 それでも動作しない場合は、足りないパッケージを手動でビルドしてみてください。
...
#### `npx nx start` または `npx nx start diex-front` 実行中にメモリエラーが発生

`npx nx start` の代わりに、必要なサービスだけを実行してください。 例えば、サーバーで作業している場合は、`npx nx worker diex-server` のみを実行してください。

**うまくいかない場合:**
WSL 上で `npx nx run diex-server:start` だけを実行しようとして、以下のメモリエラーで失敗する場合:
...
あなたのマシンのRAMを最も使用しているプロセスを調査してください。 Diexでは、一部のVScode拡張機能が多くのRAMを使用していることに気付き、一時的に無効化しました。
...
* Diexアプリを開き、`/settings`に移動し、画面左下の**Advanced**トグルを有効にします。
* 新しいWebhookを作成します。
* **Your Unique Webhook URL**をDiexの**Endpoint Url**フィールドに貼り付けます。 **Filters**を`Companies`と`Created`に設定します。
...
例のように`packages/diex-ui/vite-config.ts`内のチェッカープラグインをコメントアウトします
...
リバースプロキシの背後でDiexを実行し、接続問題が発生している場合:
...
2. **リバースプロキシ設定の確認:**
   ...
   * リバースプロキシがDiexサーバーへのリクエストを正しく転送していることを確認してください。
   * `X-Forwarded-For`や`X-Forwarded-Proto`といったヘッダーが正しく設定されていることを確認してください。

3. **サービスの再起動:**

   変更を行った後に、リバースプロキシとDiexコンテナの両方を再起動します。
...
* コミュニティサポート:
  ...
  [Diexコミュニティ](https://github.com/diexhq/diex/issues)または[サポートチャンネル](https://discord.gg/cx5n4Jzs57)に連絡してサポートを受けてください。
