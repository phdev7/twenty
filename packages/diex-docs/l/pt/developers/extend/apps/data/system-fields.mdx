...
Todo objeto no Diex é fornecido com um conjunto de **campos de sistema** que você nunca declara manualmente. Eles são criados automaticamente pelo servidor quando o objeto é provisionado:
...
<Note>
  `getFieldUniversalIdentifier` está disponível a partir do `diex-sdk` 2.21.
</Note>
...
<Note>
  `getSystemRelationFieldUniversalIdentifier` está disponível a partir da versão 2.23 do `diex-sdk` e requer um servidor Diex na versão 2.23 ou posterior.
</Note>
...
## Objetos Padrão do Diex

Para um objeto **padrão** do Diex (Person, Company, Opportunity, …), você não precisa derivar nada: os identificadores de campos de sistema são constantes pré-computadas que você pode importar diretamente.
