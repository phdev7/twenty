...
<Note>
Para a iteração local do dia a dia, quase sempre você vai querer `yarn diex dev`. Fazer deploy e publicar servem para entregar releases, **não** para o ciclo local.
</Note>

| Você quer…                                        | Comando                             | Notas                                                                                                                          |
| ------------------------------------------------- | ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ |
| Iterar localmente com sincronização em tempo real | `yarn diex dev`                   | Monitora seus arquivos e sincroniza a cada alteração.                                                                          |
| Sincronizar uma vez e sair (CI, scripts, hooks)   | `yarn diex apply`                 | Um build + sincronização, depois encerra. Adicione `--force` para pular a confirmação de mudança destrutiva.                   |
| Prever mudanças **sem aplicá-las**                | `yarn diex plan`                  | Calcula e imprime o diff; não grava nada.                                                                                      |
| Remover o app do workspace                        | `yarn diex app:uninstall`         | Adicione `--yes` para pular o prompt.                                                                                          |
| Enviar um tarball para um servidor                | `yarn diex app:publish --private` | Requer uma versão **estritamente maior** em `package.json` — veja [Publicação](/l/pt/developers/extend/apps/operations/publishing). |
| Publicar no marketplace (npm)                     | `yarn diex app:publish`           | —                                                                                                                              |
| Instalar / atualizar uma versão implantada        | `yarn diex app:install`           | Instala a versão atualmente implantada.                                                                                        |
| Limpar o servidor local e começar do zero         | `yarn diex docker:reset`          | Exclui **todos** os dados locais — último recurso.                                                                             |

<Note>
`yarn diex dev --once` e `yarn diex dev --once --dry-run` ainda funcionam como aliases obsoletos para `yarn diex apply` e `yarn diex plan`.
</Note>
...
`yarn diex plan` compila seu manifesto, pede ao servidor o plano de migração e o imprime — **sem aplicar nada**. É a forma segura de responder "o que esta sincronização mudaria?" antes de se comprometer com ela.
...
<Note>
Um plano só antevê mudanças de **metadados**, e exige que o app tenha sido sincronizado ao menos uma vez (para que o workspace o conheça). Se você rodar isso em um app que nunca foi sincronizado, o servidor informa que o app não está instalado — rode `yarn diex dev` uma vez antes.
</Note>
...
1. **Ressincronizar.** Rode `yarn diex apply` novamente. Sincronizações são idempotentes — rodar novamente um manifesto limpo é seguro e frequentemente resolve um problema transitório.
2. **Prever o plano.** Rode `yarn diex plan` para ver exatamente o que a próxima sincronização pretende mudar, sem aplicá-la.
...
4. **Desinstalar e reinstalar.** `yarn diex app:uninstall`, depois sincronize novamente (`yarn diex dev`). Isso reconstrói os metadados do app a partir do zero, mantendo o restante do seu workspace intacto.
5. **Reset completo (último recurso).** `yarn diex docker:reset`, depois faça o seeding e a sincronização novamente.

<Warning>
`yarn diex docker:reset` exclui **todos** os dados da sua instância local — todos os workspaces, registros e apps. Use isso somente depois que as etapas anteriores tiverem falhado.
</Warning>

<Note>
Encontrou um erro de metadados? Por favor, [abra uma issue](https://github.com/diexhq/diex/issues/new/choose) e inclua a mensagem de migração que falhou (com seu tipo de metadado e `universalIdentifier`), a saída de `Metadata changes` da sincronização e os comandos que você rodou.
</Note>
