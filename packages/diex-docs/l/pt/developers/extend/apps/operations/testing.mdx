---
title: Testes
description: Configuração do Vitest, testes de integração contra um servidor Diex real, verificação de tipos e CI com GitHub Actions.
icon: flask
---

O SDK fornece APIs programáticas que permitem compilar, implantar, instalar e desinstalar seu aplicativo a partir de código de teste. Em conjunto com [Vitest](https://vitest.dev/) e os clientes de API tipados, você pode escrever testes de integração que verificam que seu aplicativo funciona de ponta a ponta em um servidor Diex real.
...
Ambos os ambientes têm `diex-client-sdk/core` e `diex-client-sdk/metadata` disponíveis como módulos pré-fornecidos — eles não são empacotados, mas resolvidos em tempo de execução pelo servidor.
...
Crie um arquivo de configuração global que verifique se o servidor está acessível, escreva uma configuração de teste para o SDK (`~/.diex/config.test.json`) e sincronize o app antes da execução dos testes:
...
O subcaminho `diex-sdk/cli` exporta funções que você pode chamar diretamente a partir do código de teste:

| Função         | Descrição                                                        |
| -------------- | ---------------------------------------------------------------- |
| `appBuild`     | Compilar o aplicativo e, opcionalmente, empacotar um tarball     |
| `appDeploy`    | Enviar um tarball para o servidor                                |
| `appDevOnce`   | Compila e sincroniza o app uma vez (igual a `yarn diex apply`) |
...
Certifique-se de que seu servidor Diex local esteja em execução e, em seguida:
...
O gerador de scaffold cria um workflow pronto para uso em `.github/workflows/ci.yml`. A cada push para `main` e a cada pull request, ele inicia um servidor Diex efêmero no runner (por meio da action `diexhq/diex/.github/actions/spawn-diex-app-dev-test`) e então executa `yarn lint`, `yarn typecheck`, `yarn test:unit` e `yarn test` com `DIEX_API_URL` / `DIEX_API_KEY` apontando para esse servidor. Nenhum secret é necessário e você pode fixar a versão do servidor por meio da variável de ambiente `DIEX_VERSION` no topo do workflow.
