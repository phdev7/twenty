...
e auto-hospedadas, esse caminho é servido pelo seu servidor Diex
(por exemplo, `http://localhost:2020/s/documents/generate`). No Diex Cloud, `RestApiClient`
...
sanitizado) e o coloca em um limpo, página imprimível que mostra apenas o conteúdo do modelo- a mesma aparência que o PDF e a visualização no aplicativo.
  [Veja o ajudante](https://github.com/diexhq/diex/blob/main/packages/diex-apps/examples/document-generator/src/utils/render-document.ts).
...
`yarn diex dev:function:logs`, ou invocá-los diretamente com
