...
Os webhooks enviam dados para seus sistemas em tempo real quando eventos ocorrem no Diex — sem necessidade de polling. Use-os para manter sistemas externos em sincronia, acionar automações ou enviar alertas.
...
O Diex envia webhooks para estes tipos de eventos:
...
O Diex assina cada solicitação de webhook por segurança. Valide as assinaturas para garantir que as solicitações sejam autênticas.

### Headers

| Cabeçalho                    | Descrição                |
| ---------------------------- | ------------------------ |
| `X-Diex-Webhook-Signature` | Assinatura HMAC SHA256   |
| `X-Diex-Webhook-Timestamp` | Timestamp da solicitação |

### Etapas de validação

1. Obtenha o timestamp de `X-Diex-Webhook-Timestamp`
2. Crie a string: `{timestamp}:{JSON payload}`
3. Calcule o HMAC SHA256 usando o segredo do seu webhook
4. Compare com `X-Diex-Webhook-Signature`
...
| **Gatilho de webhook de fluxo de trabalho** | ENTRADA | Receber dados no Diex a partir de sistemas externos                            |
