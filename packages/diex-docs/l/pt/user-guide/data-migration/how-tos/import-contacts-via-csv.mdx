...
description: Guia completo passo a passo para importar pessoas/contactos para o Diex.
---

## Visão Geral

Este guia explica como importar os seus contactos (People) para o Diex. **Importe Empresas primeiro** se quiser associar Pessoas a Empresas.
...
* **Deve ser único** — sem duplicados no seu ficheiro ou no Diex
...
<Note>
**Use o formato de URL de domínio** (`https://acme.com`), não o rótulo. Isto corresponde à forma como as Empresas são armazenadas no Diex.
</Note>
...
<Note>
**Dica profissional:** Clique em **Download sample file** durante a importação ou exporte algumas Pessoas existentes para ver os nomes de colunas exatos que o Diex espera.
</Note>
...
3. Aguarde que o Diex analise o seu ficheiro

## Passo 4: Mapeie as suas colunas

O Diex tenta automaticamente corresponder as suas colunas aos campos. Revise e ajuste:
...
3. O Diex irá associar cada Pessoa à Empresa correspondente
...
1. O Diex exibe seus valores ao lado das opções existentes
2. Relacione cada valor no seu arquivo a uma opção do Diex
...
Antes de concluir a importação, o Diex valida seus dados:
...
| Erro                           | Causa                                      | Solução                                              |
| ------------------------------ | ------------------------------------------ | ---------------------------------------------------- |
| **E-mail duplicado**           | O e-mail já existe no Diex ou no arquivo | Remova o duplicado ou atualize o registro existente  |
...
Seus contatos agora estão no Diex! Próximos passos:
...
2. O Diex faz a correspondência dos registros por esse identificador único
...
<AccordionGroup>
  <Accordion title="Por que duas Pessoas não podem ter o mesmo e-mail?">
    O e-mail é um identificador único no Diex. Isso evita contatos duplicados e garante que a sincronização de e-mail vincule corretamente os e-mails à pessoa certa.
  </Accordion>
  ...
  <Accordion title="Posso importar o ID do meu CRM anterior?">
    Sim! Crie um campo personalizado marcado como "único" no seu modelo de dados para armazenar o ID externo. Observação: o nome do campo `id` é reservado para o ID interno do Diex.
  </Accordion>
