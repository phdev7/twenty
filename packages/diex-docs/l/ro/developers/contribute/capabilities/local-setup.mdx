...
icon: laptop-code
description: Ghidul pentru contribuitori (sau dezvoltatori curioși) care doresc să ruleze Diex local.
---
...
Înainte de a instala și utiliza Diex, asigurați-vă că instalați următoarele pe computerul dvs.:
...
Diex necesită un cache Redis pentru a oferi cea mai bună performanță.
...
## Pasul 8: Utilizați Diex

**Front-end**

Frontend-ul Diex va rula la [http://localhost:3001](http://localhost:3001).
Vă puteți loga folosind contul demo implicit: `tim@apple.dev` (parolă: `tim@apple.dev`)

**Back-end**

* Serverul Diex va fi operativ la [http://localhost:3000](http://localhost:3000)
