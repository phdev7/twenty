...
Fiecare obiect din Diex este livrat cu un set de **câmpuri de sistem** pe care nu le declari tu însuți. Acestea sunt create automat de server atunci când obiectul este provizionat:
...
<Note>
  `getFieldUniversalIdentifier` este disponibil începând cu `diex-sdk` 2.21.
</Note>
...
<Note>
  `getSystemRelationFieldUniversalIdentifier` este disponibil începând cu versiunea 2.23 a `diex-sdk` și necesită un server Diex pe versiunea 2.23 sau o versiune ulterioară.
</Note>
...
## Obiecte standard Diex

Pentru un obiect Diex **standard** (Person, Company, Opportunity, …), nu trebuie să derivezi nimic: identificatorii câmpurilor de sistem sunt constante pre-calculate pe care le poți importa direct.
