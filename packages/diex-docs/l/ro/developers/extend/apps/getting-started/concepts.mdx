---
title: Concepte
description: Cum funcționează aplicațiile Diex — modelul de entități, izolarea (sandboxing) și ciclul de viață al instalării.
icon: sitemap
---

Aplicațiile Diex sunt pachete TypeScript care vă extind spațiul de lucru cu obiecte personalizate, logică, componente UI și capabilități AI. Acestea rulează pe platforma Diex, cu izolare completă (sandboxing) și controale de permisiuni.

## Cum funcționează aplicațiile

O aplicație este o colecție de **entități** declarate folosind funcțiile `defineEntity()` din pachetul `diex-sdk`. SDK-ul detectează aceste declarații prin analiză AST în timpul construirii și produce un **manifest** — o descriere completă a ceea ce aplicația dvs. adaugă unui spațiu de lucru. Aceste funcții validează configurația în timpul build-ului și oferă completare automată în IDE și siguranța tipurilor.
...
| **Componentă front-end**          | Interfață React izolată (sandboxed) în Diex              | [Componente front-end](/l/ro/developers/extend/apps/layout/front-components)       |
...
* **Componentele front-end** rulează în Web Workers folosind Remote DOM — izolate de pagina principală, dar randând elemente DOM native (nu iframes). Acestea comunică cu Diex printr-un API al gazdei bazat pe transmiterea de mesaje.
* **Permisiunile** sunt aplicate la nivelul API-ului. Tokenul de rulare (`DIEX_APP_ACCESS_TOKEN`) este derivat din rolul definit în `defineApplication()`.
...
* **`yarn diex dev`** — monitorizează fișierele sursă și sincronizează în timp real modificările către un server Diex conectat. Clientul API tipizat este regenerat automat atunci când schema se schimbă.
* **`yarn diex dev:build`** — compilează TypeScript, împachetează funcțiile logice și componentele front-end cu esbuild și produce un manifest.
