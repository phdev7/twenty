...
După sincronizarea cu `yarn diex dev` (sau prin rularea o singură dată a comenzii `yarn diex apply`), acțiunea rapidă apare în colțul din dreapta sus al paginii:
...
Deoarece componenta returnează `null`, Diex omite redarea unui container pentru ea — nu apare spațiu gol în layout. Componenta are în continuare acces la toate hook-urile și la API-ul de comunicare cu gazda.

## Componentele Command din SDK

Pachetul `diex-sdk` oferă patru componente ajutătoare Command, concepute pentru componente front-end headless. Fiecare componentă execută o acțiune la montare, gestionează erorile afișând o notificare snackbar și demontează automat componenta front-end la final.

Importați-le din `diex-sdk/front-component`:
...
O funcție logică declarată cu `httpRouteTriggerSettings` este accesibilă prin HTTP la ruta sa. `RestApiClient` tratează căile care încep cu `/s/` ca rute ale aplicației, le rezolvă către URL-ul de la care sunt deservite funcțiile tale și le autentifică folosind `DIEX_APP_ACCESS_TOKEN`.

> **În Diex Cloud, funcțiile logice declanșate prin HTTP sunt deservite pe un domeniu dedicat pentru fiecare spațiu de lucru** la `https://\<your-workspace-subdomain>.withdiex.com\<path>`. Pentru apelanții externi, copiază URL-ul exact din setările **HTTP trigger** ale funcției sau din fila **Settings** a aplicației.
...
Calea transmisă către `RestApiClient` este proprietatea `httpRouteTriggerSettings.path` a funcției logice, cu prefixul `/s`. Păstrează `isAuthRequired: true`; `DIEX_APP_ACCESS_TOKEN` pe care Diex îl generează pentru componenta ta autentifică cererea:
...
<Note>
`DIEX_APP_ACCESS_TOKEN` este injectat automat — vezi [Application variables](#application-variables). Deoarece variabilele de aplicație secrete nu sunt niciodată expuse componentelor de front, păstrează cheile API și altă logică sensibilă în funcția logică, nu în componenta de front.
</Note>

### Apelarea API-ului REST Diex

Pentru a apela rute HTTP ale aplicației sau pentru a citi și scrie înregistrări Diex dintr-un front component, folosește `RestApiClient` din `diex-client-sdk/rest`. Trimite căile de forma `/s/...` către URL-ul de bază al funcțiilor spațiului tău de lucru, iar orice altă cale, inclusiv `/rest/...`, către `DIEX_API_URL`.
...
| Variabilă                 | Descriere                                                |
| ------------------------- | -------------------------------------------------------- |
| `DIEX_API_URL`          | URL-ul de bază al API-ului de bază Diex                |
| `DIEX_APP_ACCESS_TOKEN` | Token cu durată scurtă, limitat la rolul aplicației dvs. |

### `DIEX_FUNCTIONS_URL`

Diex injectează, de asemenea, `DIEX_FUNCTIONS_URL` în front components și în funcțiile logice: URL-ul de bază de la care sunt deservite funcțiile logice ale aplicației tale declanșate prin HTTP.

Există deoarece acel URL nu este întotdeauna chiar serverul Diex. În Diex Cloud, rutele aplicației sunt deservite pe un domeniu dedicat pentru fiecare spațiu de lucru (`https://\<your-workspace-subdomain>.withdiex.com` sau domeniul public principal al aplicației atunci când este configurat unul), astfel încât răspunsurile generate de aplicație să ruleze pe o origine izolată, nu pe originea aplicației Diex. Instanțele self-hosted și locale deservesc rutele aplicației sub prefixul `/s` chiar pe server și este posibil să nu seteze deloc variabila. Deoarece URL-ul de bază variază în funcție de spațiul de lucru și de instanță, codul tău nu îl poate hardcoda — serverul injectează valoarea corectă la runtime.

Rareori ai nevoie să o citești direct. Apelează-ți rutele prin `RestApiClient` folosind o cale prefixată cu `/s/`, iar clientul îți rezolvă URL-ul: elimină prefixul `/s` și țintește `DIEX_FUNCTIONS_URL`, folosind `\<DIEX_API_URL>/s` ca rezervă atunci când variabila nu este setată. Folosește `resolveUrl('/s/\<path>')` pentru a obține URL-ul absolut fără a trimite o cerere, de exemplu pentru un link. Citește variabila direct doar atunci când construiești manual un URL:
...
Componentele front-end pot declanșa navigare, ferestre modale și notificări folosind funcții din `diex-sdk`:
...
* **Componente UI Diex** — biblioteca proprie de componente a Diex; vezi [Folosirea componentelor UI Diex](#using-diex-ui-components) mai jos
...
## Folosirea componentelor UI Diex

Diex livrează biblioteca sa de componente ca pachetul [`diex-ui`](https://www.npmjs.com/package/diex-ui/v/1.0.0-alpha.1). Componentele frontend îl pot folosi pentru butoane, etichete, pastile de stare, chips, avataruri, pictograme, tipografie și tokeni de temă care se potrivesc automat cu tema luminoasă și întunecată a spațiului de lucru.

### Instalare

Adaugă pachetul în aplicația ta, fixat la versiunea cu care este livrată instanța ta de Diex:

```bash
yarn add diex-ui@1.0.0-alpha.1
```

`diex-ui` este inclus în componenta ta frontend la momentul build-ului, astfel încât trebuie să fie doar o dependență a aplicației tale — nu este nimic de configurat la runtime.
...
| Subpath                     | Ce exportă                                         |
| --------------------------- | -------------------------------------------------- |
| `diex-ui/input`           | `Button` și câmpuri de formular                    |
| `diex-ui/data-display`    | `Tag`, `Status`, `Chip`, `Avatar` și altele        |
| `diex-ui/feedback`        | `Callout`, `Banner`, `Info` și altele              |
| `diex-ui/typography`      | `H1Title`, `H2Title`, `H3Title`, `Label` și altele |
| `diex-ui/icon`            | Componente `Icon*` (de ex. `IconCheck`)            |
...
Importă pictograme individuale din `diex-ui/icon`:
...
Componentele Diex UI se potrivesc automat cu tema luminoasă și întunecată a spațiului de lucru — renderer-ul aplică schema de culori activă pe gazdă, iar componentele își determină culorile în funcție de aceasta.

Pentru a folosi aceiași tokeni de design în propriile tale stiluri inline, apelează hook-ul `useTheme()`. Acesta returnează tokenii de temă ai Diex (spațiere, culori, raze, fonturi) conectați la tema activă, fără a necesita vreo configurare `ThemeProvider` în componenta ta:
...
Pentru a ramifica explicit în funcție de schema activă, citește-o cu `useColorScheme()` din `diex-sdk/front-component`, care returnează `'light'` sau `'dark'`.
