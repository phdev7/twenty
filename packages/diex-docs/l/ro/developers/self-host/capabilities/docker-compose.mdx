...
Acest ghid oferă instrucțiuni pas cu pas pentru a instala și configura aplicația Diex folosind Docker Compose. Scopul este de a face procesul direct și de a preveni capcanele comune care ar putea deteriora configurarea dvs.
...
Instalați cea mai recentă versiune stabilă a Diex cu o singură comandă:

```bash
bash <(curl -sL https://raw.githubusercontent.com/diexhq/diex/main/packages/diex-docker/scripts/install.sh)
```

Pentru a instala o versiune sau ramură specifică:

```bash
VERSION=vx.y.z BRANCH=branch-name bash <(curl -sL https://raw.githubusercontent.com/diexhq/diex/main/packages/diex-docker/scripts/install.sh)
```
...
1. **Creați Fișierul .env**
   ...
   ```bash
   curl -o .env https://raw.githubusercontent.com/diexhq/diex/refs/heads/main/packages/diex-docker/.env.example
   ```
...
```bash
curl -o docker-compose.yml https://raw.githubusercontent.com/diexhq/diex/refs/heads/main/packages/diex-docker/docker-compose.yml
```
...
Dacă găzduiți diexCRM pe propriul computer, deschideți browserul și navigați la [http://localhost:3000](http://localhost:3000).
...
### Expuneți Diex pentru Acces Extern

Implicit, Diex rulează pe `localhost` la portul `3000`. Pentru a-l accesa printr-un domeniu extern sau adresă IP, trebuie să configurați `SERVER_URL` în fișierul dvs. `.env`.
...
SSL (HTTPS) este necesar pentru funcționarea corespunzătoare a anumitor funcții ale browserului. Deși aceste funcții pot funcționa în timpul dezvoltării locale (deoarece browserele tratează localhost diferit), o configurare SSL corespunzătoare este necesară când găzduiți Diex pe un domeniu obișnuit.
...
Recomandăm cu tărie configurarea Diex din spatele unui proxy revers cu terminare SSL pentru securitate și funcționalitate optimă.
...
2. **Actualizați Fișierul `.env`**
   ...
   * Acces prin domeniu cu SSL:
     ```ini
     SERVER_URL=https://mydiexapp.com
     ```
...
```bash
docker exec diex-postgres pg_dump -U postgres diex > backup_$(date +%Y%m%d).sql
```
...
```bash
0 2 * * * docker exec diex-postgres pg_dump -U postgres diex > /backups/diex_$(date +\%Y\%m\%d).sql
```
...
```bash
docker compose stop diex-server diex-front
```

2. Restaurați baza de date:

```bash
docker exec -i diex-postgres psql -U postgres diex < backup_20240115.sql
```
...
<Tip>
  Preferiți să nu rulați și să nu întrețineți Diex chiar dumneavoastră? [Găsiți un partener Diex certificat pentru găzduire](https://diex.com/partners/list?categories=HOSTING\&ref=docs-docker-compose) care să vă implementeze, găzduiască și gestioneze instanța de la un capăt la altul. *(Sau contactați echipa noastră: [contact@diex.com](mailto:contact@diex.com).)*
</Tip>
