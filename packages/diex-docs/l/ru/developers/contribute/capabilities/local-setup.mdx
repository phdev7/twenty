...
icon: laptop-code
description: Руководство для участников (или любопытных разработчиков), которые хотят запускать Diex локально.
---
...
Прежде чем установить и использовать Diex, убедитесь, что у вас установлено следующее:
...
Diex требует кэша Redis для обеспечения наилучшей производительности.
...
## Шаг 8: Использовать Diex

**Frontend**

Фронтенд Diex будет работать на [http://localhost:3001](http://localhost:3001).
Вы можете войти, используя учетную запись демо по умолчанию: `tim@apple.dev` (пароль: `tim@apple.dev`)

**Backend**

* Сервер Diex будет работать на [http://localhost:3000](http://localhost:3000).
