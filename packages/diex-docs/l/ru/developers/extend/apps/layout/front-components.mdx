...
description: Создавайте компоненты React, которые отображаются внутри интерфейса Diex в изолированной песочнице.
icon: window-maximize
---

Фронтенд-компоненты — это компоненты React, которые отображаются непосредственно внутри интерфейса Diex. Они выполняются в изолированном Web Worker с использованием Remote DOM — ваш код изолирован (sandboxed), но рендерится нативно на странице, а не в iframe.

## Где можно использовать фронт-компоненты

Фронт-компоненты могут отображаться в двух местах внутри Diex:
...
После синхронизации с помощью `yarn diex dev` (или однократного запуска `yarn diex dev --once`) быстрое действие появится в правом верхнем углу страницы:
...
import { defineFrontComponent } from 'diex-sdk/define';
import { useRecordId, enqueueSnackbar } from 'diex-sdk/front-component';
...
Поскольку компонент возвращает `null`, Diex пропускает рендеринг контейнера для него — в макете не появляется пустое место. Компонент по-прежнему имеет доступ ко всем хукам и API взаимодействия с хостом.

## Компоненты SDK Command

Пакет `diex-sdk` предоставляет четыре вспомогательных компонента Command, предназначенных для headless фронт-компонентов. Каждый компонент выполняет действие при монтировании, обрабатывает ошибки, показывая уведомление snackbar, и автоматически размонтирует фронт-компонент по завершении.

Импортируйте их из `diex-sdk/command`:
...
import { defineFrontComponent } from 'diex-sdk/define';
import { Command } from 'diex-sdk/command';
import { CoreApiClient } from 'diex-sdk/clients';
...
import { defineFrontComponent } from 'diex-sdk/define';
import { CommandModal } from 'diex-sdk/command';
...
Логическая функция, объявленная с `httpRouteTriggerSettings`, доступна по HTTP по своему пути маршрута. Diex внедряет в воркер базовый URL, с которого обслуживаются ваши функции, в виде `DIEX_FUNCTIONS_URL` вместе с `DIEX_APP_ACCESS_TOKEN`, который аутентифицирует вызов. Пока что нет отдельного клиентского SDK для вызова ваших собственных функций, поэтому вызывайте их с помощью обычного `fetch`:

> **В Diex Cloud логические функции с HTTP-триггером обслуживаются на выделенном домене для каждого рабочего пространства** по адресу `https://\<your-workspace-subdomain>.diex.com\<path>` — именно к этому и разрешается `DIEX_FUNCTIONS_URL`. Для внешних вызовов скопируйте точный URL из настроек **HTTP trigger** функции или на вкладке **Settings** приложения.

<Warning>
  Устаревший маршрут функции `/s/` **не рекомендуется к использованию** и будет **деактивирован 2026-07-24**. Вместо этого используйте `DIEX_FUNCTIONS_URL` (выше) и перенесите все жестко заданные URL вида `/s/` до этой даты. Маршрут `/s/` по-прежнему доступен при самостоятельном размещении (self-hosting).
</Warning>
...
const SyncPrs = () => {
  const execute = async () => {
    await fetch(`${process.env.DIEX_FUNCTIONS_URL}/github/fetch-prs`, {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${process.env.DIEX_APP_ACCESS_TOKEN}`,
        'Content-Type': 'application/json',
      },
      body: JSON.stringify({ owner: 'diexhq', repo: 'diex' }),
    });
  };
...
Путь, добавляемый к `DIEX_FUNCTIONS_URL`, — это значение `httpRouteTriggerSettings.path` логической функции. Сохраните `isAuthRequired: true`; `DIEX_APP_ACCESS_TOKEN`, который Diex выпускает для вашего компонента, аутентифицирует запрос:
...
<Note>
`DIEX_FUNCTIONS_URL` и `DIEX_APP_ACCESS_TOKEN` внедряются автоматически — см. [переменные приложения](#application-variables). Поскольку секретные переменные приложения никогда не раскрываются front-компонентам, храните ключи API и другую конфиденциальную логику в логической функции, а не во front-компоненте.
</Note>

### Вызов REST API Diex

Чтобы читать или изменять записи Diex из фронт-компонента, используйте `RestApiClient` из `diex-client-sdk/rest`. Он принадлежит к тому же семейству клиентов, что и `CoreApiClient` и `MetadataApiClient`, но нацелен на REST API Diex (`/rest/...`) вместо GraphQL API, считывая базовый URL из `DIEX_API_URL`.
...
| Переменная                | Описание                                                                      |
| ------------------------- | ----------------------------------------------------------------------------- |
| `DIEX_FUNCTIONS_URL`    | Базовый URL, с которого обслуживаются функции HTTP-логики вашего приложения   |
| `DIEX_API_URL`          | Базовый URL основного API Diex                                              |
| `DIEX_APP_ACCESS_TOKEN` | Краткоживущий токен с областью действия, ограниченной ролью вашего приложения |

## API взаимодействия с хостом

Компоненты фронтенда могут вызывать навигацию, модальные окна и уведомления с помощью функций из `diex-sdk`:
...
import { defineFrontComponent, numberOfSelectedRecords } from 'diex-sdk/define';
...
* **Diex UI components** — собственная библиотека компонентов Diex; см. раздел [Using Diex UI components](#using-diex-ui-components) ниже
...
## Использование компонентов Diex UI

Diex поставляет свою библиотеку компонентов как пакет [`diex-ui`](https://www.npmjs.com/package/diex-ui/v/1.0.0-alpha.1). Компоненты фронтенда могут использовать его для кнопок, тегов, статусных плашек, чипов, аватаров, иконок, типографики и токенов темы, которые автоматически соответствуют светлой и тёмной теме рабочего пространства.

### Установка

Добавьте пакет в своё приложение, зафиксировав его на версии, с которой поставляется ваш экземпляр Diex:

```bash
yarn add diex-ui@1.0.0-alpha.1
```

`diex-ui` включается в ваш компонент фронтенда на этапе сборки, поэтому его достаточно иметь в зависимостях вашего приложения — во время выполнения ничего настраивать не нужно.
...
| --------------------------- | ------------------------------------------------- |
| `diex-ui/input`           | `Button` и элементы ввода формы                   |
| `diex-ui/data-display`    | `Tag`, `Status`, `Chip`, `Avatar` и другие        |
| `diex-ui/feedback`        | `Callout`, `Banner`, `Info` и другие              |
| `diex-ui/typography`      | `H1Title`, `H2Title`, `H3Title`, `Label` и другие |
| `diex-ui/icon`            | Компоненты `Icon*` (например, `IconCheck`)        |
...
Импортируйте отдельные иконки из `diex-ui/icon`:
...
Компоненты Diex UI автоматически подстраиваются под светлую и тёмную темы рабочего пространства — рендерер применяет активную цветовую схему на хосте, а компоненты вычисляют свои цвета относительно неё.

Чтобы использовать те же дизайн‑токены в собственных встроенных стилях, вызовите хук `useTheme()`. Он возвращает токены темы Diex (отступы, цвета, радиусы, шрифты), привязанные к активной теме, без необходимости настраивать `ThemeProvider` в вашем компоненте:
...
Чтобы явно разветвлять логику по активной цветовой схеме, считайте её с помощью `useColorScheme()` из `diex-sdk/front-component`, который возвращает `'light'` или `'dark'`.
