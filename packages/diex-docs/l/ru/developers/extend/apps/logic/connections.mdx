...
export default defineApplication({
  universalIdentifier: '...',
  displayName: 'Linear',
  description: 'Connect Linear to Diex.',
  // OAuth client credentials live on the app registration (one OAuth app per
  // Diex server, configured by the admin) — not per-workspace. Declare them
...
* Используйте `serverVariables` (не `applicationVariables`) — учетные данные OAuth являются общими для сервера, и на каждом сервере Diex используется одно приложение OAuth.
...
```
https://<your-diex-server>/auth/apps/callback
```
...
4. Откройте установленное приложение в Diex под учетной записью администратора сервера → задайте значения в соответствующих `serverVariables`.
