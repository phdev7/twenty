...
Diex içindeki her nesne, kendiniz asla tanımlamadığınız bir dizi **sistem alanı** ile birlikte gelir. Bunlar, nesne sağlandığında sunucu tarafından otomatik olarak oluşturulur:
...
<Note>
  `getFieldUniversalIdentifier`, `diex-sdk` 2.21 sürümünden itibaren kullanılabilir.
</Note>
...
<Note>
  `getSystemRelationFieldUniversalIdentifier` 2.23 sürümünden itibaren `diex-sdk` içinde kullanılabilir ve Diex sunucusunun 2.23 veya daha yeni bir sürümünü gerektirir.
</Note>
...
## Standart Diex nesneleri

**Standart** bir Diex nesnesi (Person, Company, Opportunity, …) için hiçbir şey türetmeniz gerekmez: sistem alanı tanımlayıcıları, doğrudan içe aktarabileceğiniz önceden hesaplanmış sabitlerdir.
