...
description: Diex'nin UI'si içinde, korumalı alan (sandbox) izolasyonu ile görüntülenen React bileşenleri oluşturun.
icon: window-maximize
---

Ön uç bileşenler, Diex'nin UI'si içinde doğrudan görüntülenen React bileşenleridir. Remote DOM kullanan **izole bir Web Worker** içinde çalışırlar — kodunuz, korumalı (sandbox) ve opak kökenli bir iframe içinde yürütülür; ancak UI, o iframe ile sınırlanmak yerine sayfada yerel olarak işlenir.

## Ön uç bileşenlerinin kullanılabileceği yerler

Ön uç bileşenler, Diex içinde iki konumda işlenebilir:
...
const HelloWorld = () => {
  return (
    <div style={{ padding: '20px', fontFamily: 'sans-serif' }}>
      ...
      <p>This component renders inside Diex.</p>
    </div>
  );
};
...
`yarn diex dev` ile senkronize ettikten sonra (veya tek seferlik bir `yarn diex apply` çalıştırdıktan sonra), hızlı işlem sayfanın sağ üst köşesinde görünür:
...
import { defineFrontComponent } from 'diex-sdk/define';
import { useSelectedRecordIds, enqueueSnackbar } from 'diex-sdk/front-component';
...
Bileşen `null` döndürdüğü için, Diex bunun için bir kapsayıcı oluşturmayı atlar — düzende boş alan görünmez. Bileşen yine de tüm hook'lara ve host iletişim API'sine erişime sahiptir.

## SDK Command bileşenleri

`diex-sdk` paketi, headless ön uç bileşenler için tasarlanmış dört Command yardımcı bileşeni sağlar. Her bileşen bağlandığında bir eylem yürütür, hataları bir snackbar bildirimi göstererek ele alır ve tamamlandığında ön bileşeni otomatik olarak kaldırır.

Bunları `diex-sdk/front-component` içinden içe aktarın:
...
import { defineFrontComponent } from 'diex-sdk/define';
import { Command } from 'diex-sdk/front-component';
...
import { defineFrontComponent } from 'diex-sdk/define';
import { CommandModal } from 'diex-sdk/front-component';
...
`httpRouteTriggerSettings` ile bildirilen bir mantık işlevi, rota yolunda HTTP üzerinden erişilebilir durumdadır. `RestApiClient`, `/s/` ile başlayan yolları uygulama rotaları olarak ele alır, bunları fonksiyonlarınızın sunulduğu URL'ye çözümler ve `DIEX_APP_ACCESS_TOKEN` ile kimlik doğrulaması yapar.

> **Diex Cloud üzerinde, HTTP ile tetiklenen mantık işlevleri, çalışma alanı başına ayrılmış özel bir etki alanında** `https://\<your-workspace-subdomain>.withdiex.com\<path>` adresinde sunulur. Harici çağrıcılar için, tam URL’yi işlevin **HTTP trigger** ayarlarından veya uygulamanın **Settings** sekmesinden kopyalayın.
...
`RestApiClient`'e iletilen yol, başına `/s` eklenmiş mantık işlevinin `httpRouteTriggerSettings.path` değeridir. `isAuthRequired: true` ayarını koruyun; bileşeniniz için Diex'nin oluşturduğu `DIEX_APP_ACCESS_TOKEN` isteği kimlik doğrular:
...
<Note>
`DIEX_APP_ACCESS_TOKEN` otomatik olarak enjekte edilir — bkz. [Uygulama değişkenleri](#application-variables). Gizli uygulama değişkenleri asla ön bileşenlere açığa çıkarılmadığından, API anahtarlarını ve diğer hassas mantığı ön bileşende değil, mantık işlevinin içinde tutun.
</Note>

### Diex REST API'sini çağırma

Uygulama HTTP rotalarını çağırmak veya bir front bileşeninden Diex kayıtlarını okumak ve yazmak için `diex-client-sdk/rest` paketinden `RestApiClient` kullanın. `/s/...` yollarını çalışma alanınızdaki işlevlerin temel URL'sine, `/rest/...` dahil diğer tüm yolları ise `DIEX_API_URL`'ye gönderir.
...
const client = new RestApiClient({
  baseUrl: 'https://myworkspace.diex.com',
  token: 'my-token',
});
```
...
```tsx
import { RestApiClient, RestApiClientError } from 'diex-client-sdk/rest';
...
| Değişken                  | Açıklama                                                  |
| ------------------------- | --------------------------------------------------------- |
| `DIEX_API_URL`          | Diex çekirdek API'sinin temel URL'si                    |
| `DIEX_APP_ACCESS_TOKEN` | Uygulamanızın rolüyle sınırlanan kısa ömürlü bir belirteç |

### `DIEX_FUNCTIONS_URL`

Diex ayrıca `DIEX_FUNCTIONS_URL` değişkenini front bileşenlerine ve mantık işlevlerine enjekte eder: bu, uygulamanızın HTTP ile tetiklenen mantık işlevlerinin sunulduğu temel URL'dir.

Bu değişken, bu URL her zaman Diex sunucusunun kendisi olmadığı için mevcuttur. Diex Cloud üzerinde, uygulama rotaları, uygulama tarafından yazılmış yanıtların Diex uygulamasının kaynağı yerine yalıtılmış bir kaynak üzerinde çalışması için, çalışma alanı başına ayrılmış özel bir etki alanında (`https://\<your-workspace-subdomain>.withdiex.com` veya yapılandırıldığında uygulamanın birincil genel etki alanında) sunulur. Self-hosted ve yerel örnekler, uygulama rotalarını sunucunun kendisinde `/s` öneki altında sunar ve değişkeni hiç ayarlamayabilir. Temel URL, çalışma alanına ve örneğe göre değiştiğinden, kodunuz bunu sabit olarak yazamaz — sunucu, çalışma zamanında doğru değeri enjekte eder.

Bu değişkeni doğrudan okumanız nadiren gerekir. Rotalarınızı, yolu `/s/` önekiyle birlikte vererek `RestApiClient` üzerinden çağırın; istemci URL'yi sizin için çözümler: `/s` önekini kaldırır ve `DIEX_FUNCTIONS_URL`'yi hedefler, değişken ayarlı değilse `\<DIEX_API_URL>/s` adresine geri döner. Mutlak URL'yi bir istek göndermeden elde etmek için, örneğin bir bağlantı için, `resolveUrl('/s/\<path>')` kullanın. Değişkeni yalnızca bir URL'yi elle oluştururken doğrudan okuyun:

```ts
const routeUrl = `${process.env.DIEX_FUNCTIONS_URL || `${process.env.DIEX_API_URL}/s`}/documents/generate`;
```

## Host iletişim API'si

Ön uç bileşenleri, `diex-sdk`'deki işlevleri kullanarak gezinmeyi, modalları ve bildirimleri tetikleyebilir:
...
import { defineFrontComponent } from 'diex-sdk/define';
import { enqueueSnackbar, closeSidePanel, useSelectedRecordIds } from 'diex-sdk/front-component';
import { CoreApiClient } from 'diex-client-sdk/core';
...
import { enqueueSnackbar, closeSidePanel } from 'diex-sdk/front-component';
import { CoreApiClient } from 'diex-client-sdk/core';
...
```tsx
import { defineFrontComponent } from 'diex-sdk/define';
import { getPublicAssetUrl } from 'diex-sdk/utils';
...
* **Diex UI bileşenleri** — Diex'nin kendi bileşen kitaplığı; aşağıda [Diex UI bileşenlerini kullanma](#using-diex-ui-components) bölümüne bakın
...
## Diex UI bileşenlerini kullanma

Diex, bileşen kitaplığını [`diex-ui`](https://www.npmjs.com/package/diex-ui/v/1.0.0-alpha.1) paketi olarak sunar. Ön uç bileşenleri bunu, çalışma alanının açık ve koyu temasıyla otomatik olarak eşleşen düğmeler, etiketler, durum rozetleri, çipler, avatarlar, simgeler, tipografi ve tema belirteçleri için kullanabilir.

### Kurulum

Paketi, Diex örneğinizle birlikte gelen sürüme sabitleyerek uygulamanıza ekleyin:

```bash
yarn add diex-ui@1.0.0-alpha.1
```

`diex-ui`, derleme zamanında ön uç bileşeninize paketlenir, bu nedenle yalnızca uygulamanızın bir bağımlılığı olması gerekir — çalışma zamanında yapılandırmanız gereken hiçbir şey yoktur.
...
| --------------------------- | -------------------------------------------------------- |
| `diex-ui/input`           | `Button` ve form girdileri                               |
| `diex-ui/data-display`    | `Tag`, `Status`, `Chip`, `Avatar` ve daha fazlası        |
| `diex-ui/feedback`        | `Callout`, `Banner`, `Info` ve daha fazlası              |
| `diex-ui/typography`      | `H1Title`, `H2Title`, `H3Title`, `Label` ve daha fazlası |
| `diex-ui/icon`            | `Icon*` bileşenleri (ör. `IconCheck`)                    |
| `diex-ui/theme-constants` | `ThemeProvider`, `themeCssVariables`                     |
...
Tek tek simgeleri `diex-ui/icon` içinden içe aktarın:

```tsx
import { IconBox, IconCheck } from 'diex-ui/icon';
```
...
Diex UI bileşenleri, çalışma alanının açık ve koyu temasıyla otomatik olarak eşleşir — oluşturucu etkin renk şemasını barındırıcıda uygular ve bileşenler renklerini buna göre belirler.

Kendi satır içi stillerinizde aynı tasarım belirteçlerini kullanmak için `useTheme()` kancasını çağırın. Bu kanca, etkin temaya bağlı Diex tema belirteçlerini (boşluklandırma, renkler, köşe yarıçapları, yazı tipleri) döndürür; bileşeninizde `ThemeProvider` kurulumuna gerek yoktur:
...
Etkin şemaya açıkça dallanmak için, `diex-sdk/front-component` içindeki `useColorScheme()` ile okuyun; bu kanca `'light'` veya `'dark'` döndürür.
