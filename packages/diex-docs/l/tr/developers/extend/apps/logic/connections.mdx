...
* `serverVariables` kullanın (`applicationVariables` değil) — OAuth kimlik bilgileri sunucu genelidir ve her Diex sunucusu için bir OAuth uygulaması vardır.
...
4. Yüklü uygulamayı Diex'de bir sunucu yöneticisi olarak açın → karşılık gelen `serverVariables` üzerinde değerleri ayarlayın.
