...
Diex uygulamaları **İngilizce** olarak oluşturulur: kaynak ve manifestinizdeki dizeler İngilizce kaynak metindir, `en` çevirdiğiniz kaynak yerel ayarıdır ve çevrilmeden bırakılan herhangi bir yerel ayar buna geri döner.
...
Çeviri yardımcılarını `diex-sdk/front-component` içinden içe aktarın:
...
`diex dev:build`, katalogları derler ve geçerli kullanıcı için doğru dili sunar:
...
`diex dev:build`'i yeniden çalıştırmayı (ve yeniden dağıtmayı) gerektirir; bu, diğer herhangi bir değişiklikle aynıdır.

<Note>
Çeviriler `diex dev:build` (ve `diex apply`) tarafından derlenir. Sürekli
`diex dev` izlemesi kaynak dizeleri gösterir, bu yüzden yerelleştirilmiş çıktıyı tek seferlik bir build ile test edin.
</Note>
