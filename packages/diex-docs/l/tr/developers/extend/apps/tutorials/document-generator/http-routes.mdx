...
ve kendi sunucunuzda barındırılan örneklerde, bu yol Diex sunucunuz
tarafından sunulur (örneğin `http://localhost:2020/s/documents/generate`). Diex Cloud'da, `RestApiClient`
...
[Yardımcıyı inceleyin](https://github.com/diexhq/diex/blob/main/packages/diex-apps/examples/document-generator/src/utils/render-document.ts).
...
<Tip>
Ayrıca test ederken bir işlevin günlüklerini `yarn diex dev:function:logs` ile gerçek zamanlı izleyebilir veya `yarn diex dev:function:exec` ile doğrudan çağırabilirsiniz.
</Tip>
