...
export default defineApplication({
  ...
  websiteUrl: 'https://docs.diex.com/l/tr/developers/extend/apps',
...
Ayrıca uygulamanın bulunabilir olması için `package.json` dosyasına `diex-app` anahtar sözcüğünü ekleyin:
...
yarn diex plan   # preview the metadata diff
```
...
yarn diex app:publish --private -r <remote>
```

`app:publish` varsayılan olarak oluşturur ve npm'e yayımlar; `--private` bunun yerine bir tarball dosyasını bir Diex sunucusunun özel kaydına yükler. Yayımlanmış bir uygulamayı bir örneğin pazaryerinde görünür kılmak için, bir katalog eşitlemesini tetikleyin:

```bash filename="Terminal"
yarn diex dev:catalog-sync -r <remote>
```
...
[`packages/diex-apps/examples/document-generator`](https://github.com/diexhq/diex/tree/main/packages/diex-apps/examples/document-generator).
