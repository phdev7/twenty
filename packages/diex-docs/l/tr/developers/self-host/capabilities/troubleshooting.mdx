...
Mevcut bir Diex örneğiniz varsa ve üretim verileri varsa, bu adımları **TAKİP ETMEYİN**, çünkü bu veritabanınızı kalıcı olarak silecektir!

Diex'yi ilk kez kurarken, varsayılan veritabanı parolasını değiştirmek isteyebilirsiniz.
...
Diex kurulumu sırasında, postgres veritabanınızı doğru şemalar, uzantılar ve kullanıcılarla hazırlamanız gerekir.
...
#### Modül 'diex-emails' veya buna karşılık gelen tür açıklamaları bulunamıyor.

`diex-emails` paketini veritabanını `npx nx run diex-emails:build` ile başlatmadan önce derlemeniz gerekiyor.

#### Eksik diex-x paketi

Kök dizinde yarn çalıştırdığınızdan ve ardından `npx nx server:dev diex-server` çalıştırdığınızdan emin olun. Bu hala çalışmıyorsa eksik paketi manuel olarak derlemeyi deneyin.
...
#### `npx nx start` veya `npx nx start diex-front` çalıştırırken, bellek hatası verildi

`npx nx start` komutu yerine yalnızca ihtiyaç duyduğunuz servisleri çalıştırın. Örneğin, sunucuda çalışıyorsanız yalnızca `npx nx worker diex-server` çalıştırın.

**Bu işe yaramazsa:**
WSL'de yalnızca `npx nx run diex-server:start` çalıştırmayı denediyseniz ve aşağıdaki bellek hatasıyla karşılaşıyorsanız:
...
**Bu işe yaramazsa:**
Hangi işlemlerin makinenizin RAM'inin çoğunu kullandığını araştırın. Diex'de bazı VScode uzantılarının çok fazla RAM aldığını fark ettik, bu yüzden onları geçici olarak devre dışı bırakıyoruz.
...
* Diex uygulamanızı açın, `/settings` sayfasına gidin ve ekranın sol alt köşesinde bulunan **Gelişmiş** seçeneğini etkinleştirin.
* Yeni bir webhook oluşturun.
* **Benzersiz Webhook URL'nizi** Diex'deki **Endpoint URL** alanına yapıştırın. **Filtreleri** `Şirketler` ve `Oluşturuldu` olarak ayarlayın.
...
Aşağıdaki örnekte olduğu gibi `packages/diex-ui/vite-config.ts` içindeki checker eklentisini yorum satırına alın
...
Eğer Diex'yi ters proxy'nin arkasında çalıştırıyorsanız ve bağlantı sorunları yaşıyorsanız:
...
2. **Ters Proxy Ayarlarını Kontrol Edin:**
   ...
   * Ters proxy'nizin Diex sunucusuna talepleri düzgün bir şekilde yönlendirdiğinden emin olun.
...
3. **Servisleri Yeniden Başlatın:**
   ...
   Değişiklik yaptıktan sonra hem ters proxy'yi hem de Diex konteynerlerini yeniden başlatın.
...
* Topluluk Desteği:
  ...
  [Diex topluluğuna](https://github.com/diexhq/diex/issues) veya [destek kanallarına](https://discord.gg/cx5n4Jzs57) yardım için ulaşın.
