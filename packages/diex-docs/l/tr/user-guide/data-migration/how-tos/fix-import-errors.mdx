...
Dosyanızı yükledikten ve sütunları eşledikten sonra, Diex verilerinizi doğrular:

1. **Doğrulama çalışır** — Diex her satırı hatalar için kontrol eder
...
| ------------------------------------------------------------------------------ | -------------------------------------------------------------- |
| Yinelenen değerler sarı renkle vurgulanır                                      | Değer Diex'de zaten mevcut veya dosyanızda iki kez görünüyor |
...
**Seçenek 3: Mevcut kaydı Diex'nin güncellemesine izin verin**
...
3. Diex, yinelenen bir kayıt oluşturmak yerine mevcut kaydı güncelleyecektir
...
Benzersiz tanımlayıcılardan biri eşleştiği sürece Diex kaydı günceller.
</Note>
...
3. Kayıtların Diex'de zaten mevcut olup olmadığını kontrol edin
...
Bu, Diex'de o özel tanımlayıcıya sahip bir Şirket olmadığı anlamına gelir.
...
Diex'de mevcut olmayan bir kayda bağlanmaya çalışıyorsunuz.
...
Dosyanızdaki bir sütun eşlenemiyor çünkü alan Diex'de mevcut değil.
...
1. **Kullanıcı davetini kabul etmedi** — kullanıcı henüz Diex'de mevcut değil
2. **Eski sistemdeki kullanıcı kimliğini kullanma** — Diex başka bir sistemin kimliklerini eşleştiremez
3. **Yanlış e-posta biçimi** — e-posta, kullanıcının Diex hesabıyla eşleşmiyor

### Nasıl Düzeltilir

1. Tüm kullanıcıların Diex çalışma alanınıza **davetlerini kabul ettiğinden** emin olun
2. Kullanıcının **e-posta adresini** kullanın (adlarını veya eski sistem kimliğini değil)
3. Diex'ye katılmak için kullandıkları aynı e-postayı kullanın
...
<Check>Alan adı biçimi, Diex'nin biçimiyle birebir eşleşir</Check>
...
1. **Örnek dosyayı indirin** — Diex'nin beklediği tam biçimi görün
