...
description: Complete step-by-step guide to importing people/contacts into Diex.
---

## Overview

This guide walks you through importing your contacts (People) into Diex. **Import Companies first** if you want to link People to Companies.
...
- **Must be unique** — no duplicates in your file or in Diex
...
<Note>
**Use Domain URL format** (`https://acme.com`), not the label. This matches how Companies are stored in Diex.
</Note>
...
<Note>
**Pro tip:** Click **Download sample file** during import or export a few existing People to see the exact column names Diex expects.
</Note>
...
3. Wait for Diex to analyze your file

## Step 4: Map Your Columns

Diex automatically tries to match your columns to fields. Review and adjust:
...
3. Diex will link each Person to the matching Company
...
1. Diex shows your values alongside existing options
2. Match each value in your file to a Diex option
...
Before completing the import, Diex validates your data:
...
|-------|-------|----------|
| **Duplicate email** | Email already exists in Diex or file | Remove duplicate or update existing record |
...
Your contacts are now in Diex! Next steps:
...
2. Diex matches records by this unique identifier
...
<AccordionGroup>
  <Accordion title="Why can't two People have the same email?">
    Email is a unique identifier in Diex. This prevents duplicate contacts and ensures email sync correctly links emails to the right person.
  </Accordion>
  ...
  <Accordion title="Can I import the ID from my previous CRM?">
    Yes! Create a custom field marked as "unique" in your data model to store the external ID. Note: the field name `id` is reserved for Diex's internal ID.
  </Accordion>
