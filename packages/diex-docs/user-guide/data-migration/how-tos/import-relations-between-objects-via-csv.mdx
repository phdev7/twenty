...
The parent ("one" side) must exist in Diex before you can reference it.
...
If you need the `id`:
1. Export the parent records from Diex
...
Use `https://domain.com` (not just `domain.com`). This matches how Diex stores Company domains and prevents matching errors.
</Warning>
...
Diex will automatically link each child record to the matching parent.
...
| **Non-existent records** | The parent record doesn't exist in Diex | Import parent records first, or check for typos |
| **Case mismatch** | `Acme.com` in file but `acme.com` in Diex | Ensure exact case matching |
...
<AccordionGroup>
  <Accordion title="What if my parent record doesn't have a domain or email?">
    ...
    1. Use the Diex `id` (export parent records to get their IDs)
