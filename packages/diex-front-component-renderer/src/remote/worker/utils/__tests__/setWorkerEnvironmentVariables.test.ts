...
describe('setWorkerEnvironmentVariables', () => {
  ...
  it('should set process.env on globalThis', () => {
    setWorkerEnvironmentVariables({
      DIEX_APP_ACCESS_TOKEN: 'test-key',
      DIEX_API_URL: 'https://api.example.com',
    });
    ...
    expect(processEnvironment['DIEX_APP_ACCESS_TOKEN']).toBe('test-key');
    expect(processEnvironment['DIEX_API_URL']).toBe(
  ...
  it('should let later calls win when the same variable is set twice', () => {
    setWorkerEnvironmentVariables({
      DIEX_API_URL: 'https://application-provided.example.com',
    });
    setWorkerEnvironmentVariables({
      DIEX_API_URL: 'https://system-provided.example.com',
    });
