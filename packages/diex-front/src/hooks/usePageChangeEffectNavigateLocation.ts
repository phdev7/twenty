...
export const usePageChangeEffectNavigateLocation = () => {
  ...
  const isDiexWorkspaceOnboarding =
    currentWorkspace?.workspaceMembersCount === 1 ||
    currentUser?.userVars?.ONBOARDING_DIEX_WORKSPACE_PENDING === true;

  if (
    onboardingStatus === OnboardingStatus.SYNC_EMAIL &&
    !isMatchingLocation(
      location,
      isDiexWorkspaceOnboarding ? AppPath.DiexOnboarding : AppPath.SyncEmails,
    )
  ) {
    // The legacy connect-account flag is also used for invited members. The
    // explicit workspace marker covers new workspaces with more than one
    // member while the member-count fallback keeps older workspaces working.
    return isDiexWorkspaceOnboarding
