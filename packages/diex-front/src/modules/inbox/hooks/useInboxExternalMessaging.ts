...
import { SEND_DIEX_EMAIL } from '@/inbox/graphql/inboxEmailSyncQueries';
import {
...
import { loadEligibleDiexEmailChannels } from '@/inbox/utils/loadEligibleDiexEmailChannels';
...
import { syncDiexEmailToInbox } from '@/inbox/utils/syncDiexEmailToInbox';
...
export const useInboxExternalMessaging = ({
  ...
  const syncDiexEmail = useCallback(async (): Promise<void> => {
    if (workspaceId === null) {
      enqueueErrorSnackBar({
    ...
    try {
      const result = await syncDiexEmailToInbox({
        apolloClient,
        apolloCoreClient,
        workspaceId,
      ...
      if (result.eligibleChannels === 0) {
        enqueueWarningSnackBar({
          message:
            'Nenhum canal de e-mail compartilhado está disponível. No Diex, habilite a sincronização e a visibilidade "Compartilhar tudo" na conta que alimentará a Inbox.',
        });
      ...
      enqueueErrorSnackBar({
        message: 'Não foi possível sincronizar o e-mail do Diex.',
      });
    } finally {
      setBusyAction(null);
    }
    ...
    async ({
      ...
      try {
        ...
        if (
          ...
          selectedConversation.provider !== 'DIEX_EMAIL'
        ) {
          throw new Error(
        ...
        const metadata = readDiexEmailConversationMetadata(
        ...
        const channels = await loadEligibleDiexEmailChannels(apolloClient);
        ...
        return {
          ...
          confirmationToken: `diex-email:${crypto.randomUUID()}`,
          message:
            'Revise destinatário, assunto e corpo antes de confirmar o envio nativo do Diex.',
        };
      } catch (error) {
        enqueueErrorSnackBar({
          message:
    ...
    async (preview: InboxExternalMessagePreview): Promise<boolean> => {
      ...
      if (preview.channel === 'EMAIL') {
        ...
        const isInvalidEmailPreview =
          !preview.confirmationToken.startsWith('diex-email:') ||
          !expectedDestination ||
      ...
      try {
        ...
        if (data?.sendEmail?.success !== true) {
          throw new Error(
            data?.sendEmail?.error || 'O Diex não aceitou o envio do e-mail.',
          );
        }
        ...
        if (reconciliationSucceeded) {
          enqueueSuccessSnackBar({
            message:
              'E-mail enviado pela conta nativa do Diex e sincronizado na Inbox.',
          });
        } else {
          enqueueWarningSnackBar({
            message:
