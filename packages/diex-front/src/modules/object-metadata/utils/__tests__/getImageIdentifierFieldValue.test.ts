...
describe('getImageIdentifierFieldValue', () => {
  ...
  describe('LINKS', () => {
    it('returns the favicon url when requests to diex-icons are allowed', () => {
      expect(
        getImageIdentifierFieldValue(
          buildRecord({ primaryLinkUrl: 'diex.com' }),
          buildFieldMetadataItem(FieldMetadataType.LINKS),
          true,
        ),
      ).toBe('https://diex-icons.com/diex.com');
    });

    it('returns null when requests to diex-icons are not allowed', () => {
      expect(
        getImageIdentifierFieldValue(
          buildRecord({ primaryLinkUrl: 'diex.com' }),
          buildFieldMetadataItem(FieldMetadataType.LINKS),
