...
export const WithSecondaryLinks: Story = {
  args: {
    value: {
      ...
      secondaryLinks: [
        {
          url: 'https://docs.diex.com',
          label: 'Documentation',
        },
        {
          url: 'https://github.com/diexhq/diex',
          label: 'GitHub',
        },
      ],
    },
...
export const CreatePrimaryLink: Story = {
  play: async ({ canvasElement }) => {
    ...
    const input = await canvas.findByPlaceholderText('URL');
    await userEvent.type(input, 'https://www.diex.com{enter}');

    const linkDisplay = await canvas.findByText('diex.com');
...
export const AddSecondaryLink: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    const input = await canvas.findByPlaceholderText('URL');
    await userEvent.type(input, 'https://docs.diex.com{enter}');

    const secondaryLink = await canvas.findByText('docs.diex.com');
...
export const DeletePrimaryLink: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    const listItemToDelete = await canvas.findByText('Diex Website');
...
export const DeletePrimaryLinkAndUseSecondaryLinkAsTheNewPrimaryLink: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    const oldPrimaryLink = canvas.queryByText('Diex Website');
