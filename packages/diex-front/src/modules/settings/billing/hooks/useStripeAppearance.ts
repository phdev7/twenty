...
import { THEME_DARK, THEME_LIGHT } from 'diex-ui/theme';
import { ThemeContext } from 'diex-ui/theme-constants';

// Stripe's Appearance API rejects CSS color(display-p3 ...) values, which is
// how the Diex theme stores colors; map them to sRGB so the PaymentElement
...
const toStripeColor = (color: string): string => {
  ...
  // oxlint-disable-next-line diex/no-hardcoded-colors
