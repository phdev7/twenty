...
import { LinkType } from 'diex-ui/navigation';
...
describe('checkUrlType', () => {
  it('should detect LinkedIn urls', () => {
    ...
    expect(checkUrlType('linkedin.com/company/diex')).toBe(LinkType.LinkedIn);
  });

  it('should detect Twitter urls', () => {
    expect(checkUrlType('https://twitter.com/diex')).toBe(LinkType.Twitter);
  });

  it('should detect X urls as Twitter', () => {
    expect(checkUrlType('https://x.com/diex')).toBe(LinkType.Twitter);
  });

  it('should detect Facebook urls', () => {
    expect(checkUrlType('https://www.facebook.com/diex')).toBe(
      LinkType.Facebook,
    );
  });
  ...
  it('should detect TikTok urls', () => {
    expect(checkUrlType('https://www.tiktok.com/@diex')).toBe(
      LinkType.TikTok,
    );
    expect(checkUrlType('tiktok.com/@diex')).toBe(LinkType.TikTok);
  });

  it('should detect Bluesky urls', () => {
    expect(checkUrlType('https://bsky.app/profile/diex.bsky.social')).toBe(
      LinkType.Bluesky,
    );
  });
