...
import { SyncableEntity } from 'diex-shared/application';
...
export const registerDevCommands = (program: Command): void => {
  ...
  const devAction = async (
    ...
    if (options.dryRun && !options.once) {
      console.warn(
        chalk.yellow(
          '--dry-run only applies with --once. Ignoring it; run `yarn diex plan` to preview changes.',
        ),
      );
    }
    ...
    if (options.once) {
      console.warn(
        chalk.yellow(
          options.dryRun
            ? '⚠ `diex dev --once --dry-run` is deprecated. Use `diex plan` instead.'
            : '⚠ `diex dev --once` is deprecated. Use `diex apply` instead.',
        ),
      );
  ...
  program
    ...
    .option(
      '-o, --once',
      'Build and sync once, then exit (deprecated: use `diex apply`)',
    )
    .option(
      '--dry-run',
      'Preview the metadata changes without applying them (deprecated: use `diex plan`)',
    )
    .option(
      '-f, --force',
