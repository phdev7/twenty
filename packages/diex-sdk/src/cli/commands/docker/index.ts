...
const stopAction = (options: { test?: boolean }) => {
  ...
  if (!containerExists(containerName)) {
    console.log(chalk.yellow('No Diex server container found.'));

    return;
  }

  execSync(`docker stop ${containerName}`, { stdio: 'ignore' });
  console.log(chalk.green('Diex server stopped.'));
};
...
const statusAction = async (options: { test?: boolean }) => {
  ...
  if (!containerExists(containerName)) {
    ...
    console.log(
      chalk.gray(
        `  Run 'yarn diex docker:start${options.test ? ' --test' : ''}' to create one.`,
      ),
    );
...
const resetAction = (options: { test?: boolean }) => {
  ...
  const volumeData = options.test
    ? 'diex-app-dev-test-data'
    : 'diex-app-dev-data';
  const volumeStorage = options.test
    ? 'diex-app-dev-test-storage'
    : 'diex-app-dev-storage';
  ...
  console.log(chalk.green('Diex server data reset.'));
  console.log(
    chalk.gray(
      `Run 'yarn diex docker:start${options.test ? ' --test' : ''}' to start a fresh instance.`,
    ),
  );
};
...
const upgradeAction = async (
  ...
  if (data.containerRecreated) {
    console.log(
      chalk.gray(
        `  Run 'yarn diex docker:start${options.test ? ' --test' : ''}' to wait for the server to be ready.`,
      ),
    );
  }
};
...
export const registerServerCommands = (program: Command): void => {
  program
    ...
    .description(
      'Start the local Diex container (version defaults to the app `engines.diex` range, then `latest`)',
    )
  ...
  program
    .command('docker:stop')
    .description('Stop the local Diex container')
  ...
  const server = program
    ...
    .description(
      'Manage a Diex server (local instance and server-side actions)',
    );

  const deprecate = (oldCmd: string, newCmd: string) =>
    console.warn(
      chalk.yellow(
        `⚠ \`diex server ${oldCmd}\` is deprecated. Use \`diex ${newCmd}\` instead.`,
      ),
    );
