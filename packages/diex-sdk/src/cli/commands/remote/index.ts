...
import { normalizeUrl } from 'diex-shared/utils';
  ...
  if (serverUrl) {
    ...
    if (options.local) {
      if (!detectedUrl) {
        console.error(
          chalk.red(
            'No local Diex server found.\n' +
              'Start one with: yarn diex docker:start',
          ),
        );
        process.exit(1);
      }
...
const listAction = async () => {
  ...
  if (remotes.length === 0) {
    ...
    console.log("Use 'diex remote:add' to add one.");
  ...
  for (const remoteName of remotes) {
    ...
    const authMethod = config.diexCLIAccessToken
      ? 'oauth'
      : config.apiKey
        ? 'api-key'
        : 'none';
  ...
  console.log(
    '\n',
    chalk.gray("Use 'diex remote:use <name>' to change default"),
  );
};
...
export const registerRemoteCommands = (program: Command): void => {
  program
    ...
    .option('--local', 'Connect to a local Diex server (auto-detect)')
  ...
  const remote = program
    ...
    .description('Manage remote Diex servers');

  const deprecate = (oldCmd: string, newCmd: string) =>
    console.warn(
      chalk.yellow(
        `⚠ \`diex remote ${oldCmd}\` is deprecated. Use \`diex ${newCmd}\` instead.`,
      ),
    );
