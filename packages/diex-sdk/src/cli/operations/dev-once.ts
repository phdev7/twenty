...
import { type Manifest, OUTPUT_DIR } from 'diex-shared/application';
import { type MetadataValidationErrorResponse } from 'diex-shared/metadata';
import { isPlainObject } from 'diex-shared/utils';
  ...
  if (!validateAuth.serverUp) {
    return {
      ...
      error: {
        code: APP_ERROR_CODES.SYNC_FAILED,
        message:
          'Cannot reach Diex server.\n\n' +
          '  Start a local server:\n' +
          '    yarn diex docker:start\n\n' +
          '  Check server status:\n' +
          '    yarn diex docker:status',
      },
    };
  }
    ...
    if (outcome !== 'reauthenticated') {
      return {
        ...
        error: {
          code: APP_ERROR_CODES.SYNC_FAILED,
          message:
            'Authentication failed. Run `yarn diex remote:add` to authenticate.',
        },
      };
    }
  }
