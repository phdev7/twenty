...
const innerServerStart = async (
  ...
  if (existingUrl) {
    ...
    onProgress?.(`Diex server detected on ${existingUrl}`);
    ...
    if (!healthy) {
      return {
        ...
        error: {
          ...
          message:
            'Diex server did not become healthy in time.\n' +
            "Check: 'yarn diex docker:logs'",
        },
      };
    }
  ...
  if (containerExists(containerName)) {
    ...
    if (existingPort !== port) {
      onProgress?.(
        `Existing container uses port ${existingPort}. Run 'yarn diex docker:reset${isTest ? ' --test' : ''}' first to change ports.`,
      );
    }
    ...
    onProgress?.(
      `Pulling Docker image (${image}) and starting Diex container...`,
    );
    ...
    const runResult = spawnSync(
      ...
      [
        ...
        '-v',
        `${volumeStorage}:/app/packages/diex-server/.local-storage`,
        image,
      ],
      { stdio: 'inherit' },
    );
    ...
    if (runResult.status !== 0) {
      return {
        ...
        error: {
          ...
          message: 'Failed to start Diex container.',
        },
      };
    }
  }

  onProgress?.('Waiting for Diex to be ready...');
