...
export class ApiClient {
  ...
  constructor(options?: {
    ...
    this.client.interceptors.request.use(async (config) => {
      const diexConfig = await this.configService.getConfig();

      config.baseURL = this.serverUrlOverride ?? diexConfig.apiUrl;
    ...
    this.client.interceptors.response.use(
      ...
      async (error) => {
        if (error.response?.status === 401 && error.config) {
          ...
          console.error(
            chalk.red('Cannot connect to Diex server. Is it running?'),
          );
        }
        throw error;
      },
  ...
  async refreshToken(): Promise<string | null> {
    ...
    if (
      !config.diexCLIRefreshToken ||
      !config.diexCLIRegistrationClientId
    ) {
      return null;
    }
    ...
    try {
      const tokenResponse = await axios.post(`${config.apiUrl}/oauth/token`, {
        ...
        refresh_token: config.diexCLIRefreshToken,
        client_id: config.diexCLIRegistrationClientId,
      });
      ...
      await this.configService.setConfig({
        diexCLIAccessToken: newAccessToken,
        ...(newRefreshToken ? { diexCLIRefreshToken: newRefreshToken } : {}),
      });
  ...
  async resolveAuthToken(): Promise<string | undefined> {
    ...
    const config = await this.configService.getConfig();
    const cliToken = config.diexCLIAccessToken;
