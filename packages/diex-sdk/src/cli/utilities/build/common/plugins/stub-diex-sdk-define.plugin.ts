...
// Everything in diex-sdk/define is build-time metadata for the manifest
// extractor and is dead code in the user-app bundle. Stubbing it drops zod,
// diex-shared and ~1MB per logic-function bundle.
//
// Exports are derived from `Object.keys(diexSdkDefine)` so new ones land
...
export const DIEX_SDK_DEFINE_STUBBED_EXPORTS = partitionDefineExports(
  diexSdkDefine as unknown as Record<string, unknown>,
);

const VIRTUAL_NAMESPACE = 'diex-sdk-define-stub';
const STUB_RESOLVED_PATH = '__diex-sdk-define-stub__';

const STUB_PRELUDE = `
// Auto-generated stub for diex-sdk/define injected by the SDK CLI build.
// Real implementations would pull in zod, diex-shared and ~1MB of code; at
...
const buildStubModuleSource = (): string => {
  ...
  for (const name of DIEX_SDK_DEFINE_STUBBED_EXPORTS.any) {
...
export const createStubDiexSdkDefinePlugin = (): esbuild.Plugin => ({
  name: 'diex-sdk-define-stub',
  setup(build) {
    build.onResolve({ filter: /^diex-sdk\/define$/ }, () => ({
