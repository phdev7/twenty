...
// Neither Diex SDK package needs to be resolved by the published app at
// runtime, so both must live in "devDependencies":
// - diex-sdk ships the CLI and build/scaffolding tooling used only at dev and
//   build time, and is never imported by the published app's runtime.
// - diex-client-sdk is imported by app code but is provided at runtime by
//   Diex's injected SDK (Lambda SDK layer / server-served modules), so the
...
const BUILD_TIME_DEPENDENCY_WARNINGS: Record<string, string> = {
  'diex-sdk':
    '"diex-sdk" is listed under "dependencies" in package.json. It is a build-time only tool and should be moved to "devDependencies".',
  'diex-client-sdk':
    '"diex-client-sdk" is listed under "dependencies" in package.json. It is provided at runtime by Diex\'s injected SDK and should be moved to "devDependencies".',
};
