...
const APP_ACCESS_TOKEN_ENV_KEY = 'DIEX_APP_ACCESS_TOKEN';
const API_KEY_ENV_KEY = 'DIEX_API_KEY';

type DiexGeneratedClientOptions = ClientOptions;
...
const defaultOptions: DiexGeneratedClientOptions = {
  url: '__DIEX_DEFAULT_URL__',
  headers: {
    'Content-Type': 'application/json',
  },
};

export class DiexGeneratedClient {
  ...
  constructor(options?: DiexGeneratedClientOptions) {
    const merged: DiexGeneratedClientOptions = {
      ...defaultOptions,
      ...options,
    };
    ...
    if (!this.fetchImplementation) {
      throw new Error(
        'Global `fetch` function is not available, ' +
          'pass a fetch implementation to the Diex client',
      );
    }
    ...
    if (!this.refreshAccessTokenPromise) {
      this.refreshAccessTokenPromise = refreshAccessTokenFunction()
        ...
        .catch((refreshError: unknown) => {
          console.error('Diex client: token refresh failed', refreshError);
