...
export type RemoteConfig = {
  apiUrl: string;
  apiKey?: string;
  // CLI OAuth app credentials (from `yarn diex remote:add`)
  diexCLIRegistrationId?: string;
  diexCLIRegistrationClientId?: string;
  diexCLIAccessToken?: string;
  diexCLIRefreshToken?: string;
...
export class ConfigService {
  ...
  // to the current format (remotes, diexCLIAccessToken/diexCLIRefreshToken)
  private async migrateConfigIfNeeded(
    ...
    const migrateRemoteFields = (
      ...
      diexCLIRegistrationClientId:
        str(source.diexCLIRegistrationClientId) ?? str(source.oauthClientId),
      diexCLIAccessToken:
        str(source.diexCLIAccessToken) ??
        str(source.accessToken) ??
        str(source.applicationAccessToken),
      diexCLIRefreshToken:
        str(source.diexCLIRefreshToken) ??
  ...
  async getConfigForRemote(remoteName: string): Promise<RemoteConfig> {
    ...
    try {
      ...
      return {
        ...
        diexCLIRegistrationId: remoteConfig.diexCLIRegistrationId,
        diexCLIRegistrationClientId:
          remoteConfig.diexCLIRegistrationClientId,
        diexCLIAccessToken: remoteConfig.diexCLIAccessToken,
        diexCLIRefreshToken: remoteConfig.diexCLIRefreshToken,
