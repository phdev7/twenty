...
const getAppDeclaredSdkVersion = async (
  ...
  try {
    ...
    return (
      pkg.dependencies?.['diex-sdk'] ??
      pkg.devDependencies?.['diex-sdk'] ??
      undefined
    );
  } catch {
    return undefined;
...
export const checkSdkVersionCompatibility = async (
  ...
  if (cliMajor !== appMajor) {
    console.warn(
      chalk.yellow(
        `⚠ Version mismatch: your app requires diex-sdk@${appMajor}.x ` +
          `but the CLI running is v${cliVersion}. ` +
          `Major version mismatches may cause unexpected behavior.\n` +
          `  Update with: npm install -g diex-sdk@${appMajor}`,
      ),
    );
    console.warn('');
  }
