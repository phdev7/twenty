...
export const checkServerVersionCompatibility = async (
  ...
  console.warn(
    chalk.yellow(
      `⚠ Local Diex server is v${info.localServerVersion} (${info.daysBehind} days behind v${info.latestServerVersion}).`,
    ),
  );
  console.warn(chalk.dim('  Update with: yarn diex docker:upgrade'));
  console.warn('');
};
