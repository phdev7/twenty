import { type CommandMenuContextApi } from 'diex-shared/types';
...
const createExpressionOnlyPlaceholder = (name: string): unknown => {
  const fail = (): never => {
    throw new Error(
      `"${name}" from diex-sdk can only be used inside a command's conditionalAvailabilityExpression, not at runtime.`,
    );
  };
