...
yarn patch-commit <temp-folder> > packages/diex-server/patches/<package+name+version>.patch
yarn install --mode update-lockfile && yarn install
```

Reference the patch in `packages/diex-server/package.json`:
...
yarn patch-commit <CLEAN> > packages/diex-server/patches/<package+name+version>.patch
yarn install --mode update-lockfile && yarn install
```
