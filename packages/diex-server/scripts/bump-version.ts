 ...
 *  |_|   \\_/\\_/ \\___|_| |_|\\__|\\__, | Generated by: npx nx version:bump diex-server
 *                              |___/
 */
...
const getCurrentVersion = (): string => {
  const filePath = `${CONSTANTS_DIR}/diex-current-version.constant.ts`;
...
const getPreviousVersions = (): string[] => {
  const filePath = `${CONSTANTS_DIR}/diex-previous-versions.constant.ts`;
  const content = readFileSync(filePath, 'utf-8');
...
const getNextVersions = (): string[] => {
  const filePath = `${CONSTANTS_DIR}/diex-next-versions.constant.ts`;
  const content = readFileSync(filePath, 'utf-8');
  const matches = content.match(/'[0-9.]+'/g);
...
const updateCurrentVersion = (newVersion: string): void => {
  ...
  writeFileSync(
    filePath,
    `${AUTO_GENERATED_HEADER}export const DIEX_CURRENT_VERSION = '${newVersion}' as const;\n`,
  );

  console.log(`Updated DIEX_CURRENT_VERSION to '${newVersion}'`);
};
...
const updatePreviousVersions = (versions: string[]): void => {
  ...
  writeFileSync(
    filePath,
    `${AUTO_GENERATED_HEADER}export const DIEX_PREVIOUS_VERSIONS = [\n${formattedVersions}\n] as const;\n`,
  );

  console.log(
    `Updated DIEX_PREVIOUS_VERSIONS with ${versions.length} versions`,
  );
};
...
const updateNextVersions = (versions: string[]): void => {
  ...
  writeFileSync(
    filePath,
    `${AUTO_GENERATED_HEADER}export const DIEX_NEXT_VERSIONS = [\n${formattedVersions}\n] as const;\n`,
  );

  console.log(`Updated DIEX_NEXT_VERSIONS with ${versions.length} versions`);
};
...
const resolveNewVersion = ({
  ...
  if (nextVersions.length === 0) {
    ...
    console.log(
      `\nNo version provided and DIEX_NEXT_VERSIONS is empty, computed from current: '${computed}'`,
    );
  ...
  console.log(
    `\nNo version provided, using DIEX_NEXT_VERSIONS[0]: '${nextVersions[0]}'`,
  );
...
const bumpVersion = (newVersionArg?: string): void => {
  ...
  console.log('\nCurrent state:');
  console.log(`  DIEX_CURRENT_VERSION: '${currentVersion}'`);
  console.log(
    `  DIEX_PREVIOUS_VERSIONS: [${previousVersions.map((v) => `'${v}'`).join(', ')}]`,
  );
  console.log(
    `  DIEX_NEXT_VERSIONS: [${nextVersions.map((v) => `'${v}'`).join(', ')}]`,
  );
  ...
  console.log('\nNew state:');
  console.log(`  DIEX_CURRENT_VERSION: '${newVersion}'`);
  console.log(
    `  DIEX_PREVIOUS_VERSIONS: [${updatedPreviousVersions.map((v) => `'${v}'`).join(', ')}]`,
  );
  console.log(
    `  DIEX_NEXT_VERSIONS: [${updatedNextVersions.map((v) => `'${v}'`).join(', ')}]`,
  );
