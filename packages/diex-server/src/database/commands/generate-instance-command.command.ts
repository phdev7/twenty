...
export class GenerateInstanceCommandCommand extends CommandRunner {
  ...
  parseVersion(value: string): DiexAllVersion {
    if (
      !DIEX_ALL_VERSIONS.includes(
        value as (typeof DIEX_ALL_VERSIONS)[number],
      )
    ) {
      throw new Error(
        `Invalid version "${value}". Must be one of: ${DIEX_ALL_VERSIONS.join(', ')}`,
      );
    }

    return value as DiexAllVersion;
  }
  ...
  async run(
    ...
    const version = options.version ?? DIEX_CURRENT_VERSION;
