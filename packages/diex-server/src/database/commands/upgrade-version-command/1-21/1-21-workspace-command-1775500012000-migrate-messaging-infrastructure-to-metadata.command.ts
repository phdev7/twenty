...
export class MigrateMessagingInfrastructureToMetadataCommand extends ProvisionedWorkspaceCommandRunner {
  constructor(
    private readonly diexORMGlobalManager: GlobalWorkspaceOrmManager,
  ...
  override async runOnWorkspace({
    ...
    const connectedAccountWorkspaceRepository =
      await this.diexORMGlobalManager.getRepository<LegacyConnectedAccountWorkspaceEntity>(
        workspaceId,
        'connectedAccount',
      );

    const messageChannelWorkspaceRepository =
      await this.diexORMGlobalManager.getRepository<MessageChannelEntity>(
        workspaceId,
        'messageChannel',
      );

    const calendarChannelWorkspaceRepository =
      await this.diexORMGlobalManager.getRepository<CalendarChannelEntity>(
        workspaceId,
        'calendarChannel',
      );

    const messageFolderWorkspaceRepository =
      await this.diexORMGlobalManager.getRepository<MessageFolderEntity>(
        workspaceId,
        'messageFolder',
      );
  ...
  private async buildWorkspaceMemberIdToUserWorkspaceIdMap(
    ...
    const workspaceMemberRepository =
      await this.diexORMGlobalManager.getRepository<WorkspaceMemberWorkspaceEntity>(
        workspaceId,
        'workspaceMember',
