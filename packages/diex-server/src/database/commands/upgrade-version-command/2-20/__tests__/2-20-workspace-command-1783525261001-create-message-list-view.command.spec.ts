...
jest.mock(
  'src/engine/workspace-manager/diex-standard-application/utils/diex-standard-application-all-flat-entity-maps.constant',
);

const computeDiexStandardApplicationAllFlatEntityMapsMock =
  computeDiexStandardApplicationAllFlatEntityMaps as jest.Mock;
...
describe('CreateMessageListViewCommand', () => {
  ...
  beforeEach(() => {
    ...
    findApplicationMock = jest.fn().mockResolvedValue({
      diexStandardFlatApplication: STANDARD_APPLICATION,
    });
    ...
    computeDiexStandardApplicationAllFlatEntityMapsMock.mockReturnValue({
      allFlatEntityMaps: {
    ...
    command = new CreateMessageListViewCommand(
      ...
      {
        findWorkspaceDiexStandardAndCustomApplicationOrThrow:
          findApplicationMock,
