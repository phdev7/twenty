...
export const adminPanelModuleFactory = async (
  ...
  const config: YogaDriverConfig = {
    ...
    plugins: [
      ...
      useDisableIntrospectionAndSuggestionsForUnauthenticatedUsers(
        diexConfigService.get('NODE_ENV') === NodeEnvironment.PRODUCTION,
      ),
      useValidateGraphqlQueryComplexity({
        maximumAllowedFields: diexConfigService.get('GRAPHQL_MAX_FIELDS'),
  ...
  if (diexConfigService.get('NODE_ENV') === NodeEnvironment.DEVELOPMENT) {
