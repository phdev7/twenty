...
> {
  ...
  createGqlOptions(): YogaDriverConfig {
    const isDebugMode =
      this.diexConfigService.get('NODE_ENV') === NodeEnvironment.DEVELOPMENT;
    const plugins = [
      useDirectExecution({
      ...
      useGraphQLErrorHandlerHook({
        ...
        diexConfigService: this.diexConfigService,
      }),
      useDisableIntrospectionAndSuggestionsForUnauthenticatedUsers(
        this.diexConfigService.get('NODE_ENV') === NodeEnvironment.PRODUCTION,
      ),
      useValidateGraphqlQueryComplexity({
        maximumAllowedFields:
          this.diexConfigService.get('GRAPHQL_MAX_FIELDS'),
        maximumAllowedRootResolvers: this.diexConfigService.get(
