...
import { CONFIG_VARIABLES_GROUP_METADATA } from 'src/engine/core-modules/diex-config/constants/config-variables-group-metadata';
...
export class AdminPanelConfigService {
  ...
  getConfigVariablesGrouped(): ConfigVariablesDTO {
    const rawEnvVars = this.diexConfigService.getAll();
  ...
  getConfigVariable(key: string): ConfigVariableDTO {
    const variableWithMetadata =
      this.diexConfigService.getVariableWithMetadata(
