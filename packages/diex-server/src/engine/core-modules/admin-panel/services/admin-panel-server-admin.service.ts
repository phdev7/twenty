...
import { ServerAdminAccessChangedEmail } from 'diex-emails';
...
export class AdminPanelServerAdminService {
  ...
  private async notifyAdministrators({
    ...
    try {
      ...
      const from = `${this.diexConfigService.get('EMAIL_FROM_NAME')} <${this.diexConfigService.get('EMAIL_FROM_ADDRESS')}>`;
