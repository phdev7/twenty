...
// External registry identifier: it does not follow the twenty->diex repository
// rename, because the published package keeps its name.
...
export class AdminPanelVersionService {
  ...
  async getVersionInfo(): Promise<VersionInfoDTO> {
    const currentVersion = this.diexConfigService.get('APP_VERSION');

    try {
      const httpClient = this.secureHttpClientService.getHttpClient();

      // Compare against the Diex registry, not upstream Diex: the release
      ...
      // that the product is a Diex fork.
