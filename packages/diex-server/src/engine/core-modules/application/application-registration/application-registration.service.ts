...
import { DIEX_CLI_APPLICATION_REGISTRATION } from 'src/engine/workspace-manager/diex-standard-application/constants/diex-cli-application-registration.constant';
...
export class ApplicationRegistrationService {
  ...
  async createCliRegistrationIfNotExists(): Promise<ApplicationRegistrationEntity | null> {
    const existing = await this.findOneByUniversalIdentifier(
      DIEX_CLI_APPLICATION_REGISTRATION.universalIdentifier,
    );
    ...
    const registration = this.applicationRegistrationRepository.create({
      universalIdentifier:
        DIEX_CLI_APPLICATION_REGISTRATION.universalIdentifier,
      name: DIEX_CLI_APPLICATION_REGISTRATION.name,
      oAuthClientId: v4(),
      oAuthClientSecretHash: null,
      oAuthRedirectUris: [],
      oAuthScopes: DIEX_CLI_APPLICATION_REGISTRATION.oAuthScopes,
      ownerWorkspaceId: null,
