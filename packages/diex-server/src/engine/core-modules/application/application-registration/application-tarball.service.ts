...
import type { ApplicationManifest, Manifest } from 'diex-shared/application';
...
export class ApplicationTarballService {
  ...
  async uploadTarball(params: {
    ...
    const tempDir = join(tmpdir(), 'diex-tarball-upload', v4());
  ...
  private async extractAndValidateTarball(
    ...
    packageJson: { version: string; engines?: { diex?: string } } | null;
  }> {
