...
export class ApplicationService {
  ...
  async findWorkspaceDiexStandardAndCustomApplicationOrThrow({
    ...
    const diexStandardFlatApplication =
      flatApplicationMaps.byId[diexStandardApplicationId];
    const workspaceCustomFlatApplication =
      flatApplicationMaps.byId[workspace.workspaceCustomApplicationId];

    if (
      !isDefined(diexStandardFlatApplication) ||
  ...
  async findDiexStandardApplicationOrThrow(workspaceId: string): Promise<{
    ...
    const { diexStandardFlatApplication } =
      await this.findWorkspaceDiexStandardAndCustomApplicationOrThrow({
        workspace,
      });

    const application = await this.applicationRepository.findOne({
      where: {
        id: diexStandardFlatApplication.id,
  ...
  async createDiexStandardApplication(
    {
      workspaceId,
    ...
    const existingApplication = await this.findByUniversalIdentifier({
      universalIdentifier: DIEX_STANDARD_APPLICATION.universalIdentifier,
      workspaceId,
    });
    ...
    const diexStandardApplication = await this.create(
      {
        ...DIEX_STANDARD_APPLICATION,
    ...
    return diexStandardApplication;
  }
