...
import { SendApprovedAccessDomainValidation } from 'diex-emails';
import { FileFolder, SettingsPath } from 'diex-shared/types';
...
export class ApprovedAccessDomainService {
  ...
  async sendApprovedAccessDomainValidationEmail(
    ...
    await this.emailService.send({
      from: `${sender.name.firstName} ${sender.name.lastName} (via Diex) <${this.diexConfigService.get('EMAIL_FROM_ADDRESS')}>`,
      to,
