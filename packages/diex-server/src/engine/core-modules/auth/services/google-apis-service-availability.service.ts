...
export class GoogleApisServiceAvailabilityService {
  ...
  async checkServicesAvailability(
    ...
    const oAuth2Client = new google.auth.OAuth2({
      clientId: this.diexConfigService.get('AUTH_GOOGLE_CLIENT_ID'),
      clientSecret: this.diexConfigService.get('AUTH_GOOGLE_CLIENT_SECRET'),
  ...
  private async checkMessagingAvailability(
    ...
    if (!this.diexConfigService.get('MESSAGING_PROVIDER_GMAIL_ENABLED')) {
      return false;
    }
  ...
  private async checkCalendarAvailability(
    ...
    if (!this.diexConfigService.get('CALENDAR_PROVIDER_GOOGLE_ENABLED')) {
      return false;
    }
