...
import { DIEX_ICONS_BASE_URL } from 'diex-shared/constants';
...
export class SignInUpService {
      ...
      [
        {
          ...
          serverId: this.diexConfigService.get('SERVER_ID'),
        },
      ],
      undefined,
    );
  ...
  private async assertWorkspaceCreationAllowed(
    ...
    if (this.diexConfigService.get('IS_WORKSPACE_APPROVAL_REQUIRED')) {
      return;
    }
  ...
  private async assertWorkspaceCountWithinLimit(
    ...
    const maxWorkspaces = this.diexConfigService.get(
    ...
    try {
      const { user, workspace } = await this.dataSource.transaction(
        async (entityManager) => {
          ...
          if (isWorkEmailFound) {
            const logoUrl = `${DIEX_ICONS_BASE_URL}/${getDomainFromEmailOrThrow(email)}`;
            const logoFile =
          ...
          await this.onboardingService.setOnboardingDiexWorkspacePending(
            {
              userId: user.id,
              workspaceId: workspace.id,
              value: true,
