...
const CONFIG: Record<string, unknown> = {
  ...
  EMAIL_FROM_NAME: 'Diex',
  EMAIL_FROM_ADDRESS: 'noreply@diex.com',
};
...
const buildService = ({
  ...
  const diexConfigService = { get: (key: string) => CONFIG[key] };
  const workspaceDomainsService = {
    buildWorkspaceURL: jest.fn(
      () => new URL('https://acme.diex.com/settings/billing'),
    ),
  };

  const service = new BillingReminderService(
    // oxlint-disable-next-line typescript/no-explicit-any
    diexConfigService as any,
    // oxlint-disable-next-line typescript/no-explicit-any
  ...
  it('sends the add-a-card email the day before a no-credit-card trial ends', async () => {
    ...
    expect(BillingTrialEndingEmail).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        link: 'https://acme.diex.com/settings/billing',
      }),
    );
