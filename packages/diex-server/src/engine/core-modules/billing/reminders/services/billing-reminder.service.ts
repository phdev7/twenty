...
export class BillingReminderService {
  ...
  private async processTrialReminders(now: Date): Promise<void> {
    const withoutCardDaysBefore = this.diexConfigService.get(
      'BILLING_TRIAL_WITHOUT_CREDIT_CARD_REMINDER_DAYS_BEFORE',
    );
    const withCardDaysBefore = this.diexConfigService.get(
  ...
  private async processRenewalReminders(now: Date): Promise<void> {
    const renewalDaysBefore = this.diexConfigService.get(
  ...
  private isWithCreditCardTrial(
    ...
    const withoutCardTrialDurationDays = this.diexConfigService.get(
  ...
  private async sendReminderEmail({
    ...
    await this.emailService.send({
      ...
      from: `${this.diexConfigService.get(
        'EMAIL_FROM_NAME',
      )} <${this.diexConfigService.get('EMAIL_FROM_ADDRESS')}>`,
  ...
  private buildReminderEmail({
    ...
    switch (reminder.type) {
      case 'trial-ending':
        return {
          subject: msg`Your Diex trial is ending soon`,
          emailTemplate: BillingTrialEndingEmail({
            userName,
            workspaceDisplayName,
            trialEndsAt: reminder.trialEndsAt,
            dataRetentionDays: this.diexConfigService.get(
      ...
      case 'trial-converting':
        return {
          subject: msg`A heads up before your Diex trial ends`,
      ...
      case 'subscription-renewing':
        return {
          subject: msg`Your Diex plan renews soon`,
