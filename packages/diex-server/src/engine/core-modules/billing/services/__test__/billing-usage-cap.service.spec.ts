...
describe('BillingUsageCapService', () => {
  ...
  let diexConfigService: jest.Mocked<DiexConfigService>;
  ...
  beforeEach(async () => {
    ...
    diexConfigService = module.get(DiexConfigService);
  });
  ...
  describe('isClickHouseEnabled', () => {
    it('returns true when CLICKHOUSE_URL is configured', () => {
      diexConfigService.get.mockReturnValue('http://clickhouse:8123');

      expect(service.isClickHouseEnabled()).toBe(true);
    });

    it('returns false when CLICKHOUSE_URL is empty', () => {
      diexConfigService.get.mockReturnValue('');
