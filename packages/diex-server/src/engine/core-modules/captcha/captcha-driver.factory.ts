...
export class CaptchaDriverFactory extends DriverFactoryBase<CaptchaDriver | null> {
  ...
  protected buildConfigKey(): string {
    const driver = this.diexConfigService.get('CAPTCHA_DRIVER');
  ...
  protected createDriver(): CaptchaDriver | null {
    const driver = this.diexConfigService.get('CAPTCHA_DRIVER');
    const siteKey = this.diexConfigService.get('CAPTCHA_SITE_KEY');
    const secretKey = this.diexConfigService.get('CAPTCHA_SECRET_KEY');
