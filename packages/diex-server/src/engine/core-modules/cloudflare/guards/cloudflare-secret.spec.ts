...
describe('CloudflareSecretMatchGuard.canActivate', () => {
  ...
  beforeEach(() => {
    diexConfigService = {
      get: jest.fn(),
    } as unknown as DiexConfigService;
    guard = new CloudflareSecretMatchGuard(diexConfigService);
  });

  it('should return true when the webhook secret matches', () => {
    jest.spyOn(diexConfigService, 'get').mockReturnValue('valid-secret');
  ...
  it('should throw InternalServerErrorException when env is not set', () => {
    jest.spyOn(diexConfigService, 'get').mockReturnValue(undefined);
  ...
  it('should return false when header length differs from secret', () => {
    jest.spyOn(diexConfigService, 'get').mockReturnValue('short');
