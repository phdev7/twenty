...
export class DiexAgencyResolver {
  ...
  @Mutation(() => DiexAgencyEntity)
  async updateDiexAgencyStatus(
    @Args('input') input: UpdateAgencyStatusInput,
    @AuthUser() user: AuthContextUser,
  ): Promise<DiexAgencyEntity> {
    if (!user.canAccessFullAdminPanel) {
      throw new ForbiddenException(
        'Apenas Administradores podem suspender ou reativar agências.',
      );
    }
    return await this.diexAgencyService.updateAgencyStatus(input);
  }
