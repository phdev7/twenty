...
import { type AwsRegion } from 'src/engine/core-modules/diex-config/interfaces/aws-region.interface';
...
import { CastToLogLevelArray } from 'src/engine/core-modules/diex-config/decorators/cast-to-log-level-array.decorator';
import { CastToMeterDriverArray } from 'src/engine/core-modules/diex-config/decorators/cast-to-meter-driver.decorator';
import { CastToPositiveNumber } from 'src/engine/core-modules/diex-config/decorators/cast-to-positive-number.decorator';
import { CastToTypeORMLogLevelArray } from 'src/engine/core-modules/diex-config/decorators/cast-to-typeorm-log-level-array.decorator';
import { CastToUpperSnakeCase } from 'src/engine/core-modules/diex-config/decorators/cast-to-upper-snake-case.decorator';
...
import { ConfigVariablesMetadata } from 'src/engine/core-modules/diex-config/decorators/config-variables-metadata.decorator';
import { IsAWSRegion } from 'src/engine/core-modules/diex-config/decorators/is-aws-region.decorator';
import { IsDuration } from 'src/engine/core-modules/diex-config/decorators/is-duration.decorator';
import { IsOptionalOrEmptyString } from 'src/engine/core-modules/diex-config/decorators/is-optional-or-empty-string.decorator';
import { IsStrictlyLowerThan } from 'src/engine/core-modules/diex-config/decorators/is-strictly-lower-than.decorator';
import { IsDiexSemVer } from 'src/engine/core-modules/diex-config/decorators/is-diex-semver.decorator';
import { ConfigVariableType } from 'src/engine/core-modules/diex-config/enums/config-variable-type.enum';
import { ConfigVariablesGroup } from 'src/engine/core-modules/diex-config/enums/config-variables-group.enum';
import {
  ConfigVariableException,
  ConfigVariableExceptionCode,
} from 'src/engine/core-modules/diex-config/diex-config.exception';
...
export class ConfigVariables {
  ...
  @ConfigVariablesMetadata({
    group: ConfigVariablesGroup.META_ADS_CONFIG,
    description:
      'Enable the Meta Ads OAuth connection used by agency traffic dashboards',
    type: ConfigVariableType.BOOLEAN,
  })
  @IsOptional()
  META_ADS_OAUTH_ENABLED = false;

  @ConfigVariablesMetadata({
    group: ConfigVariablesGroup.META_ADS_CONFIG,
    isSensitive: false,
    description: 'App ID of the Meta app used to connect ad accounts',
    type: ConfigVariableType.STRING,
  })
  @ValidateIf((env) => env.META_ADS_OAUTH_ENABLED)
  META_ADS_APP_ID: string;

  @ConfigVariablesMetadata({
    group: ConfigVariablesGroup.META_ADS_CONFIG,
    isSensitive: true,
    description: 'App secret of the Meta app used to connect ad accounts',
    type: ConfigVariableType.STRING,
  })
  @ValidateIf((env) => env.META_ADS_OAUTH_ENABLED)
  ...
  @ConfigVariablesMetadata({
    group: ConfigVariablesGroup.ADVANCED_SETTINGS,
    description:
      "Enable or disable requests to diex-icons to get companies' icons",
    type: ConfigVariableType.BOOLEAN,
  })
  ALLOW_REQUESTS_TO_DIEX_ICONS = true;
  ...
  // diex-telemetry.com on every user signup. This fork sends nothing to the
  ...
  @CastToLogLevelArray()
  @IsOptional()
  LOG_LEVELS: DiexLogLevel[] = ['log', 'error', 'warn', 'performance'];
  ...
  // diex.com, not by the key itself, so a version the vendor no longer serves
  // can never validate no matter what key is configured. Diex was told by
  // Diex that this fork's version is discontinued and that it may enable the
  // features itself. This flag is that decision, kept explicit and reversible
  // rather than hidden in a patched conditional.
  @ConfigVariablesMetadata({
    group: ConfigVariablesGroup.SERVER_CONFIG,
    description:
      'Treat the enterprise plan as valid without a validity token from diex.com. Set to false to go back to upstream behaviour.',
    type: ConfigVariableType.BOOLEAN,
  })
  @IsOptional()
  ...
  @ConfigVariablesMetadata({
    group: ConfigVariablesGroup.SERVER_CONFIG,
    description: 'Base URL for the Enterprise API on diex.com',
    isHiddenInAdminPanel: true,
    type: ConfigVariableType.STRING,
  })
  @IsOptional()
  ENTERPRISE_API_URL: string = 'https://diex.com/api/enterprise';
  ...
  @ConfigVariablesMetadata({
    group: ConfigVariablesGroup.SERVER_CONFIG,
    description: 'Diex server version',
    type: ConfigVariableType.STRING,
    isEnvOnly: true,
    isHiddenInAdminPanel: true,
  })
