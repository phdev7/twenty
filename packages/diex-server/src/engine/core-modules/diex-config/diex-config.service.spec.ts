...
import { type ConfigVariables } from 'src/engine/core-modules/diex-config/config-variables';
...
jest.mock(
  'src/engine/core-modules/diex-config/constants/config-variables-masking-config',
  () => ({
...
jest.mock(
  'src/engine/core-modules/diex-config/utils/is-env-only-config-var.util',
  () => ({
    isEnvOnlyConfigVar: jest.fn(),
  }),
);

type DiexConfigServicePrivateProps = {
...
const setupTestModule = async (isDatabaseConfigEnabled = true) => {
  ...
  return {
    service: module.get<DiexConfigService>(DiexConfigService),
...
const setPrivateProps = (
  service: DiexConfigService,
  props: Partial<DiexConfigServicePrivateProps>,
) => {
...
describe('DiexConfigService', () => {
  let service: DiexConfigService;
