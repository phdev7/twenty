...
describe('DnsManagerService', () => {
  ...
  it('should initialize cloudflareClient when CLOUDFLARE_API_KEY is defined', () => {
    ...
    jest.spyOn(diexConfigService, 'get').mockReturnValue(mockApiKey);

    const instance = new DnsManagerService(diexConfigService, {} as any);

    expect(diexConfigService.get).toHaveBeenCalledWith('CLOUDFLARE_API_KEY');
  ...
  describe('registerHostname', () => {
    ...
    it('should register a custom domain successfully', async () => {
      ...
      jest.spyOn(diexConfigService, 'get').mockReturnValue('test-zone-id');
