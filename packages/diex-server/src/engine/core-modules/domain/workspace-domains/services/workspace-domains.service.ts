...
export class WorkspaceDomainsService {
  ...
  private async getDefaultWorkspace() {
    if (this.diexConfigService.get('IS_MULTIWORKSPACE_ENABLED')) {
      throw new Error(
  ...
  private getDiexWorkspaceUrl(subdomain: string) {
    const url = this.domainServerConfigService.getFrontUrl();

    url.hostname = this.diexConfigService.get('IS_MULTIWORKSPACE_ENABLED')
  ...
  getSubdomainAndCustomDomainFromWorkspaceFallbackOnDefaultSubdomain(
    ...
    if (!workspace) {
      return {
        subdomain: this.diexConfigService.get('DEFAULT_SUBDOMAIN'),
        customDomain: null,
    ...
    return {
      customUrl:
        isCustomDomainEnabled && customDomain
          ...
          : undefined,
      subdomainUrl: this.getDiexWorkspaceUrl(subdomain),
    };
  }
