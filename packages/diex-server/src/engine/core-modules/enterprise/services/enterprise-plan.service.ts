...
import { ENTERPRISE_INSTANCE_TYPE } from 'diex-shared/constants';
...
export class EnterprisePlanService implements OnModuleInit {
  ...
  private refreshKeyPayload(): void {
    const enterpriseKey = this.diexConfigService.get('ENTERPRISE_KEY');
  ...
  private async loadValidityToken(): Promise<void> {
    try {
      ...
      const tokenValue =
        dbToken?.value ??
        this.diexConfigService.get('ENTERPRISE_VALIDITY_TOKEN');
  ...
  async setEnterpriseKey(enterpriseKey: string): Promise<void> {
    try {
      await this.diexConfigService.set('ENTERPRISE_KEY', enterpriseKey);
    } catch (error) {
      if (
  ...
  async refreshValidityToken(): Promise<boolean> {
    ...
    // A self-activated instance is not asking diex.com for entitlement, so it
    ...
    if (this.isSelfActivated()) {
      this.logger.log(
        'Enterprise plan is self-activated, skipping refresh against diex.com',
      );
    ...
    const apiUrl = this.diexConfigService.get('ENTERPRISE_API_URL');
  ...
  async getOrCreateServerId(): Promise<string | null> {
    const existingServerId = this.diexConfigService.get('SERVER_ID');
    ...
    try {
      await this.diexConfigService.set('SERVER_ID', newServerId);
  ...
  private async gatherInstanceMetadata(): Promise<EnterpriseInstanceMetadata> {
    return {
      ...
      serverUrl: this.diexConfigService.get('SERVER_URL') ?? null,
      appVersion: this.diexConfigService.get('APP_VERSION') ?? null,
      nodeEnv: this.diexConfigService.get('NODE_ENV') ?? null,
      telemetryEnabled:
        this.diexConfigService.get('TELEMETRY_ENABLED') ?? null,
  ...
  private getPublicKeysToTry(): string[] {
    const nodeEnv = this.diexConfigService.get('NODE_ENV');
