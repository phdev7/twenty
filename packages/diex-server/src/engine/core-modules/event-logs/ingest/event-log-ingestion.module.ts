...
const eventSinksProvider = {
  ...
  useFactory: (
    ...
    const configuredSinkNames = diexConfigService.get('EVENT_SINKS');
    ...
    return getAvailableSinkNames(configuredSinkNames, {
      hasClickhouseUrl: Boolean(diexConfigService.get('CLICKHOUSE_URL')),
    })
      .map((name) => sinkByName[name.toLowerCase()])
      .filter(isDefined);
  },
  inject: [DiexConfigService, ClickHouseEventSink, ConsoleEventSink],
};
