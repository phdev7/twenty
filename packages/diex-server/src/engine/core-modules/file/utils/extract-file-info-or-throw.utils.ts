...
import { DIEX_MIME_POLICY } from 'src/engine/core-modules/file/constants/diex-mime-policy.constant';
...
export const extractFileInfoOrThrow = async ({
  ...
  if (isNonEmptyString(ext)) {
    // Diex policy wins over the ext-based fallback for the (small) set of
    ...
    // source upload.
    const policyMime = DIEX_MIME_POLICY[ext];
