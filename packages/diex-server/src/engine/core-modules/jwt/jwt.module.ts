...
const InternalJwtModule = NestJwtModule.registerAsync({
  useFactory: async (diexConfigService: DiexConfigService) => {
    return {
      secret: diexConfigService.get('APP_SECRET'),
      signOptions: {
        algorithm: JWT_LEGACY_ALGORITHM,
        expiresIn: diexConfigService.get('ACCESS_TOKEN_EXPIRES_IN'),
      },
      verifyOptions: {
