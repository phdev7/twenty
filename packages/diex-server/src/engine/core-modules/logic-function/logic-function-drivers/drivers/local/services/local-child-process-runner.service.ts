...
export class LocalChildProcessRunnerService {
  // Symlinks everything from the deps layer except diex-client-sdk,
  ...
  async assembleNodeModules({
    ...
    const symlinkPromises = entries
      .filter((entry) => entry.name !== 'diex-client-sdk')
      .map((entry) =>
        fs.symlink(
    ...
    await fs.symlink(
      join(sdkNodeModules, 'diex-client-sdk'),
      join(execNodeModules, 'diex-client-sdk'),
      'dir',
    );
  }
