...
export class LogicFunctionExecutorService {
  ...
  private async throttleExecution(workspaceId: string) {
    try {
      await this.throttlerService.tokenBucketThrottleOrThrow(
        ...
        1,
        this.diexConfigService.get('LOGIC_FUNCTION_EXEC_THROTTLE_LIMIT'),
        this.diexConfigService.get('LOGIC_FUNCTION_EXEC_THROTTLE_TTL'),
      );
    } catch {
  ...
  private async getExecutionEnvVariables({
    ...
    const baseUrl = cleanServerUrl(
      this.diexConfigService.get('SERVER_INTERNAL_URL') ||
