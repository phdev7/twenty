...
describe('SdkClientGenerationService', () => {
  ...
  beforeEach(async () => {
    applicationService = {
      findWorkspaceDiexStandardAndCustomApplicationOrThrow: jest.fn(),
    };
    messageQueueService = {
  ...
  describe('enqueueSdkClientGenerationForWorkspace', () => {
    ...
    const apps = {
      diexStandardFlatApplication: {
        id: 'std-app-id',
        universalIdentifier: 'diex-standard',
      },
    ...
    it('enqueues one job per application with dedup id and retry limit', async () => {
      applicationService.findWorkspaceDiexStandardAndCustomApplicationOrThrow.mockResolvedValue(
        apps as never,
      );
      ...
      expect(messageQueueService.add).toHaveBeenNthCalledWith(
        ...
        GENERATE_SDK_CLIENT_JOB_NAME,
        {
          ...
          applicationUniversalIdentifier: 'diex-standard',
