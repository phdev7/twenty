...
import { replaceCoreClient } from 'diex-client-sdk/generate';
...
const SDK_CLIENT_ARCHIVE_NAME = 'diex-client-sdk.zip';
...
export class SdkClientGenerationService {
  ...
  async enqueueSdkClientGenerationForWorkspace(
    ...
    const { diexStandardFlatApplication, workspaceCustomFlatApplication } =
      await this.applicationService.findWorkspaceDiexStandardAndCustomApplicationOrThrow(
        { workspaceId },
      );

    await Promise.all(
      [diexStandardFlatApplication, workspaceCustomFlatApplication].map(
        (application) =>
  ...
  private async generateAndStore({
    ...
    try {
      ...
      const tempPackageRoot = join(sourceTemporaryDir, 'diex-client-sdk');
