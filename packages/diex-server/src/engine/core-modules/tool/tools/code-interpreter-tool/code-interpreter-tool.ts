...
import { DIEX_MCP_HELPER } from 'src/engine/core-modules/tool/tools/code-interpreter-tool/diex-mcp-helper.const';
import {
...
export class CodeInterpreterTool implements Tool {
  ...
  async execute(
    ...
    try {
      ...
      const codeWithHelper = DIEX_MCP_HELPER + '\n\n' + code;
      ...
      const result = await this.codeInterpreterService.execute(
        ...
        {
          env: {
            DIEX_SERVER_URL: serverUrl,
            DIEX_API_TOKEN: sessionToken,
          },
