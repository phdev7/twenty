...
export const DIEX_MCP_HELPER = `# Auto-injected Diex MCP helper - provides access to Diex tools
import os
import json
...
class DiexMCP:
    """Helper for calling Diex tools from sandboxed code.
    ...
    def __init__(self):
        self.url = os.environ.get('DIEX_SERVER_URL', '')
        self.token = os.environ.get('DIEX_API_TOKEN', '')
    ...
    def call_tool(self, name: str, arguments: dict = None):
        ...
        Example:
            companies = diex.call_tool('find_many_companies', {'limit': 5})
            # companies == {'records': [...], 'count': '5'}
        """
        if not self._available:
            raise RuntimeError('Diex MCP bridge not available. Missing requests library or credentials.')
    ...
    def bulk_upsert(self, plural: str, records: list, batch_size: int = 200):
        ...
        Example:
            summary = diex.bulk_upsert('people', people_rows)
        """
...
# --------------------------------------------------------------------------
# \`diex\` is a pre-built instance of the DiexMCP class above. It is
# already bound in this module scope — DO NOT \`import diex\`. There is
# no Python package by that name. Just use it directly, e.g.:
#     companies = diex.call_tool('find_many_companies', {'limit': 10})
# --------------------------------------------------------------------------
diex = DiexMCP()
`;
