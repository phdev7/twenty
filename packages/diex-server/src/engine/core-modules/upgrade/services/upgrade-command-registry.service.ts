...
import {
  DIEX_ALL_VERSIONS,
  type DiexAllVersion,
} from 'src/engine/core-modules/upgrade/constants/diex-all-versions.constant';
import { DIEX_CROSS_UPGRADE_SUPPORTED_VERSIONS } from 'src/engine/core-modules/upgrade/constants/diex-cross-upgrade-supported-version.constant';
...
export class UpgradeCommandRegistryService implements OnModuleInit {
  ...
  onModuleInit(): void {
    for (const version of DIEX_ALL_VERSIONS) {
    ...
    for (const [version, bundle] of this.bundlesByVersion) {
      ...
      if (totalCount > 0) {
        const crossUpgradeLabel = (
          DIEX_CROSS_UPGRADE_SUPPORTED_VERSIONS as readonly string[]
        ).includes(version)
  ...
  getBundleForVersion(version: DiexAllVersion): VersionBundle {
  ...
  getCrossUpgradeSupportedFastInstanceCommands(): RegisteredFastInstanceCommand[] {
    return DIEX_CROSS_UPGRADE_SUPPORTED_VERSIONS.flatMap(
  ...
  private validateAtLeastOneVersionBundleHasWorkspaceCommands(): void {
    ...
    for (const version of DIEX_CROSS_UPGRADE_SUPPORTED_VERSIONS) {
      const bundle = this.getBundleForVersion(version);
  ...
  private validateNoVersionDuplicatesAcrossConstants(): void {
    ...
    if (uniqueVersions.size !== allVersions.length) {
      ...
      throw new Error(
        `Duplicate version(s) across DIEX_PREVIOUS_VERSIONS, DIEX_CURRENT_VERSION, and DIEX_NEXT_VERSIONS: ${duplicates.join(', ')}`,
      );
    }
  }

  private validatePreviousVersionsNotEmpty(): void {
    if ((DIEX_PREVIOUS_VERSIONS as readonly string[]).length === 0) {
      throw new Error(
        'DIEX_PREVIOUS_VERSIONS must contain at least one version before DIEX_CURRENT_VERSION',
      );
    }
  }
}
