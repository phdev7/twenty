...
export class UserResolver {
  ...
  async userVars(
    ...
    const userVarAllowList: string[] = [
      ...
      OnboardingStepKeys.ONBOARDING_DIEX_WORKSPACE_PENDING,
  ...
  supportUserHash(@Parent() parent: UserEntity): string | null {
    if (
      this.diexConfigService.get('SUPPORT_DRIVER') !== SupportDriver.FRONT
    ) {
      return null;
    }
    const key = this.diexConfigService.get('SUPPORT_FRONT_HMAC_KEY');
