...
import ms from 'ms';
import { SendInviteLinkEmail } from 'diex-emails';
import { AppPath, FileFolder } from 'diex-shared/types';
...
export class WorkspaceInvitationService {
  constructor(
    ...
    // eslint-disable-next-line diex/prefer-workspace-scoped-repository -- This durable state lives in core; every access is pinned to workspaceId and immutable state/token IDs.
  ...
  private async createInvitationTokenWithManager({
    ...
    const expiresIn = this.diexConfigService.get(
  ...
  private async sendInvitationEmail({
    ...
    const emailTemplate = SendInviteLinkEmail({
      ...
      serverUrl: this.diexConfigService.get('SERVER_URL'),
      locale: sender.locale,
    });
    const html = await render(emailTemplate);
    const text = await render(emailTemplate, { plainText: true });
    await this.emailService.send({
      from: `${sender.name.firstName} ${sender.name.lastName} (via Diex CRM) <${this.diexConfigService.get('EMAIL_FROM_ADDRESS')}>`,
      to: recipientEmail,
  ...
  private async throwIfOnboardingInvitationLimitReached(
    ...
    const maxOnboardingInvitations = this.diexConfigService.get(
