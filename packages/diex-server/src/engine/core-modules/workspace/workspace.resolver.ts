...
import { FeatureFlagKey, FileFolder } from 'diex-shared/types';
...
export class WorkspaceResolver {
  ...
  async billingSubscriptions(
    ...
    if (!this.diexConfigService.isBillingEnabled()) {
      return [];
    }
  ...
  // narrower "did diex.com issue us a token". Otherwise a self-activated
  ...
  isGoogleAuthEnabled(@Parent() workspace: WorkspaceEntity) {
    return (
      ...
      this.diexConfigService.get('AUTH_GOOGLE_ENABLED')
    );
  }
  ...
  isMicrosoftAuthEnabled(@Parent() workspace: WorkspaceEntity) {
    return (
      ...
      this.diexConfigService.get('AUTH_MICROSOFT_ENABLED')
    );
  }
  ...
  isPasswordAuthEnabled(@Parent() workspace: WorkspaceEntity) {
    return (
      ...
      this.diexConfigService.get('AUTH_PASSWORD_ENABLED')
    );
  }
