...
import { type APP_LOCALES, SOURCE_LOCALE } from 'diex-shared/translations';
...
import { belongsToDiexStandardApp } from 'src/engine/metadata-modules/utils/belongs-to-diex-standard-app.util';
import { getDiexStandardApplicationIdOrThrow } from 'src/engine/metadata-modules/utils/get-diex-standard-application-id-or-throw.util';
            ...
            const overriddenFieldMetadataEntities =
              objectFlatFieldMetadatas.map((flatFieldMetadata) => {
                ...
                const i18nContext = {
                  locale,
                  i18nInstance,
                  isStandardApp: belongsToDiexStandardApp(flatFieldMetadata),
                  applicationCatalog,
                };
    ...
    return new DataLoader<StandardApplicationIdLoaderPayload, string>(
      async (params: StandardApplicationIdLoaderPayload[]) => {
        ...
        const standardApplicationId =
          getDiexStandardApplicationIdOrThrow(flatApplicationMaps);
