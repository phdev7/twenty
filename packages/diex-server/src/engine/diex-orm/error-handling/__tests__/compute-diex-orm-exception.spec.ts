...
describe('computeDiexORMException', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });
  ...
  it('should return an INVALID_INPUT exception with a missing-field message when error is a NOT_NULL_VIOLATION', async () => {
    ...
    const result = await computeDiexORMException(error);

    expect(result).toBeInstanceOf(DiexORMException);
    expect((result as DiexORMException).code).toBe(
      DiexORMExceptionCode.INVALID_INPUT,
    );
    expect((result as DiexORMException).userFriendlyMessage).toEqual(
  ...
  it('should preserve the original message when surfacing a constraint violation as INVALID_INPUT', async () => {
    ...
    expect((result as DiexORMException).message).toBe(
      'null value in column "name" violates not-null constraint',
    );
  });
  ...
  it('should throw the generic PostgresException when error is a CHECK_VIOLATION (handled by the catch-all)', async () => {
    ...
    await expect(computeDiexORMException(error)).rejects.toThrow(
      PostgresException,
    );
    await expect(computeDiexORMException(error)).rejects.toMatchObject({
  ...
  it('should return a QUERY_READ_TIMEOUT exception when error message mentions a query read timeout', async () => {
    ...
    expect(result).toBeInstanceOf(DiexORMException);
    expect((result as DiexORMException).code).toBe(
      DiexORMExceptionCode.QUERY_READ_TIMEOUT,
    );
  });
  ...
  it('should delegate to handleDuplicateKeyError when error is a UNIQUE_VIOLATION and metadata context is provided', async () => {
    ...
    const duplicateException = new DiexORMException(
    ...
    const result = await computeDiexORMException(
      error,
      objectMetadata,
      entityManager,
      internalContext,
  ...
  it('should rethrow the original error when error is a QueryFailedError with an unknown code', async () => {
    ...
    await expect(computeDiexORMException(error)).rejects.toBe(error);
  });
