...
import { RELATION_NESTED_QUERY_KEYWORDS } from 'diex-shared/constants';
import {
  type EntityTarget,
  type ObjectLiteral,
...
} from 'src/engine/diex-orm/entity-manager/types/relation-nested-query-fields-by-entity-index.type';
import {
  DiexORMException,
  DiexORMExceptionCode,
} from 'src/engine/diex-orm/exceptions/diex-orm.exception';
import { formatConnectRecordNotFoundErrorMessage } from 'src/engine/diex-orm/field-operations/relation-nested-queries/utils/formatConnectRecordNotFoundErrorMessage.util';
import { type WorkspaceSelectQueryBuilder } from 'src/engine/diex-orm/repository/workspace-select-query-builder';
import { computeRelationConnectQueryConfigs } from 'src/engine/diex-orm/utils/compute-relation-connect-query-configs.util';
import { createSqlWhereTupleInClause } from 'src/engine/diex-orm/utils/create-sql-where-tuple-in-clause.utils';
import { extractNestedRelationFieldsByEntityIndex } from 'src/engine/diex-orm/utils/extract-nested-relation-fields-by-entity-index.util';
import { getAssociatedRelationFieldName } from 'src/engine/diex-orm/utils/get-associated-relation-field-name.util';
import { getObjectMetadataFromEntityTarget } from 'src/engine/diex-orm/utils/get-object-metadata-from-entity-target.util';
import { getRecordToConnectFields } from 'src/engine/diex-orm/utils/get-record-to-connect-fields.util';
        ...
        if (
          ...
          if (recordToConnect.length !== 1) {
            ...
            throw new DiexORMException(
              errorMessage,
              DiexORMExceptionCode.CONNECT_RECORD_NOT_FOUND,
              {
                userFriendlyMessage,
              },
            );
