...
import { buildEntitySchemaMetadataMaps } from 'src/engine/diex-orm/global-workspace-datasource/types/entity-schema-metadata.type';
...
> {
  ...
  async computeForCache(workspaceId: string): Promise<EntityMetadata[]> {
    const [objectMetadatas, fieldMetadatas, diexStandardApplication] =
      await Promise.all([
