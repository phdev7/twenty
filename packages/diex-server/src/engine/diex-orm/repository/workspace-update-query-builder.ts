...
import { QUERY_MAX_RECORDS } from 'diex-shared/constants';
...
import { type QueryDeepPartialEntityWithNestedRelationFields } from 'src/engine/diex-orm/entity-manager/types/query-deep-partial-entity-with-nested-relation-fields.type';
import { type RelationConnectQueryConfig } from 'src/engine/diex-orm/entity-manager/types/relation-connect-query-config.type';
import { type RelationDisconnectQueryFieldsByEntityIndex } from 'src/engine/diex-orm/entity-manager/types/relation-nested-query-fields-by-entity-index.type';
import { type WorkspaceEntityManager } from 'src/engine/diex-orm/entity-manager/workspace-entity-manager';
import { computeDiexORMException } from 'src/engine/diex-orm/error-handling/compute-diex-orm-exception';
import {
  DiexORMException,
  DiexORMExceptionCode,
} from 'src/engine/diex-orm/exceptions/diex-orm.exception';
import { FilesFieldSync } from 'src/engine/diex-orm/field-operations/files-field-sync/files-field-sync';
import { RelationNestedQueries } from 'src/engine/diex-orm/field-operations/relation-nested-queries/relation-nested-queries';
import { validateQueryIsPermittedOrThrow } from 'src/engine/diex-orm/repository/permissions.utils';
import { type WorkspaceDeleteQueryBuilder } from 'src/engine/diex-orm/repository/workspace-delete-query-builder';
import { WorkspaceSelectQueryBuilder } from 'src/engine/diex-orm/repository/workspace-select-query-builder';
import { type WorkspaceSoftDeleteQueryBuilder } from 'src/engine/diex-orm/repository/workspace-soft-delete-query-builder';
import { applyRowLevelPermissionPredicates } from 'src/engine/diex-orm/utils/apply-row-level-permission-predicates.util';
import { applyTableAliasOnWhereCondition } from 'src/engine/diex-orm/utils/apply-table-alias-on-where-condition';
import { computeEventSelectQueryBuilder } from 'src/engine/diex-orm/utils/compute-event-select-query-builder.util';
import { formatData } from 'src/engine/diex-orm/utils/format-data.util';
import { formatResult } from 'src/engine/diex-orm/utils/format-result.util';
import { formatDiexOrmEventToDatabaseBatchEvent } from 'src/engine/diex-orm/utils/format-diex-orm-event-to-database-batch-event.util';
import { getObjectMetadataFromEntityTarget } from 'src/engine/diex-orm/utils/get-object-metadata-from-entity-target.util';
import { validateRLSPredicatesForRecords } from 'src/engine/diex-orm/utils/validate-rls-predicates-for-records.util';
...
> extends UpdateQueryBuilder<T> {
    ...
    try {
      ...
      if (before.length > QUERY_MAX_RECORDS) {
        throw new DiexORMException(
          ...
          DiexORMExceptionCode.TOO_MANY_RECORDS_TO_UPDATE,
          {
      ...
      this.internalContext.eventEmitterService.emitDatabaseBatchEvent(
        formatDiexOrmEventToDatabaseBatchEvent({
      ...
      throw await computeDiexORMException(
        error,
        objectMetadata,
  ...
  override select(): WorkspaceSelectQueryBuilder<T> {
    throw new DiexORMException(
      ...
      DiexORMExceptionCode.METHOD_NOT_ALLOWED,
    );
  }
  ...
  private getMainAliasTarget(): EntityTarget<T> {
    ...
    if (!mainAliasTarget) {
      throw new DiexORMException(
        ...
        DiexORMExceptionCode.MISSING_MAIN_ALIAS_TARGET,
      );
    }
