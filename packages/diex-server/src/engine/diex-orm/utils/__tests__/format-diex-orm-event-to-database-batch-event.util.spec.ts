import { type ObjectRecordUpdateEvent } from 'diex-shared/database-events';
import { FieldMetadataType, RelationType } from 'diex-shared/types';
...
describe('formatDiexOrmEventToDatabaseBatchEvent', () => {
  ...
  describe('UPDATED action', () => {
    it('should throw DiexORMException when no matching before entity is found in array of beforeEntities', () => {
      const afterEntities = [
        {
          id: 'record-1',
      ...
      try {
        ...
        expect(error).toBeInstanceOf(DiexORMException);
        expect((error as DiexORMException).code).toBe(
          DiexORMExceptionCode.ORM_EVENT_DATA_CORRUPTED,
        );
        expect((error as DiexORMException).message).toBe(
    ...
    it('should successfully create update events when matching before entities are found and in the right order', () => {
      ...
      const result = formatDiexOrmEventToDatabaseBatchEvent({
        action: DatabaseEventAction.UPDATED,
        objectMetadataItem: flatObjectMetadata,
        flatFieldMetadataMaps,
        workspaceId: mockWorkspaceId,
