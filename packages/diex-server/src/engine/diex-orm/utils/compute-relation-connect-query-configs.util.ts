...
import { type ConnectObject } from 'src/engine/diex-orm/entity-manager/types/query-deep-partial-entity-with-nested-relation-fields.type';
import {
  type RelationConnectQueryConfig,
  type UniqueConstraintCondition,
} from 'src/engine/diex-orm/entity-manager/types/relation-connect-query-config.type';
import { type RelationConnectQueryFieldsByEntityIndex } from 'src/engine/diex-orm/entity-manager/types/relation-nested-query-fields-by-entity-index.type';
import {
  DiexORMException,
  DiexORMExceptionCode,
} from 'src/engine/diex-orm/exceptions/diex-orm.exception';
import { formatCompositeField } from 'src/engine/diex-orm/utils/format-data.util';
...
const computeRecordToConnectCondition = (
  ...
  if (
    ...
    throw new DiexORMException(
      ...
      DiexORMExceptionCode.CONNECT_NOT_ALLOWED,
      {
...
const checkUniqueConstraintFullyPopulated = (
  ...
  if (!hasUniqueConstraintFieldFullyPopulated) {
    throw new DiexORMException(
      ...
      DiexORMExceptionCode.CONNECT_UNIQUE_CONSTRAINT_ERROR,
      {
