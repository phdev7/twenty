...
export class SystemPromptBuilderService {
  ...
  async buildPreview(
    ...
    const operatingContextPrompt =
      await this.getOperatingContextPrompt(workspaceId);

    if (operatingContextPrompt) {
      sections.push({
        title: 'Diex Operating Context',
        content: operatingContextPrompt,
        estimatedTokenCount: estimateTokenCount(operatingContextPrompt),
      });
    }
  ...
  buildFullPrompt(
    ...
    if (operatingContextPrompt) {
      parts.push(this.buildOperatingContextSection(operatingContextPrompt));
    }
  ...
  async getOperatingContextPrompt(workspaceId: string): Promise<string | null> {
    if (!this.workspaceArchitectureService) {
      return null;
    }

    return this.workspaceArchitectureService.getAiOperatingContextPrompt(
      workspaceId,
    );
  }

  buildOperatingContextSection(prompt: string): string {
    return `\n${prompt}`;
  }
...
${normalizeDiexBrandText(instructions)}`;
  }
...
In your Python code, access files at \`/home/user/{filename}\`.`;
  ...
  buildSkillCatalogSection(skillCatalog: FlatSkill[]): string {
    ...
    const skillsList = skillCatalog
      .map(
        (skill) =>
          `- \`${normalizeDiexBrandText(skill.name)}\`: ${normalizeDiexBrandText(skill.description ?? skill.label)}`,
      )
      .join('\n');
