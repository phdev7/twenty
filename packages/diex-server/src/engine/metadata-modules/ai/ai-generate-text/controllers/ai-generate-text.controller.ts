...
export class AiGenerateTextController {
  ...
  async handleGenerateText(
    ...
    await this.aiBillingService.assertWorkspaceCanUseAi(workspace.id);
    ...
    try {
      const operatingContextPrompt =
        await this.workspaceArchitectureService.getAiOperatingContextPrompt(
          workspace.id,
        );
      result = await generateText({
        model: registeredModel.model,
        system: [body.systemPrompt, operatingContextPrompt]
          .filter(Boolean)
          .join('\n\n'),
