...
import { ensureAbsoluteUrl } from 'diex-shared/utils';
...
export class CallWebhookJob {
  ...
  private async callWebhook(data: WebhookJobData): Promise<void> {
    ...
    try {
      ...
      if (secret) {
        headers['X-Diex-Webhook-Timestamp'] = Date.now().toString();
        headers['X-Diex-Webhook-Signature'] = this.generateSignature(
          payloadWithoutSecret,
          secret,
          headers['X-Diex-Webhook-Timestamp'],
        );
        headers['X-Diex-Webhook-Nonce'] = crypto
          .randomBytes(16)
          .toString('hex');
      }
