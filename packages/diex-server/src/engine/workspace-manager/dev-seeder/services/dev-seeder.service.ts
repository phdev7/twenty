...
import { DiexStandardApplicationService } from 'src/engine/workspace-manager/diex-standard-application/services/diex-standard-application.service';
...
export class DevSeederService {
  constructor(
    ...
    private readonly diexStandardApplicationService: DiexStandardApplicationService,
  ...
  public async seedDev(
    ...
    const isBillingEnabled = this.diexConfigService.get('IS_BILLING_ENABLED');
    const appVersion = this.diexConfigService.get('APP_VERSION') ?? 'unknown';
    ...
    const { workspaceCustomFlatApplication, diexStandardFlatApplication } =
    ...
    await this.diexStandardApplicationService.synchronizeDiexStandardApplicationOrThrow(
      {
        workspaceId,
      },
    );
    ...
    await this.sdkClientGenerationService.generateSdkClientForApplication({
      workspaceId,
      applicationId: diexStandardFlatApplication.id,
    ...
    await seedPageLayouts({
      workspaceId,
      flatApplication: diexStandardFlatApplication,
      objectMetadataItems,
  ...
  private async seedCoreSchema({
    ...
    try {
      ...
      await this.applicationService.createDiexStandardApplication(
        {
          workspaceId,
