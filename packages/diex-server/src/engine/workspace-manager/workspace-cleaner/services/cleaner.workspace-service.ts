...
export class CleanerWorkspaceService {
  ...
  constructor(
    ...
    this.inactiveDaysBeforeSoftDelete = this.diexConfigService.get(
      'WORKSPACE_INACTIVE_DAYS_BEFORE_SOFT_DELETION',
    );
    this.inactiveDaysBeforeDelete = this.diexConfigService.get(
      'WORKSPACE_INACTIVE_DAYS_BEFORE_DELETION',
    );
    this.inactiveDaysBeforeWarn = this.diexConfigService.get(
  ...
  async batchCleanOnboardingWorkspaces(
    ...
    const cleanableActivationStatuses = this.diexConfigService.get(
