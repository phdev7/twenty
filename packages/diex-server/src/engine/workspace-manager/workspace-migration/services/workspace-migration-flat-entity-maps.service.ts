...
import { AllMetadataName } from 'diex-shared/metadata';
...
export class WorkspaceMigrationFlatEntityMapsService {
  ...
  private computeAllInvolvedApplicationIds({
    ...
    if (!isDefined(diexStandardApplicationId)) {
      throw new FlatEntityMapsException(
        'Diex standard application not found in workspace',
    ...
    const isBuildingDiexStandardApplication =
      applicationUniversalIdentifier ===
      DIEX_STANDARD_APPLICATION.universalIdentifier;

    if (!isBuildingDiexStandardApplication) {
      applicationIds.add(diexStandardApplicationId);
    }
