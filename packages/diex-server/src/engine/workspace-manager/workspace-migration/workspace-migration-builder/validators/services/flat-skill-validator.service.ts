...
import { ALL_METADATA_NAME } from 'diex-shared/metadata';
...
export class FlatSkillValidatorService {
  ...
  >): FailedFlatEntityValidation<'skill', 'delete'> {
    ...
    if (
      !isCallerDiexStandardApp(buildOptions) &&
      belongsToDiexStandardApp({
  ...
  >): FailedFlatEntityValidation<'skill', 'update'> {
    ...
    const isDiexStandardSkill = belongsToDiexStandardApp({
