...
if (process.env.EXCEPTION_HANDLER_DRIVER === ExceptionHandlerDriver.SENTRY) {
  Sentry.init({
    ...
    beforeSendSpan: (span) => {
      const diexContext = Sentry.getIsolationScope().getScopeData().contexts
        ?.diex as
        | {
            workspace_id?: string;
      ...
      if (!diexContext?.workspace_id) {
        return span;
      }

      span.data = {
        ...span.data,
        'diex.workspace.id': diexContext.workspace_id,
        ...(diexContext.user_workspace_id && {
          'diex.user_workspace.id': diexContext.user_workspace_id,
        }),
      };
