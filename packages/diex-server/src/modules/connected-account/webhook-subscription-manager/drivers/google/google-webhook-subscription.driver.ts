...
import { WebhookSubscriptionChannelType } from 'diex-shared/types';
...
export class GoogleWebhookSubscriptionDriver implements WebhookSubscriptionDriver {
  ...
  private async watchGmailMailbox(
    ...
    const pubSubTopicName = this.diexConfigService.get(
  ...
  private async watchPrimaryCalendar(
    ...
    const notificationAddress = `${this.diexConfigService.get('SERVER_URL')}/webhooks/google/calendar`;
