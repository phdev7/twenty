...
jest.mock(
  'src/engine/diex-orm/storage/orm-workspace-context.storage',
  () => ({
...
jest.mock(
  'src/engine/diex-orm/utils/resolve-role-permission-config.util',
  () => ({
...
describe('ChartRelationLabelService', () => {
  ...
  it('should preserve the record id through the real formatResult so labels resolve', async () => {
    const actualFormatResult = jest.requireActual(
      'src/engine/diex-orm/utils/format-result.util',
    ).formatResult;
