...
export class EvolutionProvisioningService {
  ...
  async resolveProvisioning(
    ...
    const serverBaseUrl = asString(
      this.diexConfigService.get('DIEX_EVOLUTION_SERVER_BASE_URL'),
    );
    const serverApiKey = asString(
      this.diexConfigService.get('DIEX_EVOLUTION_SERVER_API_KEY'),
    );
    const serverWebhookSecret = asString(
      this.diexConfigService.get('DIEX_EVOLUTION_SERVER_WEBHOOK_SECRET'),
    );
  ...
  buildWebhookUrl(): string {
    const apiUrl = (this.diexConfigService.get('SERVER_URL') ?? '').replace(
      /\/+$/,
      '',
    );
    ...
    ].includes(this.diexConfigService.get('NODE_ENV'));
  ...
  private extractQrCode(payload: Record<string, unknown>): string | null {
    const asQrDataUri = (value: unknown): string | null => {
    ...
    const readImagePayload = (value: unknown): string | null => {
      const direct = asQrDataUri(value);
      ...
      if (value && typeof value === 'object') {
        ...
        return (
          readImagePayload(nested.base64) ??
          readImagePayload(nested.qrCode) ??
          readImagePayload(nested.qrcode)
        );
      }
    ...
    return (
      readImagePayload(payload.base64) ??
      readImagePayload(payload.qrCode) ??
      readImagePayload(payload.qrcode)
    );
  }
