...
import {
  type InboxMessageDeliveryStatus,
...
const getMessageTypeAndBody = (
  ...
  if (
    ...
    return { type: 'IMAGE', body: text ?? null };
  }
  ...
  if (
    ...
    return { type: 'VIDEO', body: text ?? null };
  }
  ...
  if (
    ...
    return { type: 'AUDIO', body: text ?? null };
  }
  ...
  if (
    ...
    return { type: 'DOCUMENT', body: text ?? null };
  }
  ...
  if (
    ...
    return { type: 'REACTION', body: text ?? null };
  }

  if (text || typeHint.includes('conversation') || typeHint.includes('text')) {
    return { type: 'TEXT', body: text ?? null };
  }
...
export const extractEvolutionInstanceName = (payload: unknown): string | null =>
  firstString(payload, [
    'instance',
    'instanceName',
    'data.instance',
...
export const normalizeEvolutionMessages = (
  ...
  return getEvolutionDataItems(payloadRecord).flatMap((item) => {
    ...
    const senderDisplayName =
      firstString(item, ['pushName', 'contactName', 'name', 'contact.name']) ??
      null;
    ...
    return [
      {
        ...
        sentAt,
        senderDisplayName,
        deliveryStatus: fromMe ? ('SENT' as const) : ('RECEIVED' as const),
...
export const normalizeEvolutionStatuses = (
  ...
  return getEvolutionDataItems(payloadRecord).flatMap((item) => {
    const externalMessageId = firstString(item, ['key.id', 'id', 'messageId']);
...
const MESSAGE_PREVIEW_FALLBACK: Record<InboxMessageType, string> = {
...
export const buildMessagePreview = (
  ...
  (message.body?.trim() || MESSAGE_PREVIEW_FALLBACK[message.type]).slice(
    0,
    250,
  );
...
export const readExternalMessageId = (
  ...
  const externalId =
    separatorIndex === -1 ? key : key.slice(separatorIndex + 1);
