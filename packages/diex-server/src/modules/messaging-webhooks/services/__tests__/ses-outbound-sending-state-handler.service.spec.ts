...
describe('SesOutboundSendingStateHandlerService.handle', () => {
  ...
  // the event, since every diex-managed resource shares the same prefix.
  describe.each([
    {
  ...
  // diex-managed name prefix; the handler must noop in that case.
  it('does not update any workspace when the ARN does not carry the diex-managed prefix', async () => {
    const { service, emailingDomainTenantStatusService } = setUp();
