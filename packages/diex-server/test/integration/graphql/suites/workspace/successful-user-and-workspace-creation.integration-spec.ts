...
describe('Successful user and workspace creation', () => {
  ...
  it('should sign up a new user and create a new workspace successfully', async () => {
    ...
    const diexStandardApp = findManyApplicationsData.find(
      (application) =>
        application.universalIdentifier ===
        DIEX_STANDARD_APPLICATION.universalIdentifier,
    );

    jestExpectToBeDefined(diexStandardApp);
    const {
      sourcePath: _sourcePath,
      sourceType: _sourceType,
      ...expectedStandardDiexApplication
    } = DIEX_STANDARD_APPLICATION;

    expect(diexStandardApp).toMatchObject(expectedStandardDiexApplication);
