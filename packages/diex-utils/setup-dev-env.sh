#!/bin/bash
# =============================================================================
# Diex CRM — Development Environment Setup
# =============================================================================
...
#   bash packages/diex-utils/setup-dev-env.sh          # start + configure
#   bash packages/diex-utils/setup-dev-env.sh --down    # stop services
#   bash packages/diex-utils/setup-dev-env.sh --reset   # wipe data + restart
#   bash packages/diex-utils/setup-dev-env.sh --docker  # force Docker mode
# =============================================================================
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
COMPOSE_FILE="$REPO_ROOT/packages/diex-docker/docker-compose.dev.yml"
...
if command -v npx &>/dev/null && [ -d node_modules ]; then
  npx nx reset:env diex-front
  npx nx reset:env diex-server
else
  for pkg in diex-front diex-server; do
...
if command -v npx &>/dev/null && [ -d node_modules ]; then
  if schema_exists; then
    ...
    if npx nx database:init diex-server; then
  ...
  info "Run 'npx nx database:init diex-server' to initialize the schema"
fi
...
echo "  npx nx start diex-front          # frontend  -> http://localhost:3001"
echo "  npx nx start diex-server         # backend   -> http://localhost:3000"
echo ""
