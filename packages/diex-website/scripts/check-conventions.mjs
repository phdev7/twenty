...
// only serve by URL (plus diex.svg, the data layer's static export of
// src/icons/diex-logo.tsx for the mockup's brand-image-by-URL path).
const PUBLIC_SVG_BRAND_FILES = new Set([
  ...
  'public/images/shared/companies/logos/diex.svg',
  ...
  'public/images/shared/halftone/diex-logo.svg',
]);
...
// paths keep resolving once this package takes over the diex-website
...
const ROW_GAP_MULTI_AXIS_FILES = new Set([
  ...
  'sections/why-diex-editorial/Editorial.tsx',
]);
...
function walk(directory) {
  for (const entry of fs.readdirSync(directory, { withFileTypes: true })) {
    ...
    // diex-ui's theme is pure data, baked by Linaria at build time — consume
    ...
    // the theme.
    const badDiexUiSubpath = [
      ...content.matchAll(/from 'diex-ui(\/[a-z-]+)?'/g),
    ]
      .map((match) => match[1] ?? '')
      .find(
        (subpath) => subpath !== '/theme' && subpath !== '/theme-constants',
      );
    if (badDiexUiSubpath !== undefined) {
      failures.push(
        `src/${relativePath}: only diex-ui/theme is importable (pure data, baked at build); diex-ui${badDiexUiSubpath} pulls React runtime into the bundle — build a lean primitive instead.`,
      );
    }
