...
describe('buildLogicFunctionPayload', () => {
  ...
  it('forwards diex experience fields to the webhook payload', () => {
    const payload = buildLogicFunctionPayload(fullValid);
    expect(payload.diexExperience).toEqual(['WORKFLOWS']);
    expect(payload.diexExperienceNotes).toBe(validExperienceNotes);
    expect(payload.diexExperienceProofLink).toBe(
