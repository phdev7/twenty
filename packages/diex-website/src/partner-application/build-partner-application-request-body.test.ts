...
describe('buildPartnerApplicationRequestBody', () => {
  ...
  it('trims the required string fields', () => {
    ...
    expect(body.diexExperienceNotes).toBe(validExperienceNotes);
    expect(body.diexExperienceProofLink).toBe(
  ...
  it('omits enum/array fields when unset and forwards them when set', () => {
    ...
    const filled = buildPartnerApplicationRequestBody({
      ...
      diexExperience: ['CUSTOM_APPS', 'FRONT_COMPONENTS'],
    });
    ...
    expect(filled.diexExperience).toEqual([
      'CUSTOM_APPS',
      'FRONT_COMPONENTS',
    ]);
  });
