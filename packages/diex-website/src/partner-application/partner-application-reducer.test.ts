...
describe('partnerApplicationReducer', () => {
  ...
  it('starts at stepIndex 0 with empty fields', () => {
    ...
    expect(INITIAL_PARTNER_APPLICATION_STATE.diexExperience).toEqual([]);
  });
  ...
  it('TOGGLE_EXPERIENCE adds and then removes a milestone', () => {
    ...
    expect(added.diexExperience).toEqual(['CUSTOM_APPS']);
    const removed = partnerApplicationReducer(added, {
      type: 'TOGGLE_EXPERIENCE',
      value: 'CUSTOM_APPS',
    });
    expect(removed.diexExperience).toEqual([]);
  });
  ...
  it('GO_NEXT on Experience gates on milestones, narrative length, and proof URL', () => {
    ...
    expect(blocked.fieldErrors.diexExperience).toBe('required');
    expect(blocked.fieldErrors.diexExperienceNotes).toBe('required');
    expect(blocked.fieldErrors.diexExperienceProofLink).toBe('required');

    const shortNotes = partnerApplicationReducer(
      {
        ...onExperience,
        diexExperience: ['WORKFLOWS'],
        diexExperienceNotes: 'Too short.',
        diexExperienceProofLink: 'https://www.loom.com/share/example',
      },
      { type: 'GO_NEXT' },
    );
    expect(shortNotes.stepIndex).toBe(3);
    expect(shortNotes.fieldErrors.diexExperienceNotes).toBe('too_short');
