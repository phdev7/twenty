...
export function partnerApplicationReducer(
  ...
  switch (action.type) {
    ...
    case 'TOGGLE_EXPERIENCE': {
      const next = state.diexExperience.includes(action.value)
        ? state.diexExperience.filter((value) => value !== action.value)
        : [...state.diexExperience, action.value];
      return {
        ...state,
        diexExperience: next,
        fieldErrors: dropError(state.fieldErrors, 'diexExperience'),
      };
    }
    case 'SET_SKILLS':
