...
import { IconMail, IconMessage, IconNotes } from 'twenty-ui/icon';
...
const getInitialComposerMode = (
  ...
  return isEvolution || isTwentyEmail ? 'EXTERNAL' : 'INTERNAL';
};
...
const getInitialExternalSubject = (conversation: InboxConversation): string => {
  ...
  if (!isTwentyEmail || sourceSubject.length === 0) {
    return '';
  }
  ...
  return (
    <StyledFooter>
      ...
      <StyledHint>
        {composerMode === 'EXTERNAL'
          ? isEmailConversation
            ? 'O envio usa a conta nativa do Twenty disponível ao seu usuário. Revise destinatário, assunto e corpo antes de confirmar.'
