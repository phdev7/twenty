...
@Injectable()
export class AiGovernanceToolProvider implements ToolProvider {
  readonly category = ToolCategory.DIEX;

  constructor(
    private readonly aiGovernanceToolService: AiGovernanceToolWorkspaceService,
  ...
  private buildToolSet(context: ToolProviderContext): ToolSet {
    return this.aiGovernanceToolService.generateAiGovernanceTools(
      context.workspaceId,
    );
  }
}
