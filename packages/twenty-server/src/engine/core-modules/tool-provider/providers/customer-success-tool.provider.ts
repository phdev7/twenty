...
@Injectable()
export class CustomerSuccessToolProvider implements ToolProvider {
  readonly category = ToolCategory.DIEX;

  constructor(
    private readonly customerSuccessToolService: CustomerSuccessToolWorkspaceService,
  ...
  private buildToolSet(context: ToolProviderContext): ToolSet {
    return this.customerSuccessToolService.generateCustomerSuccessTools(
      context.workspaceId,
    );
  }
}
