...
export class InboxTranscriptionService {
  ...
  // not have to paste it twice to get audio turned into text.
  readOpenAiApiKey(): string | null {
    for (const name of ['DIEX_OPENAI_API_KEY', 'OPENAI_API_KEY'] as const) {
      const key = this.twentyConfigService.get(name)?.trim();
  ...
  async transcribeAudio({
    ...
    const audio = base64ToArrayBuffer(base64);

    // Failed rather than unavailable: the size belongs to this audio and will
    // not change, so the queue must not keep coming back to it.
    ...
    // Portuguese is the operating language of this inbox; stating it keeps
    // Whisper from guessing a language on short, noisy voice notes.
